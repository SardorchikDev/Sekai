...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.sekai/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.sekai/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let sekai_dir = PathBuf::from(home).join(".sekai");
    let auth_profiles_path = sekai_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: sekai auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: sekai auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.sekai/auth-profiles.json (with expired token)
    let provider = sekai::providers::create_provider("gemini", None)?;
