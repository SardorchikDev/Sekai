...
use sekai::config::migration::{self, V1Compat};
use sekai::config::{AutonomyConfig, ChannelsConfig, Config, GatewayConfig, SecurityConfig};
...
fn gateway_path_prefix_rejects_missing_leading_slash() {
    ...
    config.gateway.path_prefix = Some("sekai".into());
...
fn gateway_path_prefix_accepts_valid_prefixes() {
    for prefix in ["/sekai", "/apps/sekai", "/api/hassio_ingress/abc123"] {
        let mut config = Config::default();
...
[channels.whatsapp]
session_path = "~/.sekai/state/whatsapp-web/session.db"
allowed_numbers = ["*"]
"#;
    let parsed: Config = toml::from_str(toml_str)
    ...
    assert_eq!(
        ...
        Some("~/.sekai/state/whatsapp-web/session.db")
    );
