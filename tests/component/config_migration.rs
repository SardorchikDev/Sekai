...
use sekai::config::migration::{self, CURRENT_SCHEMA_VERSION, V1Compat};

fn migrate(toml_str: &str) -> sekai::config::Config {
...
fn exhaustive_walk_no_props_lost() {
    use sekai::config::{Config, ModelProviderConfig};
...
"#;
    ...
    let known_keys = {
        let mut keys: Vec<String> = toml::to_string(&sekai::config::Config::default())
            .ok()
