#!/bin/bash
# Sekai Telegram Integration Test Suite
...
if [ -f "target/release/sekai" ]; then
    BINARY_SIZE=$(ls -lh target/release/sekai | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/sekai 2>/dev/null || stat -c%s target/release/sekai)
...
CONFIG_PATH="$HOME/.sekai/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'sekai onboard' first"
    fi
else
    warn "No config file found - run 'sekai onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/sekai channel doctor 2>&1 || true)
END_TIME=$(date +%s)
