...
This guide covers testing the Telegram channel integration for Sekai.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug sekai channel start
    ```
...
Solution: Check user allowlist
  ...
  4. Run: sekai onboard --channels-only
```
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time sekai channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep sekai`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/sekai` |
...
```bash
# Maximum logging
RUST_LOG=trace sekai channel start
...
# Validate config
cat ~/.sekai/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [Sekai Main README](../../README.md)
- [Contributing Guide](../../CONTRIBUTING.md)
- [Issue Tracker](https://github.com/sekai-labs/sekai/issues)
