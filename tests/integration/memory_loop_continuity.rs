...
use sekai::memory::sqlite::SqliteMemory;
use sekai::memory::traits::{Memory, MemoryCategory};
use sekai::providers::ToolCall;
...
async fn compressor_with_memory_saves_summary() {
    use sekai::agent::context_compressor::{ContextCompressionConfig, ContextCompressor};
    use sekai::providers::traits::ChatMessage;
...
async fn consolidation_extracts_facts_to_memory() {
    ...
    let result = sekai::memory::consolidation::consolidate_turn(
        &provider,
        "test-model",
        mem.as_ref(),
...
async fn memory_survives_rapid_consolidation() {
    ...
    for i in 0..10 {
        ...
        let _ = sekai::memory::consolidation::consolidate_turn(
            &provider,
            "test-model",
            mem.as_ref(),
...
async fn session_backend_persists_messages() {
    use sekai::channels::session_backend::SessionBackend;
    use sekai::channels::session_sqlite::SqliteSessionBackend;
    use sekai::providers::traits::ChatMessage;
