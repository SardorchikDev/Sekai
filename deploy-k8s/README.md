# Sekai OpenShift deployment

Deploy a minimal Sekai agent on OpenShift with an external LLM
...
1. Update the `image` field in `deployment.yaml` to point to your
   registry (e.g., `ghcr.io/youruser/sekai:latest`)
...
```bash
oc -n sekai get pods
oc -n sekai get route sekai
```

Test the health endpoint:

```bash
ROUTE=$(oc -n sekai get route sekai -o jsonpath='{.spec.host}')
...
oc -n sekai rollout restart deployment sekai
```
