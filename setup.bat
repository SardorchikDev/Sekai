...
:: ============================================================================
:: Sekai Windows Setup Script
:: Simplifies building and installing Sekai on Windows.
...
set "REPO=https://github.com/sekai-labs/sekai"
...
echo %BOLD%%BLUE%  Sekai Windows Setup  v%VERSION%%RESET%
echo %BOLD%%BLUE%=========================================%RESET%
echo.
...
if not defined DOWNLOAD_URL (
    ...
    set "DOWNLOAD_URL=https://github.com/sekai-labs/sekai/releases/latest/download/sekai-%TARGET%.zip"
)

echo   Downloading from release...
curl -sSfL -o "%TEMP%\sekai-windows.zip" "!DOWNLOAD_URL!"
if %ERRORLEVEL% NEQ 0 (
...
:: Extract
echo   Extracting...
mkdir "%USERPROFILE%\.sekai\bin" 2>nul
tar -xf "%TEMP%\sekai-windows.zip" -C "%USERPROFILE%\.sekai\bin"
if %ERRORLEVEL% NEQ 0 (
    powershell -Command "Expand-Archive -Force '%TEMP%\sekai-windows.zip' '%USERPROFILE%\.sekai\bin'"
)

:: Add to PATH if not already there
echo %PATH% | findstr /I /C:".sekai\bin" >nul 2>&1
if %ERRORLEVEL% NEQ 0 (
    setx PATH "%PATH%;%USERPROFILE%\.sekai\bin" >nul 2>&1
    set "PATH=%PATH%;%USERPROFILE%\.sekai\bin"
    echo   %GREEN%OK%RESET% Added to PATH
)

echo   %GREEN%OK%RESET% Binary installed to %USERPROFILE%\.sekai\bin\sekai.exe
goto verify
...
:do_build
echo.
echo %BOLD%[3/5] Building Sekai (%BUILD_DESC%)...%RESET%
echo   Target: %TARGET%

:: Ensure we're in the repo root (check for Cargo.toml)
if not exist "Cargo.toml" (
    echo   %RED%ERROR: Cargo.toml not found. Run this script from the sekai repository root.%RESET%
    echo   Example:
    echo     git clone %REPO%
    echo     cd sekai
    echo     setup.bat
...
mkdir "%USERPROFILE%\.sekai\bin" 2>nul
copy /Y "target\%TARGET%\release\sekai.exe" "%USERPROFILE%\.sekai\bin\sekai.exe" >nul
echo   %GREEN%OK%RESET% Installed to %USERPROFILE%\.sekai\bin\sekai.exe
...
"%USERPROFILE%\.sekai\bin\sekai.exe" --version >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    for /f "tokens=*" %%v in ('"%USERPROFILE%\.sekai\bin\sekai.exe" --version 2^>nul') do (
        echo   %GREEN%OK%RESET% %%v
    )
) else (
    sekai --version >nul 2>&1
    if %ERRORLEVEL% EQU 0 (
        for /f "tokens=*" %%v in ('sekai --version 2^>nul') do (
            echo   %GREEN%OK%RESET% %%v
        )
    ) else (
...
echo %BOLD%%GREEN%  Sekai setup complete!%RESET%
echo %BOLD%%GREEN%=========================================%RESET%
echo.
echo   Next steps:
echo     1. Restart your terminal (for PATH changes)
echo     2. Run: sekai onboard
echo     3. Configure your API key in %%USERPROFILE%%\.sekai\config.toml
echo.
echo   Alternative install via Scoop:
echo     scoop bucket add sekai https://github.com/sekai-labs/scoop-sekai
echo     scoop install sekai
echo.
echo   Documentation: https://github.com/sekai-labs/sekai
echo.
goto :end
...
:show_help
echo.
echo Sekai Windows Setup Script
echo.
