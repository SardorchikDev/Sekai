//! Performance benchmarks for Sekai hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: <https://github.com/sekai-labs/sekai/issues/618> (item 7)

use criterion::{Criterion, criterion_group, criterion_main};
use std::hint::black_box;
use std::sync::{Arc, Mutex};

use sekai::agent::agent::Agent;
use sekai::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use sekai::config::MemoryConfig;
use sekai::memory;
use sekai::memory::{Memory, MemoryCategory};
use sekai::observability::{NoopObserver, Observer};
use sekai::providers::{ChatRequest, ChatResponse, Provider, ToolCall};
use sekai::tools::{Tool, ToolResult};
...
<tool_call>
{"name": "search", "arguments": {"query": "sekai architecture"}}
</tool_call>
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about sekai agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("sekai agent"), 10, None, None, None)
                    .await
                    .unwrap()
            })
        });
