...
# ── Step 1: Build ────────────────────────────────────────────────────
echo -e "${BOLD}[1/3] Building sekai (${PROFILE})...${RESET}"
if [[ "$PROFILE" == "release" ]]; then
  cargo build --release 2>&1
  BIN="$REPO_ROOT/target/release/sekai"
else
  cargo build 2>&1
  BIN="$REPO_ROOT/target/debug/sekai"
fi
...
echo "  [ ] Welcome screen renders with SEKAI banner"
