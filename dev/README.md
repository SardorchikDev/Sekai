# Sekai Development Environment

A fully containerized development sandbox for Sekai agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`sekai-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `sekai` CLI commands manually, debug the binary, or check logs internally.
...
The agent configuration lives in `target/.sekai` (mounted to `/sekai-data/.sekai`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.sekai` (config/DB) but leaves the `playground/` directory intact. To fully wipe workspace data, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `sekai-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=sekai-target`) to speed repeat local image builds.
