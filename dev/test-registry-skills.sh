...
# Installs every skill from sekai-labs/sekai-skills by bare name,
...
# ── Resolve sekai binary ─────────────────────────────────────
SEKAI=""
for candidate in \
  "$REPO_ROOT/target/debug/sekai" \
  "$REPO_ROOT/target/release/sekai" \
  "$(command -v sekai 2>/dev/null || true)"; do
...
if [ -z "$SEKAI" ]; then
  printf "${RED}Error: No sekai binary found. Run 'cargo build' first.${RESET}\n"
  exit 1
fi
...
SKILLS_DIR="$HOME/.sekai/workspace/skills"
REGISTRY_DIR="$HOME/.sekai/workspace/skills-registry"
...
if [ ! -d "$REGISTRY_DIR/skills" ]; then
  ...
  "$SEKAI" skills install __bootstrap_probe__ 2>&1 || true
fi
...
for skill in $REGISTRY_SKILLS; do
  OUTPUT=$("$SEKAI" skills install "$skill" 2>&1) || true
...
LIST_OUTPUT=$("$SEKAI" skills list 2>&1)
...
ERR_OUTPUT=$("$SEKAI" skills install nonexistent-skill-xyz 2>&1 || true)
...
DUP_OUTPUT=$("$SEKAI" skills install auto-coder 2>&1 || true)
...
if [ -f "$REGISTRY_DIR/.sekai-skills-registry-sync" ]; then
...
for skill in $INSTALLED_SKILLS; do
  REMOVE_OUTPUT=$("$SEKAI" skills remove "$skill" 2>&1) || true
