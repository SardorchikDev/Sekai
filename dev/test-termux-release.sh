...
TAG="v${VERSION}"
ASSET_NAME="sekai-${TARGET}.tar.gz"
ASSET_URL="https://github.com/sekai-labs/sekai/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t sekai-termux-test-XXXXXX)"
...
# --- Test 5: Contains sekai binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^sekai$"; then
  pass "Archive contains 'sekai' binary"
else
  fail "Archive does not contain 'sekai' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/sekai-labs/sekai/releases/download/${TAG}/SHA256SUMS"
