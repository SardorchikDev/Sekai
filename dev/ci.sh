...
build_smoke_image() {
  if docker buildx version >/dev/null 2>&1; then
    ...
    DOCKER_BUILDKIT=1 docker build --target dev -t sekai-local-smoke:latest .
  fi
}

print_help() {
  cat <<'EOF'
Sekai Local CI in Docker
...
case "$1" in
  ...
  docker-smoke)
    build_smoke_image
    docker run --rm sekai-local-smoke:latest --version
    ;;
