#!/usr/bin/env bash
# Launch the Sekai Tauri desktop app in dev mode.
#
# Assumes the gateway is reachable on 127.0.0.1:42617 (locally running
# `sekai gateway`, or an SSH port-forward from a remote host).
#
...
pkill -f 'target/debug/sekai-desktop' 2>/dev/null || true
sleep 0.5
