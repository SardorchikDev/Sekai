...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/sekai-labs/sekai/security/advisories/new)
...
Sekai implements defense-in-depth security:
...
Sekai Docker images follow CIS Docker Benchmark best practices:
...
docker build -t sekai .
docker inspect --format='{{.Config.User}}' sekai
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace sekai gateway
```
