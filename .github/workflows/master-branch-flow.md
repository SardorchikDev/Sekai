...
Sekai uses a single default branch: `master`. All contributor PRs target
...
2. `ci.yml` runs:
   - `lint` — `cargo fmt --all -- --check`, `cargo clippy --workspace
     --exclude sekai-desktop --all-targets --features ci-all -- -D warnings`
     (PRs only).
   ...
   - `test` — `cargo nextest run --locked --workspace --exclude sekai-desktop` on `ubuntu-latest`.
