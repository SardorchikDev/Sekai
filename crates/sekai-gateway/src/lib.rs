...
use uuid::Uuid;
use sekai_api::channel::{Channel, SendMessage};
use sekai_api::tool::ToolSpec;
use sekai_channels::{
...
use sekai_config::policy::SecurityPolicy;
use sekai_config::schema::Config;
use sekai_infra::session_backend::SessionBackend;
use sekai_memory::{self, Memory, MemoryCategory};
use sekai_providers::{self, Provider};
use sekai_runtime::cost::CostTracker;
use sekai_runtime::i18n;
use sekai_runtime::platform;
use sekai_runtime::security::pairing::{PairingGuard, constant_time_eq, is_public_bind};
use sekai_runtime::tools;
use sekai_runtime::tools::CanvasStore;
...
/// Read gateway request timeout from `SEKAI_GATEWAY_TIMEOUT_SECS` env var
...
pub fn gateway_request_timeout_secs() -> u64 {
    std::env::var("SEKAI_GATEWAY_TIMEOUT_SECS")
        .ok()
...
/// `SEKAI_GATEWAY_LONG_RUNNING_REQUEST_TIMEOUT_SECS` at runtime, falling back to
...
pub fn gateway_long_running_request_timeout_secs() -> u64 {
    std::env::var("SEKAI_GATEWAY_LONG_RUNNING_REQUEST_TIMEOUT_SECS")
        .ok()
        .and_then(|v| v.parse().ok())
...
fn whatsapp_memory_key(msg: &sekai_api::channel::ChannelMessage) -> String {
    format!("whatsapp_{}_{}", msg.sender, msg.id)
}

fn linq_memory_key(msg: &sekai_api::channel::ChannelMessage) -> String {
    format!("linq_{}_{}", msg.sender, msg.id)
}

fn wati_memory_key(msg: &sekai_api::channel::ChannelMessage) -> String {
    format!("wati_{}_{}", msg.sender, msg.id)
}

fn nextcloud_talk_memory_key(msg: &sekai_api::channel::ChannelMessage) -> String {
    format!("nextcloud_talk_{}_{}", msg.sender, msg.id)
}

fn sender_session_id(channel: &str, msg: &sekai_api::channel::ChannelMessage) -> String {
    match &msg.thread_ts {
...
pub struct AppState {
    ...
    pub observer: Arc<dyn sekai_runtime::observability::Observer>,
    ...
    /// subsystem in place. `None` when running standalone (`sekai gateway start`)
...
pub async fn run_gateway(
    ...
    // ── Hooks ──────────────────────────────────────────────────────
    let hooks: Option<std::sync::Arc<sekai_runtime::hooks::HookRunner>> = if config.hooks.enabled
    {
        Some(std::sync::Arc::new(
            sekai_runtime::hooks::HookRunner::new(),
        ))
    } else {
        None
    };
    ...
    let provider: Arc<dyn Provider> =
        Arc::from(sekai_providers::create_resilient_provider_with_options(
            provider_name,
    ...
    let whatsapp_app_secret: Option<Arc<str>> = std::env::var("SEKAI_WHATSAPP_APP_SECRET")
        .ok()
        .and_then(|secret| {
    ...
    // Priority: environment variable > config file
    let linq_signing_secret: Option<Arc<str>> = std::env::var("SEKAI_LINQ_SIGNING_SECRET")
        .ok()
        .and_then(|secret| {
            let secret = secret.trim();
            (!secret.is_empty()).then(|| secret.to_owned())
    ...
    let nextcloud_talk_webhook_secret: Option<Arc<str>> =
        std::env::var("SEKAI_NEXTCLOUD_TALK_WEBHOOK_SECRET")
            .ok()
            .and_then(|secret| {
                let secret = secret.trim();
                (!secret.is_empty()).then(|| secret.to_owned())
    ...
    // ── Tunnel ────────────────────────────────────────────────
    let tunnel = sekai_runtime::tunnel::create_tunnel(&config.tunnel)?;
    ...
    let web_dist_dir: Option<std::path::PathBuf> = config
        ...
        .or_else(|| {
            ...
            let mut candidates = vec![
                ...
                std::path::PathBuf::from("/sekai-data/web/dist"),
                // AUR / system package
                std::path::PathBuf::from("/usr/share/sekailabs/web/dist"),
            ];
            // XDG data home (prebuilt binary installer)
            if let Some(data_dir) = dirs_data_local() {
                candidates.push(data_dir.join("sekai/web/dist"));
            }
            candidates
                .into_iter()
    ...
    if let Some(ref dir) = web_dist_dir {
        ...
        tracing::info!(
            "Web dashboard: not available (set gateway.web_dist_dir or SEKAI_WEB_DIST_DIR)"
        );
    }

    let pfx = path_prefix.unwrap_or("");
    println!("🦀 Sekai Gateway listening on http://{display_addr}{pfx}");
    ...
    sekai_runtime::health::mark_component_ok("gateway");
    ...
    let broadcast_layer: Arc<dyn sekai_runtime::observability::Observer> = Arc::new(
        sse::BroadcastObserver::new(event_tx.clone(), event_buffer.clone()),
    );
    sekai_runtime::observability::set_broadcast_hook(broadcast_layer);
    ...
    let state_observer: Arc<dyn sekai_runtime::observability::Observer> = Arc::from(
        sekai_runtime::observability::create_observer(&config.observability),
    );
    ...
    let state = AppState {
        ...
        webauthn: if config.security.webauthn.enabled {
            let secret_store = Arc::new(sekai_runtime::security::SecretStore::new(
                &config.workspace_dir,
                true,
            ));
            let wa_config = sekai_runtime::security::webauthn::WebAuthnConfig {
                enabled: true,
            ...
            Some(Arc::new(api_webauthn::WebAuthnState {
                manager: sekai_runtime::security::webauthn::WebAuthnManager::new(
                    wa_config,
                    secret_store,
                    &config.workspace_dir,
                ),
        ...
        loop {
            tokio::select! {
                ...
                _ = shutdown_signal.changed() => {
                    tracing::info!("🦀 Sekai Gateway shutting down...");
                    break;
                }
            }
        }
...
fn format_paircode_recovery_command(host: &str, port: u16) -> String {
    let mut cmd = format!("sekai gateway get-paircode --new --port {port}");
...
async fn handle_health(State(state): State<AppState>) -> impl IntoResponse {
    let body = serde_json::json!({
        ...
        "runtime": sekai_runtime::health::snapshot_json(),
    });
    Json(body)
}
...
fn prometheus_observer_from_state(
    observer: &dyn sekai_runtime::observability::Observer,
) -> Option<&sekai_runtime::observability::PrometheusObserver> {
    ...
    observer
        .as_any()
        .downcast_ref::<sekai_runtime::observability::PrometheusObserver>()
}
...
/// `crates/sekai-runtime/locales/<locale>/cli.ftl`) so non-English
    ...
    #[cfg(not(test))]
    {
        ...
        let response = Box::pin(
            sekai_runtime::agent::loop_::TOOL_LOOP_COST_TRACKING_CONTEXT.scope(
                ...
                sekai_runtime::agent::process_message(config, message, session_id),
            ),
        )
        .await?;
...
async fn handle_webhook(
    ...
    if state.auto_save && !sekai_memory::should_skip_autosave_content(message) {
    ...
    state.observer.record_event(
        &sekai_runtime::observability::ObserverEvent::AgentStart {
            provider: provider_label.clone(),
            model: model_label.clone(),
        },
    );
    state.observer.record_event(
        &sekai_runtime::observability::ObserverEvent::LlmRequest {
            provider: provider_label.clone(),
            model: model_label.clone(),
            messages_count: 1,
        },
    ...
    match run_gateway_chat_with_tools(&state, message, session_id.as_deref()).await {
        Ok(GatewayChatOutcome {
            ...
            let tokens_used = input_tokens
                ...
                .or(output_tokens);
            state.observer.record_event(
                &sekai_runtime::observability::ObserverEvent::LlmResponse {
                    provider: provider_label.clone(),
                    model: model_label.clone(),
                    duration,
                    success: true,
            ...
            state.observer.record_metric(
                &sekai_runtime::observability::traits::ObserverMetric::RequestLatency(duration),
            );
            state.observer.record_event(
                &sekai_runtime::observability::ObserverEvent::AgentEnd {
        ...
        Err(e) => {
            let duration = started_at.elapsed();
            let sanitized = sekai_providers::sanitize_api_error(&e.to_string());
            ...
            state
                .observer
                .record_event(&sekai_runtime::observability::ObserverEvent::Error {
...
async fn handle_whatsapp_message(
    ...
    for msg in &messages {
        ...
        if let Some((token, response)) = sekai_channels::util::parse_approval_reply(&msg.content)
        {
        ...
        // Auto-save to memory
        if state.auto_save && !sekai_memory::should_skip_autosave_content(&msg.content) {
...
async fn handle_linq_webhook(
    ...
    if let Some(ref signing_secret) = state.linq_signing_secret {
        ...
        if !sekai_channels::linq::verify_linq_signature(
            signing_secret,
            &body_str,
            timestamp,
            signature,
...
async fn handle_nextcloud_talk_webhook(
    ...
    if let Some(ref webhook_secret) = state.nextcloud_talk_webhook_secret {
        ...
        if !sekai_channels::nextcloud_talk::verify_nextcloud_talk_signature(
            webhook_secret,
            random,
            &body_str,
            signature,
...
async fn handle_gmail_push_webhook(
    ...
    let body_str = String::from_utf8_lossy(&body);
    let envelope: sekai_channels::gmail_push::PubSubEnvelope =
...
/// `sekai gateway start` (no daemon supervisor) returns 503 with a
...
mod tests {
    ...
    use sekai_api::channel::ChannelMessage;
    use sekai_memory::{Memory, MemoryCategory, MemoryEntry};
    use sekai_providers::Provider;
    ...
    fn paircode_recovery_command_includes_alternate_port() {
        assert_eq!(
            ...
            "sekai gateway get-paircode --new --port 42617"
        );
    }
    ...
    fn paircode_recovery_command_includes_specific_host_when_needed() {
        assert_eq!(
            ...
            "sekai gateway get-paircode --new --port 42617 --host 192.168.1.20"
        );
    }
    ...
    fn gateway_timeout_falls_back_to_default() {
        ...
        unsafe { std::env::remove_var("SEKAI_GATEWAY_TIMEOUT_SECS") };
    ...
    fn long_running_request_timeout_falls_back_to_default() {
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_GATEWAY_LONG_RUNNING_REQUEST_TIMEOUT_SECS") };
    ...
    async fn metrics_endpoint_returns_hint_when_prometheus_is_disabled() {
        let state = AppState {
            ...
            wati: None,
            gmail_push: None,
            observer: Arc::new(sekai_runtime::observability::NoopObserver),
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let prom = sekai_runtime::observability::PrometheusObserver::new();
        sekai_runtime::observability::Observer::record_event(
            &prom,
            &sekai_runtime::observability::ObserverEvent::HeartbeatTick,
        );

        let observer: Arc<dyn sekai_runtime::observability::Observer> = Arc::new(prom);
        let state = AppState {
            config: Arc::new(Mutex::new(Config::default())),
            provider: Arc::new(MockProvider::default()),
            model: "test-model".into(),
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        let text = String::from_utf8(body.to_vec()).unwrap();
        assert!(text.contains("sekai_heartbeat_ticks_total 1"));
    }
    ...
    async fn persist_pairing_tokens_writes_config_tokens() {
        ...
        assert!(
            sekai_runtime::security::SecretStore::is_encrypted(on_disk),
