...
pub struct CronAddBody {
    ...
    pub delivery: Option<sekai_runtime::cron::DeliveryConfig>,
...
pub async fn handle_api_status(
    ...
    let health = sekai_runtime::health::snapshot();
...
pub async fn handle_api_cron_list(
    ...
    let config = state.config.lock().clone();
    match sekai_runtime::cron::list_jobs(&config) {
...
pub async fn handle_api_cron_add(
    ...
    let config = state.config.lock().clone();
    let schedule = sekai_runtime::cron::Schedule::Cron {
        expr: schedule,
        tz: None,
    };
    if let Err(e) = sekai_runtime::cron::validate_delivery_config(delivery.as_ref()) {
        return (
    ...
    let result = if is_agent {
        ...
        let session_target = session_target
            .as_deref()
            .map(sekai_runtime::cron::SessionTarget::parse)
            .unwrap_or_default();

        let default_delete = matches!(schedule, sekai_runtime::cron::Schedule::At { .. });
        let delete_after_run = delete_after_run.unwrap_or(default_delete);

        sekai_runtime::cron::add_agent_job(
            &config,
            name,
            schedule,
            prompt,
        ...
        sekai_runtime::cron::add_shell_job_with_approval(
...
pub async fn handle_api_cron_runs(
    ...
    if let Err(e) = sekai_runtime::cron::get_job(&config, &id) {
        return (
    ...
    match sekai_runtime::cron::list_runs(&config, &id, limit) {
        Ok(runs) => {
...
pub async fn handle_api_cron_run(
    ...
    let job = match sekai_runtime::cron::get_job(&config, &id) {
        Ok(job) => job,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
    ...
    let (mut success, output) =
        sekai_runtime::cron::scheduler::execute_job_now(&config, &job).await;
    ...
    if job.delivery.mode.eq_ignore_ascii_case("announce")
        ...
        && let Err(e) = sekai_runtime::cron::scheduler::deliver_announcement(
            &config,
            channel,
            target,
    ...
    if let Err(e) = sekai_runtime::cron::record_run(
        &config,
        &job.id,
        started_at,
        finished_at,
    ...
    if let Err(e) =
        sekai_runtime::cron::record_last_run(&config, &job.id, finished_at, success, &output)
    {
        tracing::warn!(
            job_id = %job.id,
            error = %e,
...
pub async fn handle_api_cron_patch(
    ...
    let schedule = match body.schedule {
        Some(expr) if !expr.trim().is_empty() => Some(sekai_runtime::cron::Schedule::Cron {
    ...
    let existing = match sekai_runtime::cron::get_job(&config, &id) {
        Ok(j) => j,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
    ...
    let is_agent = matches!(existing.job_type, sekai_runtime::cron::JobType::Agent);
    ...
    let patch = sekai_runtime::cron::CronJobPatch {
        name: body.name,
        schedule,
        command: patch_command,
        prompt: patch_prompt,
        ..sekai_runtime::cron::CronJobPatch::default()
    };

    match sekai_runtime::cron::update_shell_job_with_approval(&config, &id, patch, false) {
        Ok(job) => Json(serde_json::json!({"status": "ok", "job": job})).into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn handle_api_cron_delete(
    ...
    let config = state.config.lock().clone();
    match sekai_runtime::cron::remove_job(&config, &id) {
...
pub async fn handle_api_integrations(
    ...
    let config = state.config.lock().clone();
    let entries = sekai_runtime::integrations::registry::all_integrations(&config);
...
pub async fn handle_api_integrations_settings(
    ...
    for entry in &entries {
        let enabled = matches!(
            entry.status,
            sekai_runtime::integrations::IntegrationStatus::Active
        );
        settings.insert(
            entry.name.clone(),
            serde_json::json!({
...
pub async fn handle_api_doctor(
    ...
    let config = state.config.lock().clone();
    let results = sekai_runtime::doctor::diagnose(&config);

    let ok_count = results
        .iter()
        .filter(|r| r.severity == sekai_runtime::doctor::Severity::Ok)
        .count();
    let warn_count = results
        .iter()
        .filter(|r| r.severity == sekai_runtime::doctor::Severity::Warn)
        .count();
    let error_count = results
        .iter()
        .filter(|r| r.severity == sekai_runtime::doctor::Severity::Error)
        .count();
...
pub async fn handle_api_memory_list(
    ...
    if params.query.is_some() || params.since.is_some() || params.until.is_some() {
        ...
        let category = params.category.as_deref().map(|cat| match cat {
            "core" => sekai_memory::MemoryCategory::Core,
            "daily" => sekai_memory::MemoryCategory::Daily,
            "conversation" => sekai_memory::MemoryCategory::Conversation,
            other => sekai_memory::MemoryCategory::Custom(other.to_string()),
        });
...
pub async fn handle_api_memory_store(
    ...
    let category = body
        ...
        .unwrap_or(sekai_memory::MemoryCategory::Core);
...
pub async fn handle_api_cli_tools(
    ...
    let tools = sekai_tools::cli_discovery::discover_cli_tools(&[], &[]);
...
pub async fn handle_api_health(
    ...
    let snapshot = sekai_runtime::health::snapshot();
...
pub async fn handle_claude_code_hook(
    State(state): State<AppState>,
    Json(payload): Json<sekai_tools::claude_code_runner::ClaudeCodeHookEvent>,
) -> impl IntoResponse {
...
mod tests {
    ...
    use sekai_runtime::security::pairing::PairingGuard;
    ...
    fn test_state(config: sekai_config::schema::Config) -> AppState {
        AppState {
            ...
            web_dist_dir: None,
            canvas_store: sekai_runtime::tools::CanvasStore::new(),
    ...
    async fn cron_api_shell_roundtrip_includes_delivery() {
        ...
        let config = sekai_config::schema::Config {
            workspace_dir: tmp.path().join("workspace"),
            config_path: tmp.path().join("config.toml"),
            ..sekai_config::schema::Config::default()
        };
    ...
    async fn cron_api_accepts_agent_jobs() {
        ...
        let config = state.config.lock().clone();
        let jobs = sekai_runtime::cron::list_jobs(&config).unwrap();
        assert_eq!(jobs.len(), 1);
        assert_eq!(jobs[0].job_type, sekai_runtime::cron::JobType::Agent);
    ...
    async fn cron_api_rejects_announce_delivery_without_target() {
        ...
        let config = state.config.lock().clone();
        assert!(
            sekai_runtime::cron::list_jobs(&config)
                .unwrap()
                .is_empty()
        );
    }
    ...
    async fn cron_api_run_executes_shell_job_and_records_run() {
        ...
        let job = sekai_runtime::cron::add_shell_job_with_approval(
            &state.config.lock().clone(),
            None,
            sekai_runtime::cron::Schedule::Cron {
                expr: "*/5 * * * *".to_string(),
                tz: None,
            },
        ...
        let runs = sekai_runtime::cron::list_runs(&state.config.lock().clone(), &job.id, 10)
            .expect("runs listed");
