...
impl sekai_runtime::observability::Observer for BroadcastObserver {
    fn record_event(&self, event: &sekai_runtime::observability::ObserverEvent) {
        ...
        let json = match event {
            ...
            sekai_runtime::observability::ObserverEvent::ToolCall {
                tool,
                duration,
                success,
            } => serde_json::json!({
            ...
            sekai_runtime::observability::ObserverEvent::ToolCallStart { tool, .. } => {
                serde_json::json!({
            ...
            sekai_runtime::observability::ObserverEvent::Error { component, message } => {
                serde_json::json!({
                    "type": "error",
                    "component": component,
                    "message": message,
            ...
            sekai_runtime::observability::ObserverEvent::AgentStart { provider, model } => {
                serde_json::json!({
                    "type": "agent_start",
                    "provider": provider,
                    "model": model,
    ...
    fn record_metric(&self, _metric: &sekai_runtime::observability::traits::ObserverMetric) {
...
mod tests {
    use super::*;
    use sekai_runtime::observability::{Observer, ObserverEvent};
    ...
    fn factory_observer_events_reach_broadcast_hook() {
        ...
        sekai_runtime::observability::clear_broadcast_hook();

        let (tx, mut rx) = tokio::sync::broadcast::channel(16);
        let buffer = Arc::new(EventBuffer::new(16));
        let bo: Arc<dyn Observer> = Arc::new(BroadcastObserver::new(tx, buffer.clone()));
        sekai_runtime::observability::set_broadcast_hook(bo);

        // Same factory call site as `process_message` in the agent loop.
        let cfg = sekai_config::schema::ObservabilityConfig {
            backend: "noop".into(),
            ..Default::default()
        };
        let observer = sekai_runtime::observability::create_observer(&cfg);
