...
//!
//! Sekai's agent loop calls [`sekai_hardware::boot`] on **every** request,
//! which re-reads `~/.sekai/hardware/` from disk.  Writing to those files
...
/// Return `~/.sekai/hardware/` or an error string.
fn hardware_dir() -> Result<PathBuf, String> {
    directories::BaseDirs::new()
        .map(|b| b.home_dir().join(".sekai").join("hardware"))
...
/// `POST /api/hardware/pin` — register a single GPIO pin assignment.
///
/// Appends one line to `~/.sekai/hardware/devices/<device>.md`:
/// ```text
...
///
/// Since [`sekai_hardware::boot`] re-reads from disk on every agent invocation,
...
pub async fn handle_hardware_reload(
    ...
    let context = sekai_hardware::load_hardware_context_prompt(&[]);
