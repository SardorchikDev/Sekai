...
/// committed snapshot at `crates/sekai-gateway/openapi.json`. CI
...
pub fn build_spec() -> serde_json::Value {
    ...
    use sekai_config::api_error::ConfigApiError;
    ...
    let components = serde_json::json!({
        "schemas": {
            ...
            "Config":           schema_value::<sekai_config::schema::Config>(),
        },
        "securitySchemes": {
            "bearerAuth": {
                "type": "http",
    ...
    let paths = serde_json::json!({
        ...
        "/api/config/migrate": {
            "post": {
                ...
                "description": "Mirrors `sekai config migrate`. Backs up the previous file as `config.toml.bak` before writing.",
                "responses": {
                    "200": {
    ...
    let mut spec = serde_json::json!({
        "openapi": "3.1.0",
        "info": {
            "title": "Sekai Gateway — Config CRUD",
            "version": env!("CARGO_PKG_VERSION"),
            "description": "Per-property CRUD endpoints over the same `Config` mutation core that `sekai config get/set/list/init/migrate` uses on the CLI. See https://github.com/sekai-labs/sekai/issues/6175 for the full surface and acceptance checklist.",
        },
        "security": [{"bearerAuth": []}],
        "paths": paths,
