...
//! tool args through `sekai_runtime::approval::summarize_args` before
...
const WS_PROTOCOL: &str = "sekai.v1";
...
async fn handle_socket(
    ...
    let mut memory_session_id = sekai_api::session_keys::sanitize_session_key(&session_id);
                ...
                if let Ok(cp) = serde_json::from_str::<ConnectParams>(&text) {
                    if cp.msg_type == "connect" {
                        ...
                        if let Some(sid) = &cp.session_id {
                            memory_session_id =
                                sekai_api::session_keys::sanitize_session_key(sid);
                            debug!(
                                session_id = sid,
    ...
    let mut agent =
        match sekai_runtime::agent::Agent::from_config_with_session_cwd_and_mcp_backchannel(
            &config,
            Some(&session_cwd),
            true,
        )
    ...
    let (approval_event_tx, mut approval_event_rx) =
        tokio::sync::mpsc::channel::<sekai_api::agent::TurnEvent>(8);
        ...
        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(text) {
            if parsed["type"].as_str() == Some("message") {
                ...
                if !content.is_empty() {
                    ...
                    if let Some(ref backend) = state.session_backend {
                        let user_msg = sekai_providers::ChatMessage::user(&content);
        ...
        tokio::select! {
            ...
            approval_event = approval_event_rx.recv() => {
                ...
                let frame = match event {
                    sekai_api::agent::TurnEvent::ApprovalRequest {
                        request_id,
                        tool_name,
                        arguments_summary,
                        timeout_secs,
...
async fn process_chat_message(
    state: &AppState,
    agent: &mut sekai_runtime::agent::Agent,
    sender: &mut futures_util::stream::SplitSink<WebSocket, Message>,
    receiver: &mut futures_util::stream::SplitStream<WebSocket>,
    approval_event_rx: &mut tokio::sync::mpsc::Receiver<sekai_api::agent::TurnEvent>,
    pending_approvals: &PendingApprovals,
    content: &str,
    session_key: &str,
) {
    use futures_util::StreamExt as _;
    use sekai_runtime::agent::TurnEvent;
    ...
    let turn_fut = async {
        sekai_runtime::agent::loop_::scope_session_key(
                    ...
                    let ws_msg = match event {
                        ...
                        TurnEvent::Chunk { ref delta } => {
                            ...
                            if last_partial_save.elapsed() >= partial_save_interval {
                                if let Some(ref backend) = state.session_backend {
                                    let partial = sekai_providers::ChatMessage::assistant(
                                        &accumulated_text,
                                    );
                                    if partial_saved {
    ...
    let was_cancelled = match &result {
        Err(e) => sekai_runtime::agent::loop_::is_tool_loop_cancelled(e),
        Ok(_) => false,
    };
    ...
    if was_cancelled {
        ...
        if let Some(ref backend) = state.session_backend {
            let assistant_msg = sekai_providers::ChatMessage::assistant(&truncated);
            if partial_saved {
        ...
        sekai_runtime::observability::runtime_trace::record_event(
            "gateway_ws_turn",
            Some("ws"),
            Some(&provider_label),
            Some(&state.model),
    ...
    match result {
        Ok(response) => {
            ...
            if let Some(ref backend) = state.session_backend {
                let assistant_msg = sekai_providers::ChatMessage::assistant(&response);
                if partial_saved {
                    let _ = backend.update_last(session_key, &assistant_msg);
                } else {
                    let _ = backend.append(session_key, &assistant_msg);
            ...
            // Append a runtime-trace.jsonl record so a `sekai doctor`
...
fn record_turn_cost(
    ...
    let usage = sekai_runtime::cost::types::TokenUsage::new(
        model,
        input,
        output,
...
mod tests {
    ...
    fn extract_ws_token_from_subprotocol() {
        ...
        headers.insert(
            ...
            "sekai.v1, bearer.zc_sub456".parse().unwrap(),
        );
    ...
    fn extract_ws_token_subprotocol_with_multiple_entries() {
        ...
        headers.insert(
            "sec-websocket-protocol",
            "sekai.v1, bearer.zc_tok, other".parse().unwrap(),
        );
    ...
    fn needs_onboarding_ws_error_points_to_onboard() {
        let config = sekai_config::schema::Config::default();
    ...
    fn needs_onboarding_ws_error_uses_current_configured_model() {
        let mut config = sekai_config::schema::Config {
            providers: sekai_config::providers::ProvidersConfig {
