...
//! in `sekai-providers` and is the canonical list; `list_models()` per
...
pub async fn handle_catalog(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let providers: Vec<CatalogProvider> = sekai_providers::list_providers()
        .into_iter()
...
/// (`sekai_providers::create_provider(...).list_models()`), which goes
...
pub async fn handle_catalog_models(
    ...
    let handle = match sekai_providers::create_provider(&q.provider, None) {
        Ok(h) => h,
        Err(e) => {
...
pub struct SectionInfo {
    ...
    /// sekai-runtime so CLI / web stay aligned.
    pub key: String,
...
pub async fn handle_sections(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let map_keyed_roots: std::collections::HashSet<&'static str> =
        sekai_config::schema::Config::map_key_sections()
            .iter()
...
fn section_help(key: &str) -> &'static str {
    match key {
        "workspace" => {
            "Where Sekai stores its config and runtime data. Defaults work for most setups."
        }
        "providers" => {
            "Paste an API key (e.g. `sk-ant-...` for Anthropic, `sk-...` for OpenAI) when prompted. \
                        For OAuth-based providers run: sekai auth login --provider <name>"
        }
        "channels" => {
            "Pick which chat platforms Sekai should listen on. You can configure multiple."
        }
...
/// * `providers` → `sekai_providers::list_providers()` (CLI's catalog).
/// * `memory` → `sekai_memory::selectable_memory_backends()`.
...
fn providers_picker(cfg: &sekai_config::schema::Config) -> Vec<PickerItem> {
    sekai_providers::list_providers()
        .into_iter()
        .map(|p| {
...
fn memory_picker(cfg: &sekai_config::schema::Config) -> Vec<PickerItem> {
    let current = cfg.memory.backend.clone();
    sekai_memory::selectable_memory_backends()
        .iter()
        .map(|b| PickerItem {
            key: b.key.to_string(),
...
fn schema_walk_picker(cfg: &sekai_config::schema::Config, section: &str) -> Vec<PickerItem> {
...
fn schema_walk_picker_with_none(
    cfg: &sekai_config::schema::Config,
    section: &str,
...
pub async fn handle_section_select(
    ...
    let (fields_prefix, created) = match section.as_str() {
        "providers" => {
            ...
            if let Err(e) =
                sekai_runtime::onboard::field_visibility::apply_provider_trait_defaults(
                    &mut working,
                    &key,
                    &prefix,
                )
            ...
            // they thought they'd configured. Mirrors `sekai onboard`'s
            // post-pick semantics.
...
mod tests {
    ...
    fn empty_cfg() -> sekai_config::schema::Config {
    ...
    fn providers_picker_sources_from_list_providers() {
        // Single source of truth: sekai_providers::list_providers().
    ...
    fn memory_picker_sources_from_selectable_backends() {
        ...
        // Mirrors sekai_memory::selectable_memory_backends() exactly.
