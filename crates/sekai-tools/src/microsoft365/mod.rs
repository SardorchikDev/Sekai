...
impl Microsoft365Tool {
    pub fn new(
        config: types::Microsoft365ResolvedConfig,
        security: Arc<SecurityPolicy>,
        sekai_dir: &std::path::Path,
    ) -> anyhow::Result<Self> {
        let http_client = sekai_config::schema::build_runtime_proxy_client_with_timeouts(
            "tool.microsoft365",
            60,
            10,
        );
        let token_cache = Arc::new(auth::TokenCache::new(config.clone(), sekai_dir)?);
        Ok(Self {
            config,
            security,
            token_cache,
