...
impl Tool for ImageInfoTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // registration time (see sekai-runtime::tools::mod). Rate limiting
...
mod tests {
    ...
    /// `sekai-runtime::tools::mod`.  Use this in tests that exercise
    ...
    async fn execute_real_file() {
        ...
        let dir = std::env::temp_dir().join("sekai_image_info_test");
    ...
    async fn wrapped_blocks_external_absolute_path() {
        ...
        let workspace = std::env::temp_dir().join("sekai_image_info_wrap");
    ...
    async fn wrapped_blocks_path_traversal() {
        ...
        let workspace = std::env::temp_dir().join("sekai_image_info_trav");
        let _ = std::fs::create_dir_all(&workspace);
        let tool = wrapped_tool(workspace);
    ...
    async fn execute_with_base64() {
        let dir = std::env::temp_dir().join("sekai_image_info_b64");
        let _ = tokio::fs::create_dir_all(&dir).await;
