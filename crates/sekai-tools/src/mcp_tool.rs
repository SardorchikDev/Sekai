//! Wraps a discovered MCP tool as a sekai [`Tool`] so it is dispatched
...
/// A sekai [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Sekai's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // Sekai's security model injects `approved: bool` into built-in tool args.
