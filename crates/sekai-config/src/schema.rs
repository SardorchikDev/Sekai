...
/// Top-level Sekai configuration, loaded from `config.toml`.
///
/// Resolution order: `SEKAI_WORKSPACE` env → `active_workspace.toml` marker → `~/.sekai/config.toml`.
...
pub struct Config {
    ...
    /// Meta-state for `sekai onboard` (which sections the user has
    ...
    ///
    /// If omitted or empty, the locale is auto-detected from `SEKAI_LOCALE`,
...
/// Opaque state the `sekai onboard` flow writes so it can tell, on a
...
pub struct WorkspaceConfig {
    /// Turn on multi-workspace profiles — each named engagement gets its own memory, secrets, and audit directories so work for one client/project never bleeds into another. Leave off for single-workspace mode where everything lives under `~/.sekai/workspace`.
    #[serde(default)]
    pub enabled: bool,
    /// Which workspace profile is currently active — picks the `<workspaces_dir>/<name>/` directory Sekai reads from and writes to. Required when multi-workspace is enabled; ignored otherwise.
    #[serde(default)]
...
pub struct HardwareConfig {
    ...
    #[serde(default)]
    pub enabled: bool,
    /// How Sekai reaches the hardware: `native` = Linux SBC with direct GPIO access (Raspberry Pi, Orange Pi); `serial` = USB-tethered microcontroller speaking over a TTY; `probe` = SWD/JTAG debug probe driving a target chip via probe-rs; `none` = disabled.
    #[serde(default)]
...
pub struct SkillsConfig {
    ...
    /// Default: `https://github.com/sekai-labs/sekai-skills`
    #[serde(default)]
...
/// execution events back to Sekai's gateway, updating a Slack message
...
pub enum ProxyScope {
    ...
    Environment,
    /// Apply proxy to all Sekai-managed HTTP traffic (default).
    #[default]
    Zeroclaw,
...
fn parse_proxy_scope(raw: &str) -> Option<ProxyScope> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "sekai" | "internal" | "core" => Some(ProxyScope::Zeroclaw),
...
pub struct QdrantConfig {
    ...
    /// Falls back to `QDRANT_COLLECTION` env var, or default "sekai_memories".
...
pub struct MemoryConfig {
    ...
    pub backend: String,
    /// Auto-save what *you* tell Sekai into memory as conversation history — the agent's own replies are not saved. Turn off if you want memory to only hold things you explicitly record via the memory tool.
    pub auto_save: bool,
...
pub struct ObservabilityConfig {
    ...
    /// Service name reported to the OTel collector. Defaults to "sekai".
    #[serde(default)]
...
pub struct MatrixConfig {
    ...
    /// When set, Sekai recovers room keys and cross-signing secrets on startup.
    #[secret]
    #[cfg_attr(feature = "schema-export", schemars(extend("x-secret" = true)))]
    #[serde(default)]
...
pub struct WhatsAppConfig {
    ...
    /// Can also be set via `SEKAI_WHATSAPP_APP_SECRET` environment variable
    /// Only used in Cloud API mode
    #[serde(default)]
    #[secret]
    #[cfg_attr(feature = "schema-export", schemars(extend("x-secret" = true)))]
...
pub struct NextcloudTalkConfig {
    ...
    ///
    /// Can also be set via `SEKAI_NEXTCLOUD_TALK_WEBHOOK_SECRET`.
    #[serde(default)]
    #[secret]
    #[cfg_attr(feature = "schema-export", schemars(extend("x-secret" = true)))]
    ...
    #[serde(default)]
    pub proxy_url: Option<String>,
    /// Display name of the bot in Nextcloud Talk (e.g. "sekai").
...
/// How Sekai receives events from Feishu / Lark.
///
...
pub enum LineDmPolicy {
    ...
    /// Sekai prints the bind code on startup; send it once to unlock access.
    #[default]
    Pairing,
...
pub struct WebAuthnConfig {
    ...
    /// Relying Party display name. Default: "Sekai".
...
///
/// When `enabled` is true, Sekai validates incoming requests against a Nevis
...
pub struct NevisConfig {
    ...
    /// Nevis role to Sekai permission mappings.
    #[serde(default)]
...
/// Maps a Nevis role to Sekai tool permissions and workspace access.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
#[serde(deny_unknown_fields)]
pub struct NevisRoleMappingConfig {
    ...
    #[serde(default)]
    pub sekai_permissions: Vec<String>,
...
pub struct AuditConfig {
    ...
    /// Path to audit log file (relative to sekai dir)
...
#[cfg(feature = "voice-wake")]
#[derive(Debug, Clone, Serialize, Deserialize, sekai_macros::Configurable)]
#[cfg_attr(feature = "schema-export", derive(schemars::JsonSchema))]
#[prefix = "voice-wake"]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let sekai_dir = home.join(".sekai");

        Self {
            workspace_dir: sekai_dir.join("workspace"),
            config_path: sekai_dir.join("config.toml"),
...
fn default_config_dir() -> Result<PathBuf> {
    if let Ok(custom) = std::env::var("SEKAI_CONFIG_DIR") {
    ...
    if let Ok(home) = std::env::var("HOME")
        && !home.is_empty()
    {
        return Ok(PathBuf::from(home).join(".sekai"));
    }
...
/// (`/home/<user>/.sekai/<relative>` on Linux,
/// `C:\Users\<user>\.sekai\<relative>` on Windows, etc.) instead of a
/// literal `~/...` token that doesn't expand on Windows. Falls back to
/// `~/.sekai/<relative>` if the platform dir can't be resolved (rare —
...
/// Support/sekai/` on macOS, `%APPDATA%\sekai\` on Windows) is the
...
/// `SEKAI_CONFIG_DIR` > `SEKAI_WORKSPACE` > active workspace marker > defaults.
pub async fn resolve_runtime_dirs_for_onboarding() -> Result<(PathBuf, PathBuf)> {
    let (default_sekai_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
    let (config_dir, workspace_dir, _) =
        resolve_runtime_config_dirs(&default_sekai_dir, &default_workspace_dir).await?;
...
impl ConfigResolutionSource {
    const fn as_str(self) -> &'static str {
        match self {
            Self::EnvConfigDir => "SEKAI_CONFIG_DIR",
            Self::EnvWorkspace => "SEKAI_WORKSPACE",
...
async fn resolve_runtime_config_dirs(
    default_sekai_dir: &Path,
    default_workspace_dir: &Path,
) -> Result<(PathBuf, PathBuf, ConfigResolutionSource)> {
    if let Ok(custom_config_dir) = std::env::var("SEKAI_CONFIG_DIR") {
        let custom_config_dir = custom_config_dir.trim();
        if !custom_config_dir.is_empty() {
            let sekai_dir = expand_tilde_path(custom_config_dir);
            return Ok((
                sekai_dir.clone(),
                sekai_dir.join("workspace"),
                ConfigResolutionSource::EnvConfigDir,
            ));
        }
    }

    if let Ok(custom_workspace) = std::env::var("SEKAI_WORKSPACE")
        && !custom_workspace.is_empty()
    {
        let expanded = expand_tilde_path(&custom_workspace);
        let (sekai_dir, workspace_dir) = resolve_config_dir_for_workspace(&expanded);
        return Ok((
            sekai_dir,
            workspace_dir,
    ...
    if let Some((sekai_dir, workspace_dir)) =
        load_persisted_workspace_dirs(default_sekai_dir).await?
    {
        return Ok((
            sekai_dir,
            workspace_dir,
...
fn config_dir_creation_error(path: &Path) -> String {
    format!(
        "Failed to create config directory: {}. If running as an OpenRC service, \
         ensure this path is writable by user 'sekai'.",
        path.display()
    )
}
...
fn has_ollama_cloud_credential(config_api_key: Option<&str>) -> bool {
    ...
    ["OLLAMA_API_KEY", "SEKAI_API_KEY", "API_KEY"]
        .iter()
        .any(|name| {
            std::env::var(name)
                .ok()
...
/// Parse the `SEKAI_EXTRA_HEADERS` environment variable value.
///
...
pub fn parse_extra_headers_env(raw: &str) -> Vec<(String, String)> {
    ...
    for entry in raw.split(',') {
        ...
        if let Some((key, value)) = entry.split_once(':') {
            ...
            if key.is_empty() {
                tracing::warn!("Ignoring extra header with empty name in SEKAI_EXTRA_HEADERS");
                continue;
            }
...
///
/// When the workspace is created outside of `sekai onboard` (e.g., non-tty
    ...
    let defaults: &[(&str, &str)] = &[
        (
            ...
            "# IDENTITY.md — Who Am I?\n\n\
             I am Sekai, an autonomous AI agent.\n\n\
             ## Traits\n\
        ...
        (
            "SOUL.md",
            "# SOUL.md — Who You Are\n\n\
             You are Sekai, an autonomous AI agent.\n\n\
             ## Core Principles\n\
...
impl Config {
    ...
    pub async fn load_or_init() -> Result<Self> {
        ...
        let (sekai_dir, workspace_dir, resolution_source) =
            resolve_runtime_config_dirs(&default_sekai_dir, &default_workspace_dir).await?;

        let config_path = sekai_dir.join("config.toml");

        fs::create_dir_all(&sekai_dir)
            .await
            .with_context(|| config_dir_creation_error(&sekai_dir))?;
        ...
        if config_path.exists() {
            ...
            let store = crate::secrets::SecretStore::new(&sekai_dir, config.secrets.encrypt);
    ...
    ///    what was on disk, so `sekai config get providers.fallback` returned the
            ...
            if !has_name && !has_base_url && !has_api_key && !has_model {
                tracing::warn!(
                    ...
                    "providers.models.{profile_name} is empty (no name / base_url / api_key / model). \
                     Skipping at runtime; finish onboarding via the dashboard or `sekai onboard` \
    ...
    pub fn apply_env_overrides(&mut self) {
        // API Key: SEKAI_API_KEY or API_KEY (generic)
        if let Ok(key) = std::env::var("SEKAI_API_KEY").or_else(|_| std::env::var("API_KEY"))
            && !key.is_empty()
        {
        ...
        // 1) SEKAI_PROVIDER always wins when set.
        // 2) SEKAI_MODEL_PROVIDER/MODEL_PROVIDER (Codex app-server style).
        // 3) Legacy PROVIDER is honored only when config still uses default provider.
        if let Ok(provider) = std::env::var("SEKAI_PROVIDER")
            && !provider.is_empty()
        {
            self.providers.fallback = Some(provider);
        } else if let Ok(provider) =
            std::env::var("SEKAI_MODEL_PROVIDER").or_else(|_| std::env::var("MODEL_PROVIDER"))
            && !provider.is_empty()
        {
            self.providers.fallback = Some(provider);
        ...
        // Model: SEKAI_MODEL or MODEL
        if let Ok(model) = std::env::var("SEKAI_MODEL").or_else(|_| std::env::var("MODEL"))
            && !model.is_empty()
        {
            self.ensure_fallback_provider().model = Some(model);
        }

        // Provider HTTP timeout: SEKAI_PROVIDER_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("SEKAI_PROVIDER_TIMEOUT_SECS")
        ...
        // Extra provider headers: SEKAI_EXTRA_HEADERS
        // Format: "Key:Value,Key2:Value2"
        // Env var headers override config file headers with the same name.
        if let Ok(raw) = std::env::var("SEKAI_EXTRA_HEADERS") {
        ...
        // Workspace directory: SEKAI_WORKSPACE
        if let Ok(workspace) = std::env::var("SEKAI_WORKSPACE")
            && !workspace.is_empty()
        {
        ...
        // Open-skills opt-in flag: SEKAI_OPEN_SKILLS_ENABLED
        if let Ok(flag) = std::env::var("SEKAI_OPEN_SKILLS_ENABLED")
            && !flag.trim().is_empty()
        {
            match flag.trim().to_ascii_lowercase().as_str() {
                ...
                _ => tracing::warn!(
                    "Ignoring invalid SEKAI_OPEN_SKILLS_ENABLED (valid: 1|0|true|false|yes|no|on|off)"
                ),
            }
        }

        // Open-skills directory override: SEKAI_OPEN_SKILLS_DIR
        if let Ok(path) = std::env::var("SEKAI_OPEN_SKILLS_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        // Skills script-file audit override: SEKAI_SKILLS_ALLOW_SCRIPTS
        if let Ok(flag) = std::env::var("SEKAI_SKILLS_ALLOW_SCRIPTS")
            && !flag.trim().is_empty()
        {
            match flag.trim().to_ascii_lowercase().as_str() {
                ...
                _ => tracing::warn!(
                    "Ignoring invalid SEKAI_SKILLS_ALLOW_SCRIPTS (valid: 1|0|true|false|yes|no|on|off)"
                ),
            }
        }

        // Skills prompt mode override: SEKAI_SKILLS_PROMPT_MODE
        if let Ok(mode) = std::env::var("SEKAI_SKILLS_PROMPT_MODE")
            && !mode.trim().is_empty()
        {
            if let Some(parsed) = parse_skills_prompt_injection_mode(&mode) {
                ...
                tracing::warn!(
                    "Ignoring invalid SEKAI_SKILLS_PROMPT_MODE (valid: full|compact)"
                );
            }
        }

        // Gateway port: SEKAI_GATEWAY_PORT or PORT
        if let Ok(port_str) =
            std::env::var("SEKAI_GATEWAY_PORT").or_else(|_| std::env::var("PORT"))
            && let Ok(port) = port_str.parse::<u16>()
        {
            self.gateway.port = port;
        }

        // Gateway host: SEKAI_GATEWAY_HOST or HOST
        if let Ok(host) = std::env::var("SEKAI_GATEWAY_HOST").or_else(|_| std::env::var("HOST"))
            && !host.is_empty()
        {
            self.gateway.host = host;
        }

        // Allow public bind: SEKAI_ALLOW_PUBLIC_BIND
        if let Ok(val) = std::env::var("SEKAI_ALLOW_PUBLIC_BIND") {
            self.gateway.allow_public_bind = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Require pairing: SEKAI_REQUIRE_PAIRING
        if let Ok(val) = std::env::var("SEKAI_REQUIRE_PAIRING") {
            self.gateway.require_pairing = val == "1" || val.eq_ignore_ascii_case("true");
        }

        // Web dist dir: SEKAI_WEB_DIST_DIR
        if let Ok(path) = std::env::var("SEKAI_WEB_DIST_DIR") {
            let trimmed = path.trim();
            if !trimmed.is_empty() {
        ...
        // Temperature: SEKAI_TEMPERATURE
        if let Ok(temp_str) = std::env::var("SEKAI_TEMPERATURE") {
            match temp_str.parse::<f64>() {
                ...
                Ok(temp) => {
                    tracing::warn!(
                        "Ignoring SEKAI_TEMPERATURE={temp}: \
                ...
                Err(_) => {
                    tracing::warn!(
                        "Ignoring SEKAI_TEMPERATURE={temp_str:?}: not a valid number"
                    );
                }
            }
        }

        // Reasoning override: SEKAI_REASONING_ENABLED or REASONING_ENABLED
        if let Ok(flag) = std::env::var("SEKAI_REASONING_ENABLED")
        ...
        if let Ok(raw) = std::env::var("SEKAI_REASONING_EFFORT")
            .or_else(|_| std::env::var("REASONING_EFFORT"))
            .or_else(|_| std::env::var("SEKAI_CODEX_REASONING_EFFORT"))
        {
        ...
        // Web search enabled: SEKAI_WEB_SEARCH_ENABLED or WEB_SEARCH_ENABLED
        if let Ok(enabled) = std::env::var("SEKAI_WEB_SEARCH_ENABLED")
            .or_else(|_| std::env::var("WEB_SEARCH_ENABLED"))
        {
            self.web_search.enabled = enabled == "1" || enabled.eq_ignore_ascii_case("true");
        }

        // Web search provider: SEKAI_WEB_SEARCH_PROVIDER or WEB_SEARCH_PROVIDER
        if let Ok(provider) = std::env::var("SEKAI_WEB_SEARCH_PROVIDER")
        ...
        // Brave API key: SEKAI_BRAVE_API_KEY or BRAVE_API_KEY
        if let Ok(api_key) =
            std::env::var("SEKAI_BRAVE_API_KEY").or_else(|_| std::env::var("BRAVE_API_KEY"))
        {
        ...
        // Tavily API key: SEKAI_TAVILY_API_KEY or TAVILY_API_KEY
        if let Ok(api_key) =
            std::env::var("SEKAI_TAVILY_API_KEY").or_else(|_| std::env::var("TAVILY_API_KEY"))
        {
            let api_key = api_key.trim();
            if !api_key.is_empty() {
        ...
        // SearXNG instance URL: SEKAI_SEARXNG_INSTANCE_URL or SEARXNG_INSTANCE_URL
        if let Ok(instance_url) = std::env::var("SEKAI_SEARXNG_INSTANCE_URL")
        ...
        // Web search max results: SEKAI_WEB_SEARCH_MAX_RESULTS or WEB_SEARCH_MAX_RESULTS
        if let Ok(max_results) = std::env::var("SEKAI_WEB_SEARCH_MAX_RESULTS")
        ...
        // Web search timeout: SEKAI_WEB_SEARCH_TIMEOUT_SECS or WEB_SEARCH_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("SEKAI_WEB_SEARCH_TIMEOUT_SECS")
        ...
        // Storage provider key (optional backend override): SEKAI_STORAGE_PROVIDER
        if let Ok(provider) = std::env::var("SEKAI_STORAGE_PROVIDER") {
            let provider = provider.trim();
            if !provider.is_empty() {
        ...
        // Storage connection URL (for remote backends): SEKAI_STORAGE_DB_URL
        if let Ok(db_url) = std::env::var("SEKAI_STORAGE_DB_URL") {
        ...
        // Storage connect timeout: SEKAI_STORAGE_CONNECT_TIMEOUT_SECS
        if let Ok(timeout_secs) = std::env::var("SEKAI_STORAGE_CONNECT_TIMEOUT_SECS")
            && let Ok(timeout_secs) = timeout_secs.parse::<u64>()
            && timeout_secs > 0
        {
            self.storage.provider.config.connect_timeout_secs = Some(timeout_secs);
        }
        // Proxy enabled flag: SEKAI_PROXY_ENABLED
        let explicit_proxy_enabled = std::env::var("SEKAI_PROXY_ENABLED")
            .ok()
            .as_deref()
        ...
        // Proxy URLs: SEKAI_* wins, then generic *PROXY vars.
        let mut proxy_url_overridden = false;
        if let Ok(proxy_url) =
            std::env::var("SEKAI_HTTP_PROXY").or_else(|_| std::env::var("HTTP_PROXY"))
        {
            self.proxy.http_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("SEKAI_HTTPS_PROXY").or_else(|_| std::env::var("HTTPS_PROXY"))
        {
            self.proxy.https_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(proxy_url) =
            std::env::var("SEKAI_ALL_PROXY").or_else(|_| std::env::var("ALL_PROXY"))
        {
            self.proxy.all_proxy = normalize_proxy_url_option(Some(&proxy_url));
            proxy_url_overridden = true;
        }
        if let Ok(no_proxy) =
            std::env::var("SEKAI_NO_PROXY").or_else(|_| std::env::var("NO_PROXY"))
        {
        ...
        if let Ok(scope_raw) = std::env::var("SEKAI_PROXY_SCOPE") {
            if let Some(scope) = parse_proxy_scope(&scope_raw) {
                self.proxy.scope = scope;
            } else {
                tracing::warn!(
                    scope = %scope_raw,
                    "Ignoring invalid SEKAI_PROXY_SCOPE (valid: environment|sekai|services)"
                );
            }
        }

        if let Ok(services_raw) = std::env::var("SEKAI_PROXY_SERVICES") {
    ...
    async fn resolve_config_path_for_save(&self) -> Result<PathBuf> {
        ...
        let (default_sekai_dir, default_workspace_dir) = default_config_and_workspace_dirs()?;
        let (sekai_dir, _workspace_dir, source) =
            resolve_runtime_config_dirs(&default_sekai_dir, &default_workspace_dir).await?;
        let file_name = self
            .config_path
            .file_name()
        ...
        let resolved = sekai_dir.join(file_name);
        tracing::warn!(
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let sekai_dir = config_path
            .parent()
            .context("Config path must have a parent directory")?;
        let store = crate::secrets::SecretStore::new(sekai_dir, self.secrets.encrypt);
...
impl HasPropKind for serde_json::Value {
    ...
    // objects still use `sekai config set --json` or hand-edit the
    // `config.toml`.
...
mod tests {
    ...
    async fn expand_tilde_path_expands_tilde_when_home_set() {
        ...
        let path = expand_tilde_path("~/.sekai");
    ...
    async fn config_dir_creation_error_mentions_openrc_and_path() {
        let msg = config_dir_creation_error(Path::new("/etc/sekai"));
        assert!(msg.contains("/etc/sekai"));
        assert!(msg.contains("OpenRC"));
        assert!(msg.contains("sekai"));
    }
...
"#;
    ...
    async fn parse_extra_headers_env_basic() {
        let headers = parse_extra_headers_env("User-Agent:MyApp/1.0,X-Title:sekai");
        assert_eq!(headers.len(), 2);
        assert_eq!(
            headers[0],
            ("User-Agent".to_string(), "MyApp/1.0".to_string())
        );
        assert_eq!(headers[1], ("X-Title".to_string(), "sekai".to_string()));
    }

    #[test]
    async fn parse_extra_headers_env_with_url_value() {
        let headers =
            parse_extra_headers_env("HTTP-Referer:https://github.com/sekai-labs/sekai");
        assert_eq!(headers.len(), 1);
        // Only splits on first colon, preserving URL colons in value
        assert_eq!(headers[0].0, "HTTP-Referer");
        assert_eq!(headers[0].1, "https://github.com/sekai-labs/sekai");
    }
    ...
    async fn parse_extra_headers_env_whitespace_trimming() {
        let headers = parse_extra_headers_env("  X-Title : sekai , User-Agent : cli/1.0 ");
        assert_eq!(headers.len(), 2);
        assert_eq!(headers[0], ("X-Title".to_string(), "sekai".to_string()));
        assert_eq!(
            headers[1],
    ...
    async fn parse_extra_headers_env_trailing_comma() {
        let headers = parse_extra_headers_env("X-Title:sekai,");
        assert_eq!(headers.len(), 1);
        assert_eq!(headers[0], ("X-Title".to_string(), "sekai".to_string()));
    }
...
"#;
        ...
        assert_eq!(headers.get("X-Title").unwrap(), "sekai");
    }
...
"#;
    ...
    async fn config_save_and_load_tmpdir() {
        let dir = std::env::temp_dir().join("sekai_test_config");
        let _ = fs::remove_dir_all(&dir).await;
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_encrypts_nested_credentials() {
        let dir = std::env::temp_dir().join(format!(
            "sekai_test_nested_credentials_{}",
            uuid::Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn config_save_atomic_cleanup() {
        let dir =
            std::env::temp_dir().join(format!("sekai_test_config_{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&dir).await.unwrap();
...
"#;
    ...
    async fn whatsapp_config_backend_type_cloud_precedence_when_ambiguous() {
        let wc = WhatsAppConfig {
            ...
            verify_token: Some("ver".into()),
            app_secret: None,
            session_path: Some("~/.sekai/state/whatsapp-web/session.db".into()),
            pair_phone: None,
            pair_code: None,
            allowed_numbers: vec!["+1".into()],
            mention_only: false,
    ...
    async fn checklist_gateway_serde_roundtrip() {
        ...
        assert_eq!(parsed.path_prefix.as_deref(), Some("/sekai"));
...
"#;
    ...
    async fn env_override_api_key() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_API_KEY", "sk-test-env-key") };
        config.apply_env_overrides();
        assert_eq!(
            config
                .providers
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_API_KEY") };
    }
    ...
    async fn env_override_provider() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_PROVIDER", "anthropic") };
        config.apply_env_overrides();
        assert_eq!(config.providers.fallback.as_deref(), Some("anthropic"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_PROVIDER") };
    }
    ...
    async fn env_override_model_provider_alias() {
        ...
        unsafe { std::env::remove_var("SEKAI_PROVIDER") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_MODEL_PROVIDER", "openai-codex") };
        config.apply_env_overrides();
        assert_eq!(config.providers.fallback.as_deref(), Some("openai-codex"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_MODEL_PROVIDER") };
    }
...
"#;
    ...
    async fn env_override_open_skills_enabled_and_dir() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_OPEN_SKILLS_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_OPEN_SKILLS_DIR", "/tmp/open-skills") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_SKILLS_ALLOW_SCRIPTS", "yes") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_SKILLS_PROMPT_MODE", "compact") };
        config.apply_env_overrides();
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_OPEN_SKILLS_ENABLED") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_OPEN_SKILLS_DIR") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_SKILLS_ALLOW_SCRIPTS") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_SKILLS_PROMPT_MODE") };
    }
    ...
    async fn env_override_open_skills_enabled_invalid_value_keeps_existing_value() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_OPEN_SKILLS_ENABLED", "maybe") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_SKILLS_ALLOW_SCRIPTS", "maybe") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_SKILLS_PROMPT_MODE", "invalid") };
        config.apply_env_overrides();
    ...
    async fn env_override_zero_claw_provider_overrides_non_default_provider() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_PROVIDER", "openrouter") };
        // SAFETY: test-only, single-threaded test runner.
    ...
    async fn env_override_model() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_MODEL", "gpt-4o") };
        config.apply_env_overrides();
        assert_eq!(
            config
                .providers
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_MODEL") };
    }
    ...
    async fn model_provider_profile_maps_to_custom_endpoint() {
        ...
        // `sekai config get/set` CLI relies on.
    ...
    /// differed. That meant:
    ///
    /// - `sekai config get providers.fallback` returned the rewritten value
    ///   even though the on-disk TOML still held the user's literal key.
    /// - `sekai config set providers.fallback <new>` would persist `<new>`
...
"#;
    ...
    async fn save_repairs_bare_config_filename_using_runtime_resolution() {
        let _env_guard = env_override_lock().await;
        let temp_home =
            std::env::temp_dir().join(format!("sekai_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let resolved_config_path = temp_home.join(".sekai").join("config.toml");

        let original_home = std::env::var("HOME").ok();
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("HOME", &temp_home) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_WORKSPACE", &workspace_dir) };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_WORKSPACE") };
    ...
    async fn env_override_workspace() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_WORKSPACE", "/custom/workspace") };
        config.apply_env_overrides();
    ...
    async fn resolve_runtime_config_dirs_uses_env_config_dir_first() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_CONFIG_DIR", &explicit_config_dir) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_WORKSPACE") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_CONFIG_DIR") };
        let _ = fs::remove_dir_all(default_config_dir).await;
    }
    ...
    #[test]
    async fn default_path_under_config_dir_respects_sekai_config_dir() {
        let _env_guard = env_override_lock().await;
        let custom_dir = std::env::temp_dir().join("sekai-test-profile");
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_CONFIG_DIR", &custom_dir) };
        ...
        assert_eq!(
            ...
            "expected path under SEKAI_CONFIG_DIR, got: {result}"
        );
    }
    ...
    async fn load_or_init_workspace_suffix_uses_legacy_config_layout() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("sekai_test_home_{}", uuid::Uuid::new_v4()));
        let workspace_dir = temp_home.join("workspace");
        let legacy_config_path = temp_home.join(".sekai").join("config.toml");
    ...
    async fn load_or_init_workspace_override_keeps_existing_legacy_config() {
        ...
        let legacy_config_dir = temp_home.join(".sekai");
...
"#,
    ...
    async fn load_or_init_decrypts_feishu_channel_secrets() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("sekai_test_home_{}", uuid::Uuid::new_v4()));
        let config_dir = temp_home.join(".sekai");
    ...
    async fn load_or_init_uses_persisted_active_workspace_marker() {
        ...
        let temp_home =
            std::env::temp_dir().join(format!("sekai_test_home_{}", uuid::Uuid::new_v4()));
        let temp_default_dir = temp_home.join(".sekai");
    ...
    async fn load_or_init_env_workspace_override_takes_priority_over_marker() {
        ...
        unsafe { std::env::set_var("HOME", &temp_home) };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_WORKSPACE", &env_workspace_dir) };
    ...
    async fn persist_active_workspace_marker_is_cleared_for_default_config_dir() {
        let temp_home =
            std::env::temp_dir().join(format!("sekai_test_home_{}", uuid::Uuid::new_v4()));
        let default_config_dir = temp_home.join(".sekai");
...
"#,
    ...
    async fn env_override_empty_values_ignored() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_PROVIDER", "") };
        config.apply_env_overrides();
    ...
    async fn env_override_gateway_port() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_GATEWAY_PORT", "8080") };
        config.apply_env_overrides();
        assert_eq!(config.gateway.port, 8080);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_GATEWAY_PORT") };
    }
    ...
    async fn env_override_gateway_host() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_GATEWAY_HOST", "0.0.0.0") };
        config.apply_env_overrides();
        assert_eq!(config.gateway.host, "0.0.0.0");

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_GATEWAY_HOST") };
    }
    ...
    async fn env_override_require_pairing() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_REQUIRE_PAIRING", "false") };
        config.apply_env_overrides();
        assert!(!config.gateway.require_pairing);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_REQUIRE_PAIRING", "true") };
        config.apply_env_overrides();
        assert!(config.gateway.require_pairing);

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_REQUIRE_PAIRING") };
    }
    ...
    async fn env_override_temperature() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_TEMPERATURE", "0.5") };
        config.apply_env_overrides();
        assert!(
            (config
                .providers
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_TEMPERATURE") };
    }
    ...
    async fn env_override_temperature_out_of_range_ignored() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_TEMPERATURE", "3.0") };
        config.apply_env_overrides();
        assert!(
            (config
                .providers
    ...
    async fn env_override_reasoning_enabled() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_REASONING_ENABLED", "false") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_REASONING_ENABLED", "true") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(true));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_REASONING_ENABLED") };
    }
    ...
    async fn env_override_reasoning_invalid_value_ignored() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_REASONING_ENABLED", "maybe") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_enabled, Some(false));
    ...
    async fn env_override_reasoning_effort() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_REASONING_EFFORT", "HIGH") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("high"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_REASONING_EFFORT") };
    }
    ...
    async fn env_override_reasoning_effort_legacy_codex_env() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_CODEX_REASONING_EFFORT", "minimal") };
        config.apply_env_overrides();
        assert_eq!(config.runtime.reasoning_effort.as_deref(), Some("minimal"));

        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_CODEX_REASONING_EFFORT") };
    }
    ...
    async fn env_override_storage_provider_config() {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_STORAGE_PROVIDER", "qdrant") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_STORAGE_DB_URL", "http://localhost:6333") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_STORAGE_CONNECT_TIMEOUT_SECS", "15") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_STORAGE_PROVIDER") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_STORAGE_DB_URL") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_STORAGE_CONNECT_TIMEOUT_SECS") };
    }
    ...
    async fn env_override_proxy_scope_services() {
        ...
        let mut config = Config::default();
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_PROXY_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_HTTP_PROXY", "http://127.0.0.1:7890") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe {
            std::env::set_var(
                "SEKAI_PROXY_SERVICES",
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_PROXY_SCOPE", "services") };
    ...
    async fn env_override_proxy_scope_environment_applies_process_env() {
        ...
        unsafe { std::env::set_var("SEKAI_PROXY_ENABLED", "true") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_PROXY_SCOPE", "environment") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_HTTP_PROXY", "http://127.0.0.1:7890") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_HTTPS_PROXY", "http://127.0.0.1:7891") };
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_NO_PROXY", "localhost,127.0.0.1") };
...
[security.estop]
enabled = true
state_file = "~/.sekai/estop-state.json"
require_otp_to_resume = true
"#,
        );
