...
Complete guide to setting up a Sekai-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.sekai/models
cp models/ggml-base.bin ~/.sekai/models/
```
...
mkdir -p ~/.sekai/models/piper
cd ~/.sekai/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.sekai/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build Sekai Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/sekai-labs/sekai
cd sekai

# Build robot kit
cargo build --release -p sekai-robot-kit

# Build main sekai (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.sekai
nano ~/.sekai/robot.toml
```

```toml
# ~/.sekai/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.sekai/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/sekai_sensors.fifo"
...
```bash
# Configure Sekai to use robot tools
cat > ~/.sekai/config.toml << 'EOF'
...
cp ~/sekai/crates/robot-kit/SOUL.md ~/.sekai/workspace/

# Start agent
./target/release/sekai agent
```
...
# Start sensor loop
if [ ! -p /tmp/sekai_sensors.fifo ]; then
    mkfifo /tmp/sekai_sensors.fifo
fi
...
```bash
# /etc/systemd/system/sekai-robot.service
sudo tee /etc/systemd/system/sekai-robot.service << 'EOF'
[Unit]
Description=Sekai Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/sekai
...
sudo systemctl enable sekai-robot
sudo systemctl start sekai-robot

# Check status
sudo systemctl status sekai-robot
journalctl -u sekai-robot -f  # View logs
```
