...
A complete toolkit for building AI-powered robots with Sekai. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build Sekai with robot tools

```bash
# Clone and build
git clone https://github.com/sekai-labs/sekai
cd sekai
cargo build -p sekai-robot-kit --release
```
...
# Copy config
mkdir -p ~/.sekai
cp crates/robot-kit/robot.toml ~/.sekai/
cp crates/robot-kit/SOUL.md ~/.sekai/workspace/
...
# Test in mock mode
./target/release/sekai agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/sekai agent -m "Move forward 1 meter"
```
...
```rust
use sekai_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `sekai`, add a thin adapter that maps this
...
mkdir sekai-robot-kit
cp -r target/release/sekai sekai-robot-kit/
cp -r examples/robot_kit sekai-robot-kit/
cp -r ~/.sekai sekai-robot-kit/dot-sekai

# Include models
mkdir -p sekai-robot-kit/models
cp ~/.sekai/models/ggml-base.bin sekai-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf sekai-robot-kit.tar.gz sekai-robot-kit/

# Copy to USB
cp sekai-robot-kit.tar.gz /media/usb/TarBalls/
```
