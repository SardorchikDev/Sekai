...
//! daemon heartbeat path (`crates/sekai-runtime/src/daemon/mod.rs`) calls
...
use sekai_config::providers::ProvidersConfig;
...
async fn scheduled_run_does_not_leak_conversation_memory_into_provider_request() {
    ...
    // Matches `crates/sekai-runtime/src/daemon/mod.rs:476` / `:599`:
    //   crate::agent::run(
    ...
    let run_result = sekai_runtime::agent::run(
        config,
        Some(prompt),
        None,
        None,
