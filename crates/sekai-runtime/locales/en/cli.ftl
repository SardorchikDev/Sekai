...
cli-try-onboard = Try `sekai onboard` to initialize your workspace.
...
cli-config-about = Manage Sekai configuration
cli-update-about = Check for and apply Sekai updates
cli-self-test-about = Run diagnostic self-tests
cli-completions-about = Generate shell completion scripts
cli-desktop-about = Launch the Sekai companion desktop app
...
cli-wechat-bound-success = ✅ WeChat account bound successfully. You can talk to Sekai now.
...
cli-peripheral-flash-about = Flash Sekai firmware to an Arduino board
...
cli-migrate-openclaw-about = Import memory from an OpenClaw workspace into this Sekai workspace
...
cli-daemon-long-about =
    ...
    Launches the full Sekai runtime: gateway server, all configured channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and the cron scheduler. This is the recommended way to run Sekai in production or as an always-on assistant.

    Use 'sekai service install' to register the daemon as an OS service (systemd/launchd) for auto-start on boot.
...
cli-channel-long-about =
    ...
    Add, remove, list, send, and health-check channels that connect Sekai to messaging platforms. Supported channel types: telegram, discord, slack, whatsapp, matrix, imessage, email.
