...
const SERVICE_LABEL: &str = "com.sekai.daemon";
const WINDOWS_TASK_NAME: &str = "Sekai Daemon";
...
/// Returns whether the Sekai daemon service is currently running.
...
fn is_running_linux() -> bool {
    ...
    if run_capture(Command::new("systemctl").args(["--user", "is-active", "sekai.service"]))
        .map(|out| out.trim() == "active")
        .unwrap_or(false)
    {
        return true;
    }
    run_capture(Command::new("rc-service").args(["sekai", "status"]))
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "sekai.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["sekai", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "sekai.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["sekai", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "sekai.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["sekai", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["sekai", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/sekai");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn logs_linux(config: &Config, init_system: InitSystem, lines: usize, follow: bool) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            // OpenRC logs go to /var/log/sekai/error.log (as configured in the init script)
            let log_file = Path::new("/var/log/sekai/error.log");
            if !log_file.exists() {
                // Fall back to access log
                let access_log = Path::new("/var/log/sekai/access.log");
                if !access_log.exists() {
                    bail!("No log files found at /var/log/sekai/. Is the service installed?");
                }
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/sekai");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "sekai", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove sekai from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/sekai)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
/// corresponding `var/sekai` directory.
///
...
fn detect_homebrew_var_dir(exe: &Path) -> Option<PathBuf> {
    ...
    // Symlinked binary: <prefix>/bin/sekai
    // Cellar binary:    <prefix>/Cellar/sekai/<version>/bin/sekai
    let prefix = if path_str.contains("/Cellar/") {
        // Walk up from .../Cellar/sekai/<ver>/bin/sekai to the prefix
        let mut ancestor = exe.to_path_buf();
        while let Some(parent) = ancestor.parent() {
            ancestor = parent.to_path_buf();
            if ancestor.file_name().is_some_and(|n| n == "Cellar") {
                // prefix is one level above Cellar
                return ancestor.parent().map(|p| p.join("var").join("sekai"));
            }
        }
        return None;
    } else if let Some(bin_parent) = exe.parent() {
        // <prefix>/bin/sekai → check if <prefix>/Cellar exists (Homebrew marker)
    ...
    prefix.map(|p| p.join("var").join("sekai"))
}
...
fn install_macos(config: &Config) -> Result<()> {
    ...
    // data so that `brew services start sekai` works out of the box.
    ...
    // When running under Homebrew, inject SEKAI_CONFIG_DIR and
...
"#,
    ...
    println!("   Start with: sekai service start");
    Ok(())
}
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "sekai.service"]));
...
/// Check if the sekai user exists and has expected properties.
...
fn check_sekai_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "sekai"]).output();
    let is_alpine = Path::new("/etc/alpine-release").exists();

    let (del_cmd, add_cmd) = if is_alpine {
        (
            "deluser sekai && delgroup sekai",
            "addgroup -S sekai && adduser -S -s /sbin/nologin -H -D -G sekai sekai",
        )
    } else {
        ("userdel sekai", "useradd -r -s /sbin/nologin sekai")
    };
        ...
        Ok(output) if output.status.success() => {
            ...
            if parts.len() >= 7 {
                ...
                if uid.parse::<u32>().unwrap_or(999) >= 1000 {
                    bail!(
                        "User 'sekai' exists but has unexpected UID {} (expected system UID < 1000).\n\
                ...
                if !shell.contains("nologin") && !shell.contains("false") {
                    bail!(
                        "User 'sekai' exists but has unexpected shell '{}'.\n\
                ...
                if home != "/var/lib/sekai" && home != "/nonexistent" {
                    eprintln!(
                        "⚠️  Warning: sekai user has home directory '{}' (expected /var/lib/sekai or /nonexistent)",
                        home
                    );
                }
...
fn ensure_sekai_user() -> Result<()> {
    let output = Command::new("getent").args(["passwd", "sekai"]).output();
    if let Ok(output) = output
        && output.status.success()
    {
        return check_sekai_user();
    }

    let is_alpine = Path::new("/etc/alpine-release").exists();

    if is_alpine {
        let group_output = Command::new("getent").args(["group", "sekai"]).output();
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "sekai"])
                .output()
                .context("Failed to create sekai group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create sekai group: {}", stderr.trim());
            }
            println!("✅ Created system group: sekai");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create sekai user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create sekai user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "sekai"])
            .output()
            .context("Failed to create sekai user")?;
    ...
    println!("✅ Created system user: sekai");
    Ok(())
}

/// Change ownership of a path to sekai:sekai
#[cfg(unix)]
fn chown_to_sekai(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["sekai:sekai", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to sekai:sekai: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_sekai(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_sekai(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "sekai:sekai", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to sekai:sekai: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_sekai(_path: &Path) -> Result<()> {
    Ok(())
}
...
fn resolve_invoking_user_config_dir() -> Option<PathBuf> {
        ...
        && output.status.success()
    {
        ...
        if fields.len() >= 6 {
            return Some(PathBuf::from(fields[5]).join(".sekai"));
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'sekai' cannot write {} ({details}). \
             Re-run `sudo sekai service install` and ensure ownership is sekai:sekai.",
            path.display(),
        );
    }
...
command_user="sekai:sekai"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/sekai/access.log"
error_log="/var/log/sekai/error.log"
...
start_pre() {{
    checkpath --directory --owner sekai:sekai --mode 0750 /var/lib/sekai
}}
"#,
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/sekai");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo sekai service install"
        );
    }
    ...
    let config_dir = Path::new("/etc/sekai");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/sekai");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_sekai(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by sekai:sekai)",
    ...
    chown_recursive_to_sekai(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/sekai");
    ...
    run_checked(Command::new("rc-update").args(["add", "sekai", "default"]))?;
    println!("✅ Installed OpenRC service: /etc/init.d/sekai");
    println!("   Config path: /etc/sekai/config.toml");
    println!("   Start with: sudo sekai service start");
    let _ = config;
    Ok(())
}
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("sekai-daemon.cmd");
    let stdout_log = logs_dir.join("daemon.stdout.log");
    let stderr_log = logs_dir.join("daemon.stderr.log");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/sekai.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "Sekai Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/sekai");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/sekai"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"sekai\""));
        assert!(script.contains("description=\"Sekai daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/sekai\""));
        assert!(script.contains("command_args=\"--config-dir /etc/sekai daemon\""));
        assert!(!script.contains("env SEKAI_CONFIG_DIR"));
        assert!(!script.contains("env SEKAI_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"sekai:sekai\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/sekai/access.log\""));
        assert!(script.contains("error_log=\"/var/log/sekai/error.log\""));
    ...
    fn generate_openrc_script_sets_home_for_browser() {
        ...
        assert!(
            script.contains("export HOME=\"/var/lib/sekai\""),
    ...
    fn generate_openrc_script_creates_home_directory() {
        ...
        assert!(
            script.contains("checkpath --directory --owner sekai:sekai"),
            "start_pre must ensure /var/lib/sekai exists with correct ownership"
        );
    }
    ...
    fn systemd_unit_contains_home_and_pass_environment() {
        let unit = "[Unit]\n\
             ...
             [Service]\n\
             Type=simple\n\
             ExecStart=/usr/local/bin/sekai daemon\n\
             Restart=always\n\
             RestartSec=3\n\
             # Ensure HOME is set so headless browsers can create profile/cache dirs.\n\
             Environment=HOME=%h\n\
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/sekai");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/sekai");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/sekai");
    ...
    fn openrc_writability_probe_prefers_runuser_when_available() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/sekai"), true);
    ...
    fn detect_homebrew_var_dir_from_cellar_path() {
        let exe = PathBuf::from("/opt/homebrew/Cellar/sekai/1.2.3/bin/sekai");
        let var_dir = detect_homebrew_var_dir(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/opt/homebrew/var/sekai")));
    }

    #[test]
    fn detect_homebrew_var_dir_intel_cellar_path() {
        let exe = PathBuf::from("/usr/local/Cellar/sekai/1.0.0/bin/sekai");
        let var_dir = detect_homebrew_var_dir(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/usr/local/var/sekai")));
    }

    #[test]
    fn detect_homebrew_var_dir_non_homebrew_path() {
        let exe = PathBuf::from("/home/user/.cargo/bin/sekai");
        let var_dir = detect_homebrew_var_dir(&exe);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/sekai/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/sekai/workspace'".to_string(),
                "sekai".to_string()
            ]
        );
    }
    ...
    fn tail_file_errors_on_missing_file() {
        let missing = Path::new("/tmp/sekai-test-nonexistent-log-file.log");
