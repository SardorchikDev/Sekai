...
use sekai_config::schema::{DelegateAgentConfig, DelegateToolConfig};
use sekai_memory::{Memory, NamespacedMemory};
...
impl DelegateTool {
    ...
    pub fn with_multimodal_config(
        mut self,
        config: sekai_config::schema::MultimodalConfig,
    ) -> Self {
...
impl DelegateTool {
    ...
    async fn execute_sync(
        ...
        let provider: Box<dyn Provider> = match sekai_providers::create_provider_with_options(
...
impl DelegateTool {
    ...
    fn build_enriched_system_prompt(
        ...
        let ctx = PromptContext {
            ...
            tools: sub_tools,
            skills: &skills,
            skills_prompt_mode: sekai_config::schema::SkillsPromptInjectionMode::Full,
...
mod tests {
    ...
    use anyhow::anyhow;
    use sekai_config::schema::{
        DEFAULT_DELEGATE_AGENTIC_TIMEOUT_SECS, DEFAULT_DELEGATE_TIMEOUT_SECS,
    };
    use sekai_providers::{ChatRequest, ChatResponse, ToolCall};
    ...
    async fn check_result_retrieves_persisted_background_result() {
        let workspace = std::env::temp_dir().join(format!(
            "sekai_delegate_check_retrieve_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn check_result_rejects_path_traversal() {
        let workspace = std::env::temp_dir().join(format!(
            "sekai_delegate_traversal_check_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn cancel_task_rejects_path_traversal() {
        let workspace = std::env::temp_dir().join(format!(
            "sekai_delegate_traversal_cancel_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
