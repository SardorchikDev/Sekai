...
use sekai_config::schema::{CronJobDecl, CronScheduleDecl};
...
async fn run_agent_job(
    ...
    let memory_context = if !job.uses_memory {
        ...
        match sekai_memory::create_memory(
            &config.memory,
            &config.workspace_dir,
            config
                .providers
    ...
    match run_result {
        ...
        Err(e) => {
            ...
            let mem_session_key = sekai_api::session_keys::sanitize_session_key(&format!(
                "cli:{}",
                session_path.display()
            ));
            if let Ok(mem) = sekai_memory::create_memory(
                &config.memory,
                &config.workspace_dir,
                config
                    .providers
...
///
/// Uses `sh -c <command>` (non-login shell). On Windows, Sekai users
...
mod tests {
    ...
    async fn persist_job_result_delivery_stubbed_succeeds() {
        // Delivery is stubbed (moved to sekai-channels orchestrator).
    ...
    // scan_and_redact_output tests moved to sekai-channels orchestrator
