...
async fn probe_models(config: &Config) -> Vec<DiagResult> {
    ...
    for provider_name in &targets {
        let result = match sekai_providers::create_provider(provider_name, None) {
...
pub async fn run(config: &Config) -> Result<()> {
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `sekai doctor` again.");
    }
...
pub async fn run_models(
    ...
    if targets.is_empty() {
        anyhow::bail!("No configured providers to probe — run `sekai onboard providers` first");
    }

    println!("🩺 Sekai Doctor — Model Catalog Probe");
    ...
    for provider_name in &targets {
        ...
        let outcome = match sekai_providers::create_provider(provider_name, None) {
            Ok(handle) => handle.list_models().await,
            Err(e) => Err(e),
        };
...
pub fn run_traces(
    ...
    println!();
    println!("Use `sekai doctor traces --id <trace-id>` to inspect a full event payload.");
    Ok(())
}
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `sekai onboard` to set one up",
        ));
    }
...
fn provider_validation_error(name: &str) -> Option<String> {
    match sekai_providers::create_provider(name, None) {
        Ok(_) => None,
        Err(err) => Some(
            err.to_string()
                .lines()
...
mod tests {
    ...
    fn config_validation_warns_empty_model_route() {
        let mut config = Config::default();
        config.providers.model_routes = vec![sekai_config::schema::ModelRouteConfig {
            hint: "fast".into(),
            provider: "groq".into(),
            model: String::new(),
            api_key: None,
    ...
    fn config_validation_warns_empty_embedding_route_model() {
        let mut config = Config::default();
        config.providers.embedding_routes = vec![sekai_config::schema::EmbeddingRouteConfig {
            hint: "semantic".into(),
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(
            first
                ...
                .is_some_and(|name| name.starts_with(".sekai_doctor_probe_"))
        );
    }
    ...
    fn config_validation_reports_delegate_agents_in_sorted_order() {
        ...
        config.agents.insert(
            "zeta".into(),
            sekai_config::schema::DelegateAgentConfig {
