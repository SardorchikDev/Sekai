...
/// `sekai_providers::default_provider_config`, etc.) and copy its
...
trait PropFieldSource {
    fn prop_fields(&self) -> Vec<sekai_config::traits::PropFieldInfo>;
}

impl PropFieldSource for sekai_config::schema::ModelProviderConfig {
    fn prop_fields(&self) -> Vec<sekai_config::traits::PropFieldInfo> {
        sekai_config::schema::ModelProviderConfig::prop_fields(self)
    }
}
...
///
/// Source of truth is `sekai_providers::default_provider_config(name)`
...
pub fn apply_provider_trait_defaults(cfg: &mut Config, provider: &str, prefix: &str) -> Result<()> {
    ...
    let source_prefix = sekai_config::schema::ModelProviderConfig::configurable_prefix();
