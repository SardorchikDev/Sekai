...
//! `sekai-config::traits`). Section-scoped entry points let callers run
//! just one slice (`sekai onboard channels`) or the whole flow.
//!
...
use sekai_config::traits::{Answer, OnboardUi, PropKind, SelectItem};
...
/// `sekai onboard` picks up where they left off.
...
fn is_known_provider_name(provider: &str) -> bool {
    ...
    sekai_providers::list_providers().iter().any(|entry| {
...
async fn providers(cfg: &mut Config, ui: &mut dyn OnboardUi, flags: &Flags) -> Result<Nav> {
    ...
    ui.note(
        "Paste an API key (e.g. `sk-ant-…` for Anthropic, `sk-…` for OpenAI) \
         ...
         sekai auth login --provider <name>",
    );

    // Menu is driven by sekai_providers::list_providers() — single source
    // of truth for canonical names, display names, aliases.
    let entries = sekai_providers::list_providers();
...
/// schema fields of `sekai_providers::default_provider_config(provider)`
...
fn provider_trait_defaults_for_prompts(provider: &str, prefix: &str) -> Vec<FieldDefault> {
    let defaults = sekai_providers::default_provider_config(provider);
    let source_dot = format!(
        "{}.",
        sekai_config::schema::ModelProviderConfig::configurable_prefix()
    );
    defaults
        .prop_fields()
        .into_iter()
...
///
/// Calls `Provider::list_models()` (no auth — see `sekai-providers`
...
async fn prompt_model(cfg: &mut Config, ui: &mut dyn OnboardUi, provider: &str) -> Result<Nav> {
    ...
    let catalog_models = match sekai_providers::create_provider(provider, None) {
        Ok(handle) => {
    ...
    loop {
        ...
        let mut options: Vec<SelectItem> = all_channels
            ...
            .map(|name| {
                ...
                // `schema_walk_picker` in `crates/sekai-gateway/src/api_onboard.rs`.
                let is_active = cfg
...
async fn memory(cfg: &mut Config, ui: &mut dyn OnboardUi, flags: &Flags) -> Result<Nav> {
    ...
    let backends = sekai_memory::selectable_memory_backends();
...
mod tests {
    ...
    use sekai_config::schema::{Config, ModelProviderConfig};
