...
/// - `X-Sekai-Timestamp` — unix epoch seconds
/// - `X-Sekai-Nonce` — random UUID v4
/// - `X-Sekai-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-Sekai-Timestamp", timestamp.to_string())
            .header("X-Sekai-Nonce", &nonce)
            .header("X-Sekai-Signature", &signature)
