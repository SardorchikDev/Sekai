...
const OPEN_SKILLS_SYNC_MARKER: &str = ".sekai-open-skills-sync";
...
// ─── Skills registry (sekai-skills) ────────────────────────────────────────
const SKILLS_REGISTRY_REPO_URL: &str = "https://github.com/sekai-labs/sekai-skills";
const SKILLS_REGISTRY_DIR_NAME: &str = "skills-registry";
const SKILLS_REGISTRY_SYNC_MARKER: &str = ".sekai-skills-registry-sync";
const SKILLS_REGISTRY_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24;

/// A skill is a user-defined or community-built capability.
/// Skills live in `~/.sekai/workspace/skills/<name>/SKILL.md`
...
/// Trust tier of a skill listed in the `sekai-skills` registry.
///
...
/// a separate curation signal (sekai-labs hand-picked, but still authored
/// outside sekai-labs) and we expect to render it differently later — e.g.
/// "Featured — community-curated by sekai-labs but not maintained by us".
...
pub fn build_install_tier_banner(name: &str, version: Option<&str>, tier: SkillTier) -> String {
    ...
    match tier {
        SkillTier::Official => {
            format!("Installing {name} v{version_label} — Official (sekai-labs maintained)\n")
        }
        SkillTier::Community | SkillTier::Featured | SkillTier::Unknown => format!(
            "Installing {name} v{version_label} — Community submission\n\
             This skill is not audited by Sekai. Review the skill content\n\
             and run `sekai skills audit {name}` before granting any\n\
    ...
    if scripts_blocked && !allow_scripts {
        ...
        eprintln!(
            "warning: skill '{}' was skipped because it contains script files. \
             Set `skills.allow_scripts = true` in your sekai config to enable it.",
            path.file_name()
...
fn open_skills_enabled(config_open_skills_enabled: Option<bool>) -> bool {
    let env_override = std::env::var("SEKAI_OPEN_SKILLS_ENABLED").ok();
...
fn resolve_open_skills_dir(config_open_skills_dir: Option<&str>) -> Option<PathBuf> {
    let env_dir = std::env::var("SEKAI_OPEN_SKILLS_DIR").ok();
...
pub fn skills_to_prompt_with_mode(
    skills: &[Skill],
    workspace_dir: &Path,
    mode: sekai_config::schema::SkillsPromptInjectionMode,
) -> String {
    use std::fmt::Write;
    ...
    let mut prompt = match mode {
        sekai_config::schema::SkillsPromptInjectionMode::Full => String::from(
            "## Available Skills\n\n\
             Skill instructions and tool metadata are preloaded below.\n\
             Follow these instructions directly; do not read skill files at runtime unless the user asks.\n\n\
             <available_skills>\n",
        ),
        sekai_config::schema::SkillsPromptInjectionMode::Compact => String::from(
            "## Available Skills\n\n\
...
) -> Vec<Box<dyn sekai_api::tool::Tool>> {
    let mut tools: Vec<Box<dyn sekai_api::tool::Tool>> = Vec::new();
    for skill in skills {
        for tool in &skill.tools {
            match tool.kind.as_str() {
                "shell" | "script" => {
                    ...
                    tools.push(Box::new(sekai_tools::wrappers::RateLimitedTool::new(
                        inner,
                        security.clone(),
                    )));
                }
...
#[cfg(feature = "plugins-wasm")]
pub fn load_plugin_skills_from_config(config: &sekai_config::schema::Config) -> Vec<Skill> {
    ...
    let signature_mode = sekai_plugins::host::PluginHost::parse_signature_mode(
        &config.plugins.security.signature_mode,
    );
    let trusted_keys = config.plugins.security.trusted_publisher_keys.clone();

    let host = match sekai_plugins::host::PluginHost::with_security(
        &parent,
        signature_mode,
        trusted_keys,
    ) {
...
mod registry_tests {
    ...
    fn test_is_registry_source_rejects_paths() {
        ...
        assert!(!is_registry_source("~/.sekai/skills/foo"));
    }
    ...
    fn build_install_tier_banner_official_is_single_line() {
        ...
        assert!(banner.contains("Official (sekai-labs maintained)"));
    ...
    fn build_install_tier_banner_community_warns() {
        ...
        assert!(banner.contains("not audited by Sekai"));
        assert!(banner.contains("sekai skills audit discord-moderator"));
    }
