...
pub fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `sekai onboard` to configure channels/providers."
        );
    };
    ...
    // standalone book walkthrough stay here so `sekai integration info
    ...
    match entry.name.as_str() {
        "Telegram" => {
            ...
            println!("    3. Run: sekai onboard --channels-only");
            println!("    4. Start: sekai channel start");
        }
        "Discord" => {
            println!("  Setup:");
            ...
            println!("    4. Run: sekai onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "Browser" => {
            println!("  Built-in:");
            println!("    Sekai can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.sekai/workspace/cron/");
            println!("    Run: sekai cron list");
        }
        "Weather" => {
            println!("  Built-in:");
        ...
        _ if entry.category == IntegrationCategory::Chat => {
            println!("  Setup:");
            println!("    Run: sekai onboard --channels-only");
        }
        _ => {}
    }
