...
//! - AI providers: `sekai_providers::list_providers()` (each
...
use sekai_providers::ProviderActivation;
...
fn evaluate_provider_activation(
    config: &Config,
    info: &sekai_providers::ProviderInfo,
) -> IntegrationStatus {
...
pub fn all_integrations(config: &Config) -> Vec<IntegrationEntry> {
    ...
    let providers = sekai_providers::list_providers()
        .into_iter()
        .map(|info| {
...
mod tests {
    use super::*;
    use sekai_config::schema::Config;
    use sekai_config::schema::{IMessageConfig, MatrixConfig, StreamMode, TelegramConfig};
    ...
    fn regional_provider_aliases_activate_expected_ai_integrations() {
        ...
        for (fallback_key, canonical) in cases {
            ...
            let entries = all_integrations(&config);
            let info = sekai_providers::list_providers()
                .into_iter()
