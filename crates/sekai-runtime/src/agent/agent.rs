...
use sekai_memory::{self, Memory, MemoryCategory};
use sekai_providers::{self, ChatMessage, ChatRequest, ConversationMessage, Provider};
use sekai_tool_call_parser::strip_think_tags;

// Re-export TurnEvent from sekai-types for backwards compatibility.
pub use sekai_api::agent::TurnEvent;
...
pub struct Agent {
    ...
    config: sekai_config::schema::AgentConfig,
    multimodal_config: sekai_config::schema::MultimodalConfig,
    model_name: String,
    temperature: f64,
    workspace_dir: std::path::PathBuf,
    identity_config: sekai_config::schema::IdentityConfig,
    skills: Vec<crate::skills::Skill>,
    skills_prompt_mode: sekai_config::schema::SkillsPromptInjectionMode,
    auto_save: bool,
    memory_session_id: Option<String>,
    history: Vec<ConversationMessage>,
    classification_config: sekai_config::schema::QueryClassificationConfig,
    available_hints: Vec<String>,
    route_model_by_hint: HashMap<String, String>,
    #[allow(dead_code)] // WIP: stored for future runtime tool filtering
    allowed_tools: Option<Vec<String>>,
    response_cache: Option<Arc<sekai_memory::response_cache::ResponseCache>>,
...
impl AgentChannelHandles {
    ...
    pub fn register_channel(
        ...
        channel: Arc<dyn sekai_api::channel::Channel>,
    ) {
        let name = name.into();
    ...
    pub fn get_channel(&self, name: &str) -> Option<Arc<dyn sekai_api::channel::Channel>> {
        for handle in [&self.ask_user, &self.reaction, &self.escalate, &self.poll]
            .into_iter()
            .flatten()
        {
...
pub struct AgentBuilder {
    ...
    config: Option<sekai_config::schema::AgentConfig>,
    multimodal_config: Option<sekai_config::schema::MultimodalConfig>,
    model_name: Option<String>,
    temperature: Option<f64>,
    workspace_dir: Option<std::path::PathBuf>,
    identity_config: Option<sekai_config::schema::IdentityConfig>,
    skills: Option<Vec<crate::skills::Skill>>,
    skills_prompt_mode: Option<sekai_config::schema::SkillsPromptInjectionMode>,
    auto_save: Option<bool>,
    memory_session_id: Option<String>,
    classification_config: Option<sekai_config::schema::QueryClassificationConfig>,
...
impl AgentBuilder {
    ...
    pub fn config(mut self, config: sekai_config::schema::AgentConfig) -> Self {
    ...
    pub fn response_cache(
        mut self,
        cache: Option<Arc<sekai_memory::response_cache::ResponseCache>>,
    ) -> Self {
...
impl Agent {
    ...
    fn parse_response_for_effective_tools(
        &self,
        response: &sekai_providers::ChatResponse,
    ...
    async fn from_config_with_session_cwd_and_mcp_approval_mode(
        ...
        let security = Arc::new({
            ...
            // Sekai workspace (where skills, identity, and config data live)
        ...
        let memory: Arc<dyn Memory> =
            Arc::from(sekai_memory::create_memory_with_storage_and_routes(
                &config.memory,
        ...
        let provider: Box<dyn Provider> = sekai_providers::create_routed_provider_with_options(
            provider_name,
            fallback_provider_ag.and_then(|e| e.api_key.as_deref()),
        ...
        let response_cache = if config.memory.response_cache_enabled {
            sekai_memory::response_cache::ResponseCache::with_hot_cache(
                &config.workspace_dir,
    ...
    async fn prepare_provider_messages(
        ...
        let prepared = sekai_providers::multimodal::prepare_messages_for_provider(
            messages,
    ...
    async fn execute_tool_call(&self, call: &ParsedToolCall) -> ToolExecutionResult {
            ...
            && approval_requirement == ApprovalRequirement::Prompt
        {
            ...
            let (decision, decision_channel) = if mgr.is_non_interactive() {
                ...
                let ch_request = sekai_api::channel::ChannelApprovalRequest {
                    tool_name: request.tool_name.clone(),
                    arguments_summary: crate::approval::summarize_args(&request.arguments),
                };
                let mut channel_decision: Option<sekai_api::channel::ChannelApprovalResponse> =
                    None;
                ...
                let channels: Vec<(String, Arc<dyn sekai_api::channel::Channel>)> = self
                    .channel_handles
                    .ask_user
                    .as_ref()
                    .map(|h| {
                ...
                let approval = match channel_decision {
                    Some(sekai_api::channel::ChannelApprovalResponse::Approve) => {
                        ApprovalResponse::Yes
                    }
                    Some(sekai_api::channel::ChannelApprovalResponse::AlwaysApprove) => {
                        ApprovalResponse::Always
                    }
                    Some(sekai_api::channel::ChannelApprovalResponse::Deny) => {
                        ApprovalResponse::No
                    }
                    None => {
                        tracing::warn!(
            ...
            let cache_key = if self.temperature == 0.0 {
                self.response_cache.as_ref().map(|_| {
                    ...
                    let system = messages
                        ...
                        .map(|m| m.content.as_str());
                    sekai_memory::response_cache::ResponseCache::cache_key(
                        &effective_model,
                        system,
                        last_user,
                    )
    ...
    pub async fn turn_streamed(
        ...
        for _ in 0..self.config.max_tool_iterations {
            ...
            let stream_opts =
                sekai_providers::traits::StreamOptions::new(self.provider.supports_streaming());
            let mut stream = self.provider.stream_chat(
                sekai_providers::ChatRequest {
                    messages: &prepared_messages,
                    tools: if self.should_send_tool_specs() {
                        Some(&self.tool_specs)
                    } else {
            ...
            let mut streamed_tool_calls: Vec<sekai_providers::traits::ToolCall> = Vec::new();
            let mut streamed_usage: Option<sekai_providers::traits::TokenUsage> = None;
            ...
            loop {
                ...
                match item {
                    Ok(event) => match event {
                        sekai_providers::traits::StreamEvent::TextDelta(chunk) => {
                        ...
                        sekai_providers::traits::StreamEvent::PreExecutedToolResult {
                            name,
                            output,
                        } => {
                        ...
                        sekai_providers::traits::StreamEvent::Usage(usage) => {
                            streamed_usage = Some(usage);
                        }
                        sekai_providers::traits::StreamEvent::Final => break,
                    },
                    Err(_) => break,
                }
            }
    ...
    pub async fn run_interactive(&mut self) -> Result<()> {
        println!("🦀 Sekai Interactive Mode");
        ...
        let listen_handle = tokio::spawn(async move {
            let _ = sekai_api::channel::Channel::listen(&*cli, tx).await;
        });
...
mod tests {
    ...
    struct MockProvider {
        responses: Mutex<Vec<sekai_providers::ChatResponse>>,
    }
    ...
    impl Provider for MockProvider {
        ...
        async fn chat(
            ...
            _model: &str,
            _temperature: Option<f64>,
        ) -> Result<sekai_providers::ChatResponse> {
            let mut guard = self.responses.lock();
            if guard.is_empty() {
                return Ok(sekai_providers::ChatResponse {
                    text: Some("done".into()),
                    tool_calls: vec![],
                    usage: None,
                    reasoning_content: None,
    ...
    impl Provider for MultimodalCaptureProvider {
        ...
        async fn chat(
            ...
            Ok(sekai_providers::ChatResponse {
                text: Some("done".into()),
                tool_calls: vec![],
                usage: None,
                reasoning_content: None,
        ...
        fn stream_chat(
            ...
            _model: &str,
            _temperature: Option<f64>,
            _options: sekai_providers::traits::StreamOptions,
        ) -> futures_util::stream::BoxStream<
            'static,
            sekai_providers::traits::StreamResult<sekai_providers::traits::StreamEvent>,
        > {
            ...
            if self.streamed {
                let chunk = sekai_providers::traits::StreamEvent::TextDelta(
                    sekai_providers::traits::StreamChunk {
                ...
                stream::iter(vec![
                    Ok(chunk),
                    Ok(sekai_providers::traits::StreamEvent::Final),
                ])
                .boxed()
            } else {
                stream::iter(vec![Ok(sekai_providers::traits::StreamEvent::Final)]).boxed()
            }
        }
    ...
    struct ApprovalChannel {
        response: sekai_api::channel::ChannelApprovalResponse,
        requests: Arc<AtomicUsize>,
    }

    #[async_trait]
    impl sekai_api::channel::Channel for ApprovalChannel {
        fn name(&self) -> &str {
            "acp"
        }

        async fn send(&self, _message: &sekai_api::channel::SendMessage) -> anyhow::Result<()> {
            Ok(())
        }
        ...
        async fn request_approval(
            &self,
            _recipient: &str,
            _request: &sekai_api::channel::ChannelApprovalRequest,
        ) -> anyhow::Result<Option<sekai_api::channel::ChannelApprovalResponse>> {
    ...
    async fn turn_without_tools_returns_text() {
        let provider = Box::new(MockProvider {
            responses: Mutex::new(vec![sekai_providers::ChatResponse {
        ...
        let memory_cfg = sekai_config::schema::MemoryConfig {
            backend: "none".into(),
            ..sekai_config::schema::MemoryConfig::default()
        };
        let mem: Arc<dyn Memory> = Arc::from(
            sekai_memory::create_memory(&memory_cfg, std::path::Path::new("/tmp"), None)
    ...
    fn native_agent_prompt_omits_duplicate_tools_section() {
        ...
        let mem: Arc<dyn Memory> = Arc::from(
            sekai_memory::create_memory(&memory_cfg, workspace.path(), None)
                .expect("memory creation should succeed with valid config"),
        );
        let observer: Arc<dyn Observer> = Arc::from(crate::observability::NoopObserver {});
    ...
    async fn direct_agent_tool_execution_requests_acp_approval() {
        ...
        let approval_cfg = sekai_config::schema::AutonomyConfig {
        ...
        let channel: Arc<dyn sekai_api::channel::Channel> = Arc::new(ApprovalChannel {
            response: sekai_api::channel::ChannelApprovalResponse::Approve,
    ...
    async fn direct_agent_tool_execution_denies_when_acp_rejects() {
        ...
        agent.channel_handles.ask_user = Some(Arc::clone(&handle));
        let channel: Arc<dyn sekai_api::channel::Channel> = Arc::new(ApprovalChannel {
            response: sekai_api::channel::ChannelApprovalResponse::Deny,
            requests: Arc::clone(&approval_requests),
        });
        handle.write().insert("acp".to_string(), channel);
    ...
    async fn direct_agent_shell_does_not_trust_model_supplied_approved_arg() {
        ...
        let approval_cfg = sekai_config::schema::AutonomyConfig::default();
        let mut agent = Agent::builder()
            .provider(provider)
    ...
    async fn direct_agent_shell_keeps_runtime_approval_from_always_allowlist() {
        ...
        agent.channel_handles.ask_user = Some(Arc::clone(&handle));
        let channel: Arc<dyn sekai_api::channel::Channel> = Arc::new(ApprovalChannel {
            response: sekai_api::channel::ChannelApprovalResponse::AlwaysApprove,
            requests: Arc::clone(&approval_requests),
        });
        handle.write().insert("acp".to_string(), channel);
        ...
        let provider = Box::new(MockProvider {
            responses: Mutex::new(vec![
                sekai_providers::ChatResponse {
                    ...
                    tool_calls: vec![sekai_providers::ToolCall {
                        id: "tc1".into(),
                        name: "echo".into(),
                        arguments: "{}".into(),
                        extra_content: None,
    ...
    async fn from_config_passes_extra_headers_to_custom_provider() {
        ...
        let mut config = sekai_config::schema::Config {
            workspace_dir,
        ...
        config.providers.fallback = Some(format!("custom:http://{addr}"));
        {
            ...
            entry.extra_headers.insert(
                ...
                "sekai-web-test/1.0".to_string(),
            );
            entry
                .extra_headers
                .insert("X-Title".to_string(), "sekai-web".to_string());
        }
    ...
    fn session_cwd_keeps_workspace_in_allowed_roots() {
        let workspace = std::env::temp_dir().join("sekai_test_session_cwd_workspace");
        let session = std::env::temp_dir().join("sekai_test_session_cwd_session");
        ...
        let autonomy = sekai_config::schema::AutonomyConfig::default();
    ...
    impl Provider for StreamToolCaptureProvider {
        ...
        > {
            ...
            *count += 1;
            if *count == 1 {
                let tc = sekai_providers::traits::StreamEvent::ToolCall(
                    sekai_providers::ToolCall {
                        id: "00000000-0000-0000-0000-000000000001".into(),
                        name: "echo".into(),
                        arguments: "{}".into(),
                        extra_content: None,
    ...
    async fn turn_normalizes_user_image_markers_before_provider_call() {
        ...
        let mut agent = Agent::builder()
            ...
            .tool_dispatcher(Box::new(NativeToolDispatcher))
            .workspace_dir(std::path::PathBuf::from("/tmp"))
            .multimodal_config(sekai_config::schema::MultimodalConfig::default())
            .build()
            .expect("agent builder should succeed with valid config");
    ...
    fn trim_history_does_not_leave_orphan_tool_results() {
        use sekai_providers::{ToolCall, ToolResultMessage};
        ...
        let agent_config = sekai_config::schema::AgentConfig {
            max_history_messages: 4,
            ..sekai_config::schema::AgentConfig::default()
        };
    ...
    impl Provider for NarrationStreamProvider {
        ...
        > {
            ...
            *count += 1;
            if *count == 1 {
                stream::iter(vec![
                    Ok(sekai_providers::traits::StreamEvent::TextDelta(
                        sekai_providers::traits::StreamChunk {
                    ...
                    Ok(sekai_providers::traits::StreamEvent::ToolCall(
                        sekai_providers::ToolCall {
                            id: "tc1".into(),
                            name: "echo".into(),
                            arguments: "{}".into(),
