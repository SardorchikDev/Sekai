...
use sekai_api::provider::{ChatMessage, Provider};
use sekai_memory::traits::Memory;
use sekai_providers::multimodal;

pub use sekai_config::scattered_types::ContextCompressionConfig;
        ...
        if let Some(ref memory) = self.memory {
            ...
            if let Err(e) = memory
                .store(
                    &facts_key,
                    &summary,
                    sekai_memory::traits::MemoryCategory::Daily,
                    None,
                )
                .await
            {
...
mod tests {
    ...
    impl Provider for CaptureSummarizerProvider {
        ...
        async fn chat(
            &self,
            _request: sekai_api::provider::ChatRequest<'_>,
            _model: &str,
            _temperature: Option<f64>,
        ) -> Result<sekai_api::provider::ChatResponse> {
    ...
    fn test_build_summarizer_transcript_strips_all_attachment_kinds_for_non_vision_provider() {
        ...
        // `crates/sekai-channels/src/util.rs`. Regression: a `[PHOTO:]`
