...
//!    `~/.sekai/hardware/datasheets/<device>.pdf`.
...
/// Manages device datasheet files in `~/.sekai/hardware/datasheets/`.
...
impl DatasheetManager {
    /// Create a manager rooted at the default Sekai datasheets directory.
    pub fn new() -> Option<Self> {
        let home = directories::BaseDirs::new()?.home_dir().to_path_buf();
        Some(Self {
            datasheet_dir: home.join(".sekai").join("hardware").join("datasheets"),
        })
    }
    ...
    ///
    /// The file is saved as `~/.sekai/hardware/datasheets/<device_name>.pdf`.
    ...
    pub async fn download_datasheet(
        ...
        let client = reqwest::Client::builder()
            .user_agent("Sekai/0.1 (datasheet downloader)")
...
impl Default for DatasheetManager {
    fn default() -> Self {
        Self::new().unwrap_or_else(|| Self {
            datasheet_dir: PathBuf::from(".sekai/hardware/datasheets"),
        })
    }
}
        ...
        match action.as_str() {
                    ...
                    Ok(path) => Ok(ToolResult {
                        ...
                        output: format!(
                            "Datasheet for '{device}' downloaded successfully.\n\
                             ...
                             ~/.sekai/hardware/devices/aardvark0.md with the key \
            ...
            "list" => {
                ...
                let output = if datasheets.is_empty() {
                    ...
                    format!(
                        "{} cached datasheet(s) in ~/.sekai/hardware/datasheets/:\n{}",
                        datasheets.len(),
                        datasheets
                            .iter()
                            .map(|n| format!("  - {n}"))
