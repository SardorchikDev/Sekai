...
//!
//! Wire protocol (Sekai serial JSON):
//! ```text
...
/// Default baud rate for Sekai serial devices.
...
/// Serial transport for Sekai hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify Sekai firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "sekai"`.
    ///
    /// Returns `true` if a Sekai device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "sekai" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
