...
impl Provider for OllamaProvider {
    ...
    fn supports_native_tools(&self) -> bool {
        ...
        // See: https://github.com/sekai-labs/sekai/issues/3999
        false
    }

    async fn chat(
        &self,
        request: sekai_api::provider::ChatRequest<'_>,
        model: &str,
        temperature: Option<f64>,
    ) -> anyhow::Result<ChatResponse> {
            ...
            && !specs.is_empty()
        {
            let tools: Vec<serde_json::Value> = specs
                ...
                .map(|s| {
                    let params =
                        sekai_api::schema::SchemaCleanr::clean_for_openai(s.parameters.clone());
                    serde_json::json!({
                        "type": "function",
                        "function": {
                            "name": s.name,
