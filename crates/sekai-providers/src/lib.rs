...
pub struct ProviderRuntimeOptions {
    ...
    /// These are merged from the config file and `SEKAI_EXTRA_HEADERS` env var.
...
pub fn provider_runtime_options_from_config(
    ...
    ProviderRuntimeOptions {
        ...
        provider_api_url: fallback
            ...
            .map(ToString::to_string),
        sekai_dir: config.config_path.parent().map(PathBuf::from),
...
/// 3. Generic fallback variables (`SEKAI_API_KEY`, `API_KEY`)
///
...
fn resolve_provider_credential(name: &str, credential_override: Option<&str>) -> Option<String> {
    ...
    for env_var in ["SEKAI_API_KEY", "API_KEY"] {
        if let Ok(value) = std::env::var(env_var) {
            let value = value.trim();
            if !value.is_empty() {
                return Some(value.to_string());
...
/// case.
pub fn default_provider_config(name: &str) -> sekai_config::schema::ModelProviderConfig {
    use sekai_config::schema::ModelProviderConfig;
...
fn create_provider_with_url_and_options(
    ...
    match name {
        ...
        "ollama" => {
            let env_url = std::env::var("SEKAI_PROVIDER_URL").ok();
        ...
        "gemini" | "google" | "google-gemini" => {
            let state_dir = options.sekai_dir.clone().unwrap_or_else(|| {
                directories::UserDirs::new().map_or_else(
                    || PathBuf::from(".sekai"),
                    |dirs| dirs.home_dir().join(".sekai"),
                )
            });
        ...
        _ => anyhow::bail!(
            "Unknown provider: {name}. Check README for supported providers or run `sekai onboard` to reconfigure.\n\
...
pub struct ProviderInfo {
    ...
    /// `sekai integration info <name>`. Empty string suppresses the
    /// description line.
...
/// Return the list of all known providers for display in `sekai providers list`.
///
...
mod tests {
    ...
    fn resolve_qwen_oauth_context_prefers_explicit_override() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/sekai-qwen-oauth-home-{}", std::process::id());
    ...
    fn resolve_qwen_oauth_context_uses_env_token_and_resource_url() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/sekai-qwen-oauth-home-{}-env", std::process::id());
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, Some("oauth-token"));
    ...
    fn resolve_qwen_oauth_context_reads_cached_credentials_file() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/sekai-qwen-oauth-home-{}-file", std::process::id());
    ...
    fn resolve_qwen_oauth_context_placeholder_does_not_use_dashscope_fallback() {
        ...
        let fake_home = format!(
            "/tmp/sekai-qwen-oauth-home-{}-placeholder",
            std::process::id()
        );
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, None);
    ...
    fn resolve_provider_credential_opencode_go_env() {
        ...
        let _sekai_guard = EnvGuard::set("SEKAI_API_KEY", None);
    ...
    fn provider_runtime_options_from_config_propagates_native_tools() {
        ...
        let mut groq = sekai_config::schema::ModelProviderConfig {
            native_tools: Some(true),
            ..Default::default()
        };
    ...
    fn provider_runtime_options_from_config_propagates_lmstudio_system_merge_without_base_url() {
        ...
        config.providers.models.insert(
            "lmstudio".to_string(),
            sekai_config::schema::ModelProviderConfig {
                merge_system_into_user: true,
                ..Default::default()
            },
        );
    ...
    fn resilient_provider_ignores_duplicate_and_invalid_fallbacks() {
        let reliability = sekai_config::schema::ReliabilityConfig {
            provider_retries: 1,
    ...
    fn resilient_provider_errors_for_invalid_primary() {
        let reliability = sekai_config::schema::ReliabilityConfig::default();
        let provider = create_resilient_provider(
            "totally-invalid",
            Some("provider-test-credential"),
            None,
    ...
    fn provider_runtime_options_extra_headers_passed_through() {
        ...
        extra_headers.insert("X-Title".to_string(), "sekai".to_string());
        let options = ProviderRuntimeOptions {
            extra_headers,
            ..ProviderRuntimeOptions::default()
        };
        assert_eq!(options.extra_headers.len(), 1);
        assert_eq!(options.extra_headers.get("X-Title").unwrap(), "sekai");
    }

    #[test]
    fn env_provider_url_overrides_api_url() {
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("SEKAI_PROVIDER_URL", "http://env-ollama:11434") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("SEKAI_PROVIDER_URL") };
    }
