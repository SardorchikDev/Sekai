...
use sekai_config::schema::{MatrixConfig, StreamMode, TranscriptionConfig};
...
mod client {
    ...
    use super::session;
    use sekai_config::schema::MatrixConfig;
        ...
        if recovery_attempts > 1 {
            bail!(
                "matrix: corruption recovery looped — aborting to avoid an infinite restart cycle. \
                 Wipe ~/.sekai/state/matrix/ manually and restart."
            );
        }
    ...
    async fn password_login(client: &Client, config: &MatrixConfig, password: &str) -> Result<()> {
        ...
        let mut login = client
            ...
            .initial_device_display_name("Sekai");
...
mod inbound {
    ...
    use sekai_config::schema::{MatrixConfig, TranscriptionConfig};
    ...
    async fn save_media_to_workspace(
        ...
        let Some(workspace) = workspace else {
            warn!(
                "matrix: cannot persist {} — channels.matrix workspace_dir not configured. Set SEKAI_DIR or run via the orchestrator.",
                info.file_name
            );
            return Ok(None);
        };
...
mod outbound {
    ...
    use sekai_api::{channel::SendMessage, media::MediaAttachment};
    ...
    /// streaming-cap pattern in `crates/sekai-tools/src/web_fetch.rs`.
...
mod tests {
    ...
    mod mention {
        ...
        fn body_fallback_display_name() {
            let bot = user_id!("@bot:example.org");
            assert!(is_mentioned(bot, Some("Sekai"), None, "hi sekai!"));
        }
    ...
    mod outbound_sandbox {
        ...
        fn rejects_dotdot_traversal() {
            ...
            let outside_dir = parent.join("sekai-test-outside");
    ...
    mod outbound_attachment_info {
        ...
        use sekai_api::media::MediaAttachment;
