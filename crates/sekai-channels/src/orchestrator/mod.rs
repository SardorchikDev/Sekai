...
//! Sekai to external platforms. Each channel implements the [`Channel`] trait
...
//! `sekai-channels/src/`, declare it in `lib.rs` behind the appropriate feature
...
pub use sekai_api::channel::{Channel, ChannelMessage, SendMessage};
// Local channel types (in misc, not sekai-channels)
...
pub use sekai_infra::debounce::MessageDebouncer;
pub use sekai_infra::session_backend::SessionBackend;
pub use sekai_infra::session_sqlite::SqliteSessionBackend;
pub use sekai_infra::stall_watchdog::StallWatchdog;
...
use sekai_api::session_keys::sanitize_session_key;
use sekai_config::schema::Config;
use sekai_memory::{self, MEMORY_CONTEXT_CLOSE, MEMORY_CONTEXT_OPEN, Memory};
use sekai_providers::reliable::{scope_provider_fallback, take_last_provider_fallback};
use sekai_providers::{self, ChatMessage, Provider};
use sekai_runtime::agent::loop_::{
...
use sekai_runtime::approval::ApprovalManager;
#[cfg(not(feature = "whatsapp-web"))]
use sekai_runtime::i18n;
use sekai_runtime::observability::traits::{ObserverEvent, ObserverMetric};
use sekai_runtime::observability::{self, Observer, runtime_trace};
use sekai_runtime::platform;
use sekai_runtime::security::{AutonomyLevel, SecurityPolicy};
use sekai_runtime::tools::{self, Tool};
use sekai_runtime::util::truncate_with_ellipsis;
...
// System prompt functions live in `sekai_runtime::agent::system_prompt`.
#[allow(unused_imports)]
pub use sekai_runtime::agent::system_prompt::{
...
struct ChannelRuntimeDefaults {
    ...
    reliability: sekai_config::schema::ReliabilityConfig,
}
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "sekai.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "sekai.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["sekai", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["sekai", "restart"];
...
struct ChannelCostTrackingState {
    tracker: Arc<sekai_runtime::cost::CostTracker>,
    prices: Arc<HashMap<String, sekai_config::schema::ModelPricing>>,
}
...
struct ChannelRuntimeContext {
    ...
    prompt_config: Arc<sekai_config::schema::Config>,
    ...
    api_key: Option<String>,
    api_url: Option<String>,
    reliability: Arc<sekai_config::schema::ReliabilityConfig>,
    provider_runtime_options: sekai_providers::ProviderRuntimeOptions,
    workspace_dir: Arc<PathBuf>,
    message_timeout_secs: u64,
    interrupt_on_new_message: InterruptOnNewMessageConfig,
    multimodal: sekai_config::schema::MultimodalConfig,
    media_pipeline: sekai_config::schema::MediaPipelineConfig,
    transcription_config: sekai_config::schema::TranscriptionConfig,
    hooks: Option<Arc<sekai_runtime::hooks::HookRunner>>,
    non_cli_excluded_tools: Arc<Vec<String>>,
    autonomy_level: AutonomyLevel,
    tool_call_dedup_exempt: Arc<Vec<String>>,
    model_routes: Arc<Vec<sekai_config::schema::ModelRouteConfig>>,
    query_classification: sekai_config::schema::QueryClassificationConfig,
    ack_reactions: bool,
    show_tool_calls: bool,
    session_store: Option<Arc<dyn sekai_infra::session_backend::SessionBackend>>,
    ...
    activated_tools:
        Option<std::sync::Arc<std::sync::Mutex<sekai_runtime::tools::ActivatedToolSet>>>,
    cost_tracking: Option<ChannelCostTrackingState>,
    pacing: sekai_config::schema::PacingConfig,
    max_tool_result_chars: usize,
    context_token_budget: usize,
    debouncer: Arc<sekai_infra::debounce::MessageDebouncer>,
    ...
    receipt_generator: Option<sekai_runtime::agent::tool_receipts::ReceiptGenerator>,
...
fn conversation_memory_key(msg: &sekai_api::channel::ChannelMessage) -> String {
...
pub fn conversation_history_key(msg: &sekai_api::channel::ChannelMessage) -> String {
...
fn followup_thread_id(msg: &sekai_api::channel::ChannelMessage) -> Option<String> {
...
fn interruption_scope_key(msg: &sekai_api::channel::ChannelMessage) -> String {
...
fn resolve_provider_alias(name: &str) -> Option<String> {
    ...
    let providers_list = sekai_providers::list_providers();
...
fn decrypt_optional_secret_for_runtime_reload(
    store: &sekai_runtime::security::SecretStore,
    value: &mut Option<String>,
    field_name: &str,
) -> Result<()> {
    if let Some(raw) = value.clone()
        && sekai_runtime::security::SecretStore::is_encrypted(&raw)
    {
        *value = Some(
            store
                .decrypt(&raw)
...
async fn load_runtime_defaults_from_config_file(path: &Path) -> Result<ChannelRuntimeDefaults> {
    ...
    if let Some(sekai_dir) = path.parent() {
        let store =
            sekai_runtime::security::SecretStore::new(sekai_dir, parsed.secrets.encrypt);
...
async fn maybe_apply_runtime_config_update(ctx: &ChannelRuntimeContext) -> Result<()> {
    ...
    let next_default_provider = sekai_providers::create_resilient_provider_with_options(
    ...
    if let Err(err) = next_default_provider.warmup().await {
        if sekai_providers::reliable::is_non_retryable(&err) {
            tracing::warn!(
...
fn refreshed_new_session_system_prompt(ctx: &ChannelRuntimeContext) -> String {
    let refreshed_skills = sekai_runtime::skills::skills_to_prompt_with_mode(
        &sekai_runtime::skills::load_skills_with_config(
...
fn should_rollback_failed_user_turn(error: &anyhow::Error) -> bool {
    if error
        .downcast_ref::<sekai_providers::ProviderCapabilityError>()
        .is_some_and(|capability| capability.capability.eq_ignore_ascii_case("vision"))
    {
        return true;
    }

    sekai_providers::reliable::is_non_retryable(error)
}

fn should_skip_memory_context_entry(key: &str, content: &str) -> bool {
    if sekai_memory::is_assistant_autosave_key(key) {
        return true;
    }
    ...
    if sekai_memory::is_user_autosave_key(key) {
        return true;
    }

    if sekai_memory::should_skip_autosave_content(content) {
        return true;
    }
...
async fn create_resilient_provider_nonblocking(
    ...
    tokio::task::spawn_blocking(move || {
        sekai_providers::create_resilient_provider_with_options(
            &provider_name,
            api_key.as_deref(),
            api_url.as_deref(),
            &reliability,
...
fn build_models_help_response(
    ...
    workspace_dir: &Path,
    model_routes: &[sekai_config::schema::ModelRouteConfig],
) -> String {
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `sekai models refresh --provider {}`.",
...
fn build_providers_help_response(current: &ChannelRouteSelection) -> String {
    ...
    for provider in sekai_providers::list_providers() {
...
fn build_config_text_response(
    ...
    resp.push_str("\nAvailable providers:\n");
    for p in sekai_providers::list_providers() {
...
fn build_config_block_kit(
    ...
    let provider_options: Vec<serde_json::Value> = sekai_providers::list_providers()
        .iter()
        .map(|p| {
            serde_json::json!({
    ...
    let mut provider_select = serde_json::json!({
        "type": "static_select",
        "action_id": "sekai_config_provider",
...
async fn handle_runtime_command_if_needed(
    ctx: &ChannelRuntimeContext,
    msg: &sekai_api::channel::ChannelMessage,
    ...
    let response = match command {
            ...
            match resolve_provider_alias(&raw_provider) {
                Some(provider_name) => {
                    match get_or_create_provider(ctx, &provider_name, None).await {
                        ...
                        Err(err) => {
                            let safe_err = sekai_providers::sanitize_api_error(&err.to_string());
                            format!(
        ...
        ChannelRuntimeCommand::ShowConfig => {
            if msg.channel == "slack" {
                ...
                format!("__SEKAI_BLOCK_KIT__{blocks_json}")
            } else {
...
fn append_recalled_memory_entries(
    entries: &mut Vec<sekai_memory::MemoryEntry>,
    seen_keys: &mut HashSet<String>,
    recalled: Result<Vec<sekai_memory::MemoryEntry>>,
) {
...
fn format_memory_context(
    entries: &[sekai_memory::MemoryEntry],
    min_relevance_score: f64,
) -> String {
...
async fn classify_channel_reply_intent(
    ...
    for msg in history.iter().filter(|m| m.role != "system") {
        ...
        let safe_content = sekai_providers::multimodal::strip_media_markers(&msg.content);
...
fn sanitize_channel_response(response: &str, tools: &[Box<dyn Tool>]) -> String {
    ...
    match sekai_runtime::security::LeakDetector::new().scan(&sanitized) {
        sekai_runtime::security::LeakResult::Clean => sanitized,
        sekai_runtime::security::LeakResult::Detected { patterns, redacted } => {
            tracing::warn!(
                patterns = ?patterns,
...
fn spawn_supervised_listener(
    ch: Arc<dyn Channel>,
    tx: tokio::sync::mpsc::Sender<sekai_api::channel::ChannelMessage>,
...
fn spawn_supervised_listener_with_health_interval(
    ...
    tokio::spawn(async move {
        ...
        loop {
            sekai_runtime::health::mark_component_ok(&component);
            ...
            match result {
                Ok(()) => {
                    ...
                    sekai_runtime::health::mark_component_error(
                        &component,
                ...
                Err(e) => {
                    ...
                    sekai_runtime::health::mark_component_error(&component, e.to_string());
                }
            }

            sekai_runtime::health::bump_component_restart(&component);
...
async fn process_channel_message(
    ...
    let mut msg = if let Some(hooks) = &ctx.hooks {
        match hooks.run_on_message_received(msg).await {
            sekai_runtime::hooks::HookResult::Cancel(reason) => {
                tracing::info!(%reason, "incoming message dropped by hook");
                return;
            }
            sekai_runtime::hooks::HookResult::Continue(modified) => modified,
        }
    } else {
        msg
    };
    ...
    if let Some(hint) =
        sekai_runtime::agent::classifier::classify(&ctx.query_classification, &msg.content)
    ...
    if ctx.auto_save_memory
        ...
        && !sekai_memory::should_skip_autosave_content(&msg.content)
    {
        ...
        let _ = ctx
            ...
            .store(
                &autosave_key,
                &msg.content,
                sekai_memory::MemoryCategory::Conversation,
                Some(&history_key),
            )
            .await;
    }
    ...
    if !active_provider.supports_vision() && prior_turns.len() > 1 {
        ...
        for turn in &mut prior_turns[..last_idx] {
            if turn.content.contains("[IMAGE:") {
                let (cleaned, _refs) =
                    sekai_providers::multimodal::parse_image_markers(&turn.content);
    ...
    // and preserving key decisions through LLM-driven summarization.
    {
        ...
        let compressor = sekai_runtime::agent::context_compressor::ContextCompressor::new(
            cc_config,
    ...
    let (delta_tx, delta_rx) = if use_draft_streaming {
        let (tx, rx) = tokio::sync::mpsc::channel::<sekai_runtime::agent::loop_::DraftEvent>(64);
        (Some(tx), Some(rx))
    } else {
        (None, None)
    };
    ...
    let draft_updater = if use_draft_streaming {
        ...
        if let (Some(mut rx), Some(draft_id_ref), Some(channel_ref)) = (
            ...
            Some(tokio::spawn(async move {
                use sekai_runtime::agent::loop_::StreamDelta;
    ...
    let cost_tracking_context = ctx.cost_tracking.clone().map(|state| {
        sekai_runtime::agent::loop_::ToolLoopCostTrackingContext::new(
            state.tracker,
            state.prices,
        )
    });
    ...
    let receipt_scope = ctx.receipt_generator.as_ref().map(|generator| {
        sekai_runtime::agent::tool_receipts::ReceiptScope {
            ...
            let loop_result = tokio::select! {
                ...
                result = tokio::time::timeout(
                    ...
                    scope_session_key(
                        ...
                        sekai_runtime::agent::loop_::TOOL_LOOP_COST_TRACKING_CONTEXT.scope(
                            cost_tracking_context.clone(),
                        sekai_runtime::agent::tool_receipts::TOOL_LOOP_RECEIPT_CONTEXT.scope(
                            receipt_scope.clone(),
                        run_tool_call_loop(
                        active_provider.as_ref(),
                        &mut history,
    ...
    match llm_result {
        ...
        LlmExecutionResult::Completed(Ok(Ok(response))) => {
            ...
            if let Some(hooks) = &ctx.hooks {
                    ...
                    .await
                {
                    ...
                    sekai_runtime::hooks::HookResult::Continue((
                        hook_channel,
                        hook_recipient,
            ...
            if ctx.auto_save_memory && msg.content.chars().count() >= AUTOSAVE_MIN_MESSAGE_CHARS {
                ...
                tokio::spawn(async move {
                    if let Err(e) = sekai_memory::consolidation::consolidate_turn(
                        provider.as_ref(),
                        &model,
                        memory.as_ref(),
                        &user_msg,
        ...
        LlmExecutionResult::Completed(Ok(Err(e))) => {
            if sekai_runtime::agent::loop_::is_tool_loop_cancelled(&e)
                || cancellation_token.is_cancelled()
            {
                ...
                if sekai_providers::reliable::is_auth_error(&e) {
                ...
                let safe_error = sekai_providers::sanitize_api_error(&e.to_string());
                runtime_trace::record_event(
                    "channel_message_error",
                    Some(msg.channel.as_str()),
                    Some(route.provider.as_str()),
...
async fn run_message_dispatch_loop(
    mut rx: tokio::sync::mpsc::Receiver<sekai_api::channel::ChannelMessage>,
    ctx: Arc<ChannelRuntimeContext>,
    ...
    while let Some(msg) = rx.recv().await {
        ...
        let msg = if msg.channel != "cli" && ctx.debouncer.enabled() {
            ...
            match ctx.debouncer.debounce(&debounce_key, &msg.content).await {
                sekai_infra::debounce::DebounceResult::Pending(rx) => {
                ...
                sekai_infra::debounce::DebounceResult::Passthrough(content) => {
                    let mut m = msg;
                    m.content = content;
                    m
                }
...
pub async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `sekai onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `sekai daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `sekai service stop && sekai service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        if !listed.contains("com.sekai.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.sekai.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.sekai.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/sekai");
...
fn build_channel_by_id(config: &Config, channel_id: &str) -> Result<Arc<dyn Channel>> {
    match channel_id {
        ...
        "matrix" => {
            #[cfg(feature = "channel-matrix")]
            {
                ...
                let state_dir = config
                    ...
                    .unwrap_or_else(|| std::path::PathBuf::from(".sekai/state/matrix"));
                Ok(Arc::new(
        ...
        "discord_history" | "discord-history" => {
            ...
            let discord_mem =
                sekai_memory::SqliteMemory::new_named(&config.workspace_dir, "discord")
...
fn collect_configured_channels(
    ...
    if let Some(ref dh) = config.channels.discord_history {
        if dh.enabled {
            match sekai_memory::SqliteMemory::new_named(&config.workspace_dir, "discord") {
                Ok(discord_mem) => {
                    channels.push(ConfiguredChannel {
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `sekai onboard` first.");
        return Ok(());
    }

    println!("🩺 Sekai Channel Doctor");
    println!();
    ...
    if config.channels.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `sekai gateway` then GET /health");
    }
...
pub async fn start_channels(
    config: Config,
    canvas_store: Option<sekai_runtime::tools::CanvasStore>,
) -> Result<()> {
    ...
    let provider_runtime_options =
        sekai_providers::provider_runtime_options_from_config(&config);
    ...
    let temperature = config
        ...
        .and_then(|e| e.temperature)
        .unwrap_or(0.7);
    let mem: Arc<dyn Memory> = Arc::from(sekai_memory::create_memory_with_storage_and_routes(
        &config.memory,
    ...
    let mut ch_activated_handle: Option<
        std::sync::Arc<std::sync::Mutex<sekai_runtime::tools::ActivatedToolSet>>,
    > = None;
    if config.mcp.enabled && !config.mcp.servers.is_empty() {
        ...
        match sekai_runtime::tools::McpRegistry::connect_all(&config.mcp.servers).await {
            Ok(registry) => {
                let registry = std::sync::Arc::new(registry);
                if config.mcp.deferred_loading {
                    let deferred_set = sekai_runtime::tools::DeferredMcpToolSet::from_registry(
                    ...
                    deferred_section =
                        sekai_runtime::tools::build_deferred_tools_section(&deferred_set);
                    let activated = std::sync::Arc::new(std::sync::Mutex::new(
                        sekai_runtime::tools::ActivatedToolSet::new(),
                    ));
                    ch_activated_handle = Some(std::sync::Arc::clone(&activated));
                    built_tools.push(Box::new(sekai_runtime::tools::ToolSearchTool::new(
                        deferred_set,
                        activated,
                    )));
                } else {
                    ...
                    for name in names {
                        if let Some(def) = registry.get_tool_def(&name).await {
                            let wrapper: std::sync::Arc<dyn Tool> =
                                std::sync::Arc::new(sekai_runtime::tools::McpToolWrapper::new(
                                    name,
                                    def,
                                    std::sync::Arc::clone(&registry),
                                ));
                            ...
                            built_tools
                                .push(Box::new(sekai_runtime::tools::ArcToolRef(wrapper)));
                            registered += 1;
                        }
                    }
                    tracing::info!(
    ...
    let skills = sekai_runtime::skills::load_skills_with_config(&workspace, &config);
    ...
    sekai_runtime::tools::register_skill_tools(&mut built_tools, &skills, security.clone());
    ...
    let i18n_locale = config
        ...
        .filter(|s| !s.is_empty())
        .map(ToString::to_string)
        .unwrap_or_else(sekai_runtime::i18n::detect_locale);
    sekai_runtime::i18n::init(&i18n_locale);
    ...
    if matches!(
        ...
        sekai_config::schema::SkillsPromptInjectionMode::Compact
    ) {
        tool_descs.push((
            "read_skill",
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `sekai onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 Sekai Channel Server");
    println!("  🤖 Model:    {model}");
    let effective_backend = sekai_memory::effective_memory_backend_name(
    ...
    sekai_runtime::health::mark_component_ok("channels");
    ...
    let (tx, rx) = tokio::sync::mpsc::channel::<sekai_api::channel::ChannelMessage>(100);
    ...
    let runtime_ctx = Arc::new(ChannelRuntimeContext {
        ...
        hooks: if config.hooks.enabled {
            let mut runner = sekai_runtime::hooks::HookRunner::new();
            if config.hooks.builtin.command_logger {
                runner.register(Box::new(
                    sekai_runtime::hooks::builtin::CommandLoggerHook::new(),
                ));
            }
            if config.hooks.builtin.webhook_audit.enabled {
                runner.register(Box::new(
                    sekai_runtime::hooks::builtin::WebhookAuditHook::new(
        ...
        session_store: if config.channels.session_persistence {
            match sekai_infra::make_session_backend(
                &config.workspace_dir,
        ...
        cost_tracking: sekai_runtime::cost::CostTracker::get_or_init_global(
            config.cost.clone(),
            &config.workspace_dir,
        )
        ...
        debouncer: Arc::new(sekai_infra::debounce::MessageDebouncer::new(
            Duration::from_millis(config.channels.debounce_ms),
        )),
        receipt_generator: if config.agent.tool_receipts.enabled {
            Some(sekai_runtime::agent::tool_receipts::ReceiptGenerator::new())
        } else {
            None
        },
    ...
    if let Some(ref store) = runtime_ctx.session_store {
        ...
        for key in session_keys {
            ...
            sekai_runtime::agent::history_pruner::remove_orphaned_tool_messages(&mut msgs);
            hydrated += 1;
...
pub async fn deliver_announcement(
    config: &sekai_config::schema::Config,
    channel: &str,
    target: &str,
    thread_id: Option<String>,
    output: &str,
) -> anyhow::Result<()> {
    use sekai_api::channel::SendMessage;

    // Scan for credential leaks before delivering
    let leak_detector = sekai_runtime::security::LeakDetector::new();
    let safe_output = match leak_detector.scan(output) {
        sekai_runtime::security::LeakResult::Detected { redacted, .. } => redacted,
        sekai_runtime::security::LeakResult::Clean => output.to_string(),
    };
    ...
    match channel.to_ascii_lowercase().as_str() {
        ...
        "telegram" => {
            ...
            sekai_api::channel::Channel::send(&ch, &make_msg(&safe_output)).await?;
        }
        "discord" => {
            let dc = config
                .channels
...
mod tests {
    ...
    use sekai_memory::{Memory, MemoryCategory, SqliteMemory};
    use sekai_providers::{ChatMessage, Provider};
    use sekai_runtime::agent::loop_::build_tool_instructions;
    use sekai_runtime::observability::NoopObserver;
    use sekai_runtime::tools::{Tool, ToolResult};
    ...
    fn pacing_config_defaults_preserve_existing_behavior() {
        let pacing = sekai_config::schema::PacingConfig::default();
...
<tool_result name="http_request">{"status":200}</tool_result>"#;
    ...
    fn compact_sender_history_keeps_recent_truncated_messages() {
        ...
        let ctx = ChannelRuntimeContext {
            ...
            api_key: None,
            api_url: None,
            reliability: Arc::new(sekai_config::schema::ReliabilityConfig::default()),
            interrupt_on_new_message: InterruptOnNewMessageConfig {
                telegram: false,
                slack: false,
                discord: false,
            ...
            multimodal: sekai_config::schema::MultimodalConfig::default(),
            media_pipeline: sekai_config::schema::MediaPipelineConfig::default(),
            transcription_config: sekai_config::schema::TranscriptionConfig::default(),
            hooks: None,
            provider_runtime_options: sekai_providers::ProviderRuntimeOptions::default(),
            workspace_dir: Arc::new(std::env::temp_dir()),
            prompt_config: Arc::new(sekai_config::schema::Config::default()),
            message_timeout_secs: CHANNEL_MESSAGE_TIMEOUT_SECS,
            non_cli_excluded_tools: Arc::new(Vec::new()),
            autonomy_level: AutonomyLevel::default(),
            tool_call_dedup_exempt: Arc::new(Vec::new()),
            model_routes: Arc::new(Vec::new()),
            query_classification: sekai_config::schema::QueryClassificationConfig::default(),
            ack_reactions: true,
            show_tool_calls: true,
            session_store: None,
            approval_manager: Arc::new(ApprovalManager::for_non_interactive(
                &sekai_config::schema::AutonomyConfig::default(),
            )),
            activated_tools: None,
            cost_tracking: None,
            pacing: sekai_config::schema::PacingConfig::default(),
    ...
    fn rollback_orphan_user_turn_also_removes_from_session_store() {
        ...
        let store: Arc<dyn sekai_infra::session_backend::SessionBackend> =
            Arc::new(sekai_infra::session_store::SessionStore::new(tmp.path()).unwrap());
    ...
    impl Channel for TelegramRecordingChannel {
        ...
        async fn listen(
            &self,
            _tx: tokio::sync::mpsc::Sender<sekai_api::channel::ChannelMessage>,
        ) -> anyhow::Result<()> {
            Ok(())
        }
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_executes_tool_calls_instead_of_sending_raw_json() {
        ...
        process_channel_message(
            runtime_ctx,
            sekai_api::channel::ChannelMessage {
                id: "msg-1".to_string(),
    ...
    async fn process_channel_message_scopes_sender_session_key_for_sessions_current_tool() {
        ...
        let tmp = TempDir::new().unwrap();
        let session_store: Arc<dyn sekai_infra::session_backend::SessionBackend> =
            Arc::new(sekai_infra::session_store::SessionStore::new(tmp.path()).unwrap());
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            tools_registry: Arc::new(vec![Box::new(
                sekai_runtime::tools::SessionsCurrentTool::new(Arc::clone(&session_store)),
            )]),
            observer: Arc::new(NoopObserver),
            system_prompt: Arc::new("test-system-prompt".to_string()),
            model: Arc::new("test-model".to_string()),
            ...
            approval_manager: Arc::new(ApprovalManager::for_non_interactive(&{
                let mut autonomy = sekai_config::schema::AutonomyConfig::default();
    ...
    async fn process_channel_message_renders_trailing_tool_receipts_block_when_enabled() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            approval_manager: Arc::new(ApprovalManager::for_non_interactive(&{
                let mut autonomy = sekai_config::schema::AutonomyConfig::default();
                autonomy.level = sekai_config::autonomy::AutonomyLevel::Full;
            ...
            receipt_generator: Some(
                sekai_runtime::agent::tool_receipts::ReceiptGenerator::new(),
            ),
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
            ...
            store.insert(
                ...
                RuntimeConfigState {
                    defaults: ChannelRuntimeDefaults {
                        ...
                        api_key: None,
                        api_url: None,
                        reliability: sekai_config::schema::ReliabilityConfig::default(),
                    },
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(sekai_config::schema::ReliabilityConfig::default()),
            provider_runtime_options: sekai_providers::ProviderRuntimeOptions {
                sekai_dir: Some(temp.path().to_path_buf()),
                ..sekai_providers::ProviderRuntimeOptions::default()
            },
            workspace_dir: Arc::new(std::env::temp_dir()),
            prompt_config: Arc::new(sekai_config::schema::Config::default()),
            message_timeout_secs: CHANNEL_MESSAGE_TIMEOUT_SECS,
    ...
    async fn process_channel_message_respects_configured_max_tool_iterations_above_default() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            pacing: sekai_config::schema::PacingConfig {
                ...
                ..sekai_config::schema::PacingConfig::default()
            },
            max_tool_result_chars: 0,
            context_token_budget: 0,
            debouncer: Arc::new(sekai_infra::debounce::MessageDebouncer::new(
    ...
    impl Memory for NoopMemory {
        ...
        async fn store(
            ...
            _key: &str,
            _content: &str,
            _category: sekai_memory::MemoryCategory,
        ...
        async fn recall(
            ...
            _since: Option<&str>,
            _until: Option<&str>,
        ) -> anyhow::Result<Vec<sekai_memory::MemoryEntry>> {
            Ok(Vec::new())
        }

        async fn get(&self, _key: &str) -> anyhow::Result<Option<sekai_memory::MemoryEntry>> {
            Ok(None)
        }

        async fn list(
            &self,
            _category: Option<&sekai_memory::MemoryCategory>,
            _session_id: Option<&str>,
        ) -> anyhow::Result<Vec<sekai_memory::MemoryEntry>> {
            Ok(Vec::new())
        }
    ...
    impl Memory for RecallMemory {
        ...
        async fn recall(
            ...
            Ok(vec![sekai_memory::MemoryEntry {
                id: "entry-1".to_string(),
                key: "memory_key_1".to_string(),
                content: "Age is 45".to_string(),
                category: sekai_memory::MemoryCategory::Conversation,
    ...
    async fn message_dispatch_processes_messages_in_parallel() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<sekai_api::channel::ChannelMessage>(4);
        tx.send(sekai_api::channel::ChannelMessage {
            id: "1".to_string(),
            sender: "alice".to_string(),
    ...
    async fn message_dispatch_interrupts_in_flight_telegram_request_and_preserves_context() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<sekai_api::channel::ChannelMessage>(8);
    ...
    async fn process_channel_message_precheck_uses_configured_model_override() {
        ...
        let mut config = sekai_config::schema::Config::default();
    ...
    fn prompt_skills_include_instructions_and_tools() {
        let ws = make_workspace();
        let skills = vec![sekai_runtime::skills::Skill {
            name: "code-review".into(),
            description: "Review code for bugs".into(),
            version: "1.0.0".into(),
            author: None,
            tags: vec![],
            tools: vec![sekai_runtime::skills::SkillTool {
                name: "lint".into(),
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from Sekai 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";

        // Reproduces the production crash path where channel logs truncate at 80 chars.
        let result =
            std::panic::catch_unwind(|| sekai_runtime::util::truncate_with_ellipsis(msg, 80));
        assert!(
            result.is_ok(),
    ...
    fn full_autonomy_prompt_executes_allowed_tools_without_extra_approval() {
        let ws = make_workspace();
        let config = sekai_config::schema::AutonomyConfig {
            level: sekai_runtime::security::AutonomyLevel::Full,
            ..sekai_config::schema::AutonomyConfig::default()
        };
        let prompt = build_system_prompt_with_mode_and_autonomy(
            ...
            Some(&config),
            false,
            sekai_config::schema::SkillsPromptInjectionMode::Full,
            false,
            0,
        );
    ...
    fn readonly_prompt_explains_policy_blocks_without_fake_approval() {
        let ws = make_workspace();
        let config = sekai_config::schema::AutonomyConfig {
            level: sekai_runtime::security::AutonomyLevel::ReadOnly,
            ..sekai_config::schema::AutonomyConfig::default()
        };
        let prompt = build_system_prompt_with_mode_and_autonomy(
            ws.path(),
    ...
    fn channel_notify_observer_truncates_utf8_arguments_safely() {
        ...
        observer.record_event(
            &sekai_runtime::observability::traits::ObserverEvent::ToolCallStart {
    ...
    fn conversation_memory_key_uses_message_id() {
        let msg = sekai_api::channel::ChannelMessage {
            id: "msg_abc123".into(),
            sender: "U123".into(),
    ...
    fn matrix_root_conversation_history_key_omits_event_id() {
        let first = sekai_api::channel::ChannelMessage {
        ...
        let second = sekai_api::channel::ChannelMessage {
    ...
    fn conversation_memory_key_is_unique_per_message() {
        let msg1 = sekai_api::channel::ChannelMessage {
            id: "msg_1".into(),
            sender: "U123".into(),
            reply_target: "C456".into(),
            content: "first".into(),
        ...
        let msg2 = sekai_api::channel::ChannelMessage {
            id: "msg_2".into(),
            sender: "U123".into(),
            reply_target: "C456".into(),
            content: "second".into(),
    ...
    async fn autosaved_group_conversation_memory_stays_session_scoped() {
        let tmp = TempDir::new().unwrap();
        let mem = SqliteMemory::new(tmp.path()).unwrap();
        let group_a_msg = sekai_api::channel::ChannelMessage {
            id: "msg_1".into(),
            sender: "U123".into(),
        ...
        let group_b_msg = sekai_api::channel::ChannelMessage {
            id: "msg_2".into(),
            sender: "U123".into(),
    ...
    async fn process_channel_message_refreshes_available_skills_after_new_session() {
        ...
        let initial_skills =
            sekai_runtime::skills::load_skills_with_config(workspace.path(), &config);
...
This is an example JSON object for profile settings."#;
    ...
    fn aieos_identity_from_file() {
        use tempfile::TempDir;
        use sekai_config::schema::IdentityConfig;
    ...
    fn collect_configured_channels_includes_mattermost_when_configured() {
        ...
        config.channels.mattermost = Some(sekai_config::schema::MattermostConfig {
            enabled: true,
    ...
    fn collect_configured_channels_skips_disabled_email() {
        let mut config = Config::default();
        config.channels.email = Some(sekai_config::scattered_types::EmailConfig {
            enabled: false,
            ..Default::default()
        });
    ...
    fn collect_configured_channels_skips_disabled_voice_call() {
        let mut config = Config::default();
        config.channels.voice_call = Some(sekai_config::scattered_types::VoiceCallConfig {
            enabled: false,
            ..Default::default()
        });
    ...
    async fn supervised_listener_marks_error_and_restarts_on_failures() {
        ...
        let (tx, rx) = tokio::sync::mpsc::channel::<sekai_api::channel::ChannelMessage>(1);
        ...
        let snapshot = sekai_runtime::health::snapshot_json();
    ...
    async fn supervised_listener_refreshes_health_while_running() {
        ...
        let first_last_ok = sekai_runtime::health::snapshot_json()["components"]
        ...
        let second_last_ok = sekai_runtime::health::snapshot_json()["components"]
            [&component_name]["last_ok"]
            .as_str()
            .unwrap_or("")
            .to_string();
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "sekai.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "sekai.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["sekai", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["sekai", "restart"]);
    }
    ...
    async fn e2e_failed_vision_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .peek("test-channel_chat-photo_sekai_user")
    ...
    async fn e2e_failed_non_retryable_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .peek("test-channel_chat-format_sekai_user")
            .expect("history should exist for sender");
        assert_eq!(turns.len(), 2);
        assert_eq!(turns[0].role, "user");
        assert_eq!(turns[0].content, "What is WAL?");
    ...
    async fn process_channel_message_applies_query_classification_route() {
        ...
        let classification_config = sekai_config::schema::QueryClassificationConfig {
            enabled: true,
            rules: vec![sekai_config::schema::ClassificationRule {
                hint: "vision".into(),
        ...
        let model_routes = vec![sekai_config::schema::ModelRouteConfig {
            hint: "vision".into(),
    ...
    async fn process_channel_message_classification_priority_selects_highest() {
        ...
        let classification_config = sekai_config::schema::QueryClassificationConfig {
            enabled: true,
            rules: vec![
                sekai_config::schema::ClassificationRule {
                    hint: "fast".into(),
        ...
        let model_routes = vec![
            sekai_config::schema::ModelRouteConfig {
                hint: "fast".into(),
    ...
    fn build_channel_by_id_configured_telegram_succeeds() {
        let mut config = Config::default();
        config.channels.telegram = Some(sekai_config::schema::TelegramConfig {
            enabled: true,
            bot_token: "test-token".to_string(),
            allowed_users: vec![],
            stream_mode: sekai_config::schema::StreamMode::Off,
    ...
    fn build_channel_by_id_configured_voice_call_succeeds() {
        ...
        config.channels.voice_call = Some(sekai_config::scattered_types::VoiceCallConfig {
            enabled: true,
            provider: sekai_config::scattered_types::VoiceProvider::Twilio,
    ...
    fn is_stop_command_matches_with_bot_suffix() {
        assert!(is_stop_command("/stop@sekai_bot"));
    }
    ...
    fn default_keep_tool_context_turns_is_two() {
        let config = sekai_config::schema::AgentConfig::default();
