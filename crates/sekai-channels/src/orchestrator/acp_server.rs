...
use sekai_runtime::agent::agent::{Agent, TurnEvent};
...
fn map_tool_kind(name: &str) -> &'static str {
    match name {
        ...
        // Sekai tools return user-visible text, so use `other` to keep the
...
mod tests {
    ...
    fn initialize_response_uses_acp_v1_shape() {
        ...
        assert_eq!(result["agentInfo"]["name"], "sekai-acp");
        assert_eq!(result["agentInfo"]["title"], "Sekai ACP");
    ...
    fn handle_initialize_default_model_absent_when_unconfigured() {
        ...
        assert!(
            result["_meta"]["sekai"].get("defaultModel").is_none(),
            "defaultModel must be absent when no provider is configured, got: {}",
            result["_meta"]["sekai"]["defaultModel"]
        );
    }
    ...
    fn handle_initialize_default_model_reflects_configured_provider() {
        ...
        let result = server.handle_initialize(&serde_json::json!({})).unwrap();
        assert_eq!(result["_meta"]["sekai"]["defaultModel"], "llama3.2");
    }
    ...
    async fn session_new_does_not_wait_for_configured_mcp_servers() {
        ...
        let config = Config {
            ...
            mcp: sekai_config::schema::McpConfig {
                enabled: true,
                servers: vec![sekai_config::schema::McpServerConfig {
                    name: "slow".to_string(),
                    transport: sekai_config::schema::McpTransport::Stdio,
