...
//! from the iLink developer portal, not from Sekai config. Users who
...
impl WeChatChannel {
    pub fn new(
        ...
        let state_dir = state_dir.unwrap_or_else(|| {
            directories::UserDirs::new()
                .map(|u| u.home_dir().join(".sekai").join("wechat"))
                .unwrap_or_else(|| PathBuf::from(".sekai/wechat"))
        });
    ...
    async fn send_message_items(
        ...
        let client_id = format!("sekai-{}", uuid::Uuid::new_v4());
        let body = serde_json::json!({
            "msg": {
                "from_user_id": "",
                "to_user_id": to,
