...
impl QQChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        sekai_config::schema::build_channel_proxy_client("channel.qq", self.proxy_url.as_deref())
    }
...
allowed_users = ["user1"]
"#;
        let config: sekai_config::schema::QQConfig = toml::from_str(toml_str).unwrap();
