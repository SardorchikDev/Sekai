...
use tokio::fs;
use sekai_api::channel::{Channel, ChannelMessage, SendMessage};
use sekai_config::schema::{Config, StreamMode};
...
pub struct TelegramChannel {
    ...
    api_base: String,
    transcription: Option<sekai_config::schema::TranscriptionConfig>,
    transcription_manager: Option<std::sync::Arc<super::transcription::TranscriptionManager>>,
    voice_transcriptions: Mutex<std::collections::HashMap<String, String>>,
    workspace_dir: Option<std::path::PathBuf>,
    ack_reactions: bool,
    tts_config: Option<sekai_config::schema::TtsConfig>,
    ...
    pending_approvals: Arc<
        tokio::sync::Mutex<
            std::collections::HashMap<
                String,
                tokio::sync::oneshot::Sender<sekai_api::channel::ChannelApprovalResponse>,
            >,
        >,
    >,
...
impl TelegramChannel {
    ...
    pub fn with_transcription(
        mut self,
        config: sekai_config::schema::TranscriptionConfig,
    ) -> Self {
        if !config.enabled {
            return self;
        }
    ...
    pub fn with_tts(mut self, config: sekai_config::schema::TtsConfig) -> Self {
        if config.enabled {
    ...
    fn http_client(&self) -> reqwest::Client {
        sekai_config::schema::build_channel_proxy_client(
            "channel.telegram",
    ...
    async fn load_config_without_env() -> anyhow::Result<Config> {
        ...
        config.workspace_dir = sekai_dir.join("workspace");
        Ok(config)
    }
    ...
    async fn persist_allowed_identity(&self, identity: &str) -> anyhow::Result<()> {
        ...
        let Some(telegram) = config.channels.telegram.as_mut() else {
            anyhow::bail!(
                ...
                or run `sekai onboard --channels-only` to configure interactively"
            );
        };
    ...
    async fn register_bot_commands(&self) {
        ...
        if let Some(ref workspace_dir) = self.workspace_dir {
            let skills = sekai_runtime::skills::load_skills(workspace_dir);
    ...
    async fn synthesize_and_send_voice(
        ...
        text: &str,
        tts_config: &sekai_config::schema::TtsConfig,
    ) -> anyhow::Result<()> {
        ...
        let client = sekai_config::schema::build_runtime_proxy_client("channel.telegram");
                                ...
                                Ok(()) => {
                                    let _ = self
                                        .send(&SendMessage::new(
                                            "✅ Telegram account bound successfully. You can talk to Sekai now.",
                                            &chat_id,
                                        ))
                                        .await;
                                    tracing::info!(
...
Ensure only one `sekai` process is using this bot token."
                    );
                    ...
                    if let Some(cb) = update.get("callback_query") {
                            ...
                            && let Some((approval_id, action)) = rest.rsplit_once(':')
                        {
                            let response = match action {
                                ...
                                "always" => Some(
                                    sekai_api::channel::ChannelApprovalResponse::AlwaysApprove,
                                ),
                                "deny" => {
    ...
    async fn request_approval(
        &self,
        recipient: &str,
        request: &sekai_api::channel::ChannelApprovalRequest,
    ) -> anyhow::Result<Option<sekai_api::channel::ChannelApprovalResponse>> {
        use sekai_api::channel::ChannelApprovalResponse;
...
mod tests {
    ...
    fn telegram_extract_bind_code_supports_bot_mention() {
        assert_eq!(
            TelegramChannel::extract_bind_code("/bind@sekai_bot 654321"),
            Some("654321")
        );
    }
    ...
    fn with_transcription_sets_config_when_enabled() {
        let tc = sekai_config::schema::TranscriptionConfig {
            enabled: true,
            api_key: Some("test_key".to_string()),
            ..sekai_config::schema::TranscriptionConfig::default()
        };
    ...
    fn with_transcription_skips_when_disabled() {
        let tc = sekai_config::schema::TranscriptionConfig::default(); // enabled = false
        let ch =
            TelegramChannel::new("token".into(), vec!["*".into()], false).with_transcription(tc);
    ...
    async fn e2e_live_voice_transcription_and_reply_cache() {
        ...
        let config = sekai_config::schema::TranscriptionConfig {
            enabled: true,
            ..Default::default()
        };
    ...
    fn markdown_attachment_not_detected_by_multimodal_image_markers() {
        ...
        let messages = vec![sekai_providers::ChatMessage::user(content)];
        assert_eq!(
            sekai_providers::multimodal::count_image_markers(&messages),
            0,
    ...
    fn photo_image_marker_detected_by_multimodal() {
        ...
        let messages = vec![sekai_providers::ChatMessage::user(
            photo_content.to_string(),
        )];
        let count = sekai_providers::multimodal::count_image_markers(&messages);
        assert_eq!(
            count, 1,
    ...
    fn e2e_attachment_saves_file_and_formats_content() {
        ...
        let doc_msgs = vec![sekai_providers::ChatMessage::user(doc_content)];
        assert_eq!(
            sekai_providers::multimodal::count_image_markers(&doc_msgs),
            0,
        ...
        let photo_msgs = vec![sekai_providers::ChatMessage::user(photo_content.clone())];
        assert_eq!(
            sekai_providers::multimodal::count_image_markers(&photo_msgs),
            1,
        ...
        let cap_msgs = vec![sekai_providers::ChatMessage::user(captioned.clone())];
        assert_eq!(
            sekai_providers::multimodal::count_image_markers(&cap_msgs),
            1,
        ...
        let md_msgs = vec![sekai_providers::ChatMessage::user(md_content)];
        assert_eq!(
            sekai_providers::multimodal::count_image_markers(&md_msgs),
            0,
            "markdown file must not trigger image marker detection"
        );
    }
    ...
    fn groq_provider_rejects_photo_with_vision_error() {
        ...
        use sekai_providers::compatible::{AuthStyle, OpenAiCompatibleProvider};
        ...
        let marker_count = sekai_providers::multimodal::count_image_markers(&messages);
