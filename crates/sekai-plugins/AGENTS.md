...
WASM plugin host for Sekai. Handles plugin discovery, manifest parsing,
Ed25519 signature verification, and Extism-based WASM execution. Bridges
plugin-exported functions into Sekai's `Tool` and `Channel` traits so
plugins appear as native capabilities to the agent runtime.

## What this crate is allowed to depend on

- `sekai-api` (traits only — `Tool`, `Channel`, `ToolResult`)
- `extism` (WASM runtime)
...
- Concrete tool or channel implementations (those go in `sekai-tools` or
  `sekai-channels`, or in a WASM plugin)
- Plugin business logic (that belongs in the plugin's own crate)
- Config schema definitions (those go in `sekai-config`)
