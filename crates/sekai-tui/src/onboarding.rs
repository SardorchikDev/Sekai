...
use sekai_config::traits::{Answer, OnboardUi, SelectItem};
...
fn render_breadcrumb(
    ...
    let mut spans: Vec<Span<'_>> = vec![Span::styled("Sekai Onboard", theme::heading_style())];
...
impl OnboardUi for RatatuiUi {
    ...
    async fn editor(&mut self, hint: &str, initial: &str) -> Result<Answer<String>> {
        ...
        self.suspend()?;
        let path =
            std::env::temp_dir().join(format!("sekai-onboard-{}.txt", std::process::id()));
