...
Download the latest Sekai release, unzip, and run:
...
3. Installs to `%USERPROFILE%\.cargo\bin\sekai.exe`
4. Runs `sekai onboard` automatically
...
del "%USERPROFILE%\.cargo\bin\sekai.exe"

:: Scoop
scoop uninstall sekai
```

Remove config and workspace (optional — this deletes conversation history):

```cmd
rmdir /s /q "%USERPROFILE%\.sekai"
rmdir /s /q "%LOCALAPPDATA%\Sekai"
```
...
- **Task Scheduler stop-at-idle.** By default Windows may terminate scheduled tasks on idle / battery. The installed task explicitly disables these conditions; verify under Task Scheduler → Sekai → Properties → Conditions.
