...
When a provider experiences a transient error (timeout, connection failure, auth issue), Sekai automatically attempts fallback providers in the order specified.

**Example**: If your primary provider is `openai` but it's temporarily unavailable, Sekai can automatically fall back to `anthropic`, then `groq`.
...
When the primary provider recovers, Sekai resumes using it (no sticky failover).
...
If `claude-opus-4-7` fails or is unavailable, Sekai tries the fallback models in order while staying within the same provider (unless a provider-level fallback is also configured).

### API Key Rotation

For providers that frequently encounter rate limits, you can supply additional API keys that Sekai will rotate through on `429` responses.
...
**Behavior**: If OpenAI times out or returns an error, Sekai will retry twice with exponential backoff, then attempt the same request using Anthropic.
...
sekai doctor traces --contains "fallback"

# Show provider retry details
sekai doctor traces --contains "provider"

# Show rate-limit rotation
sekai doctor traces --contains "429"
```
...
3. Generic fallback: `SEKAI_API_KEY`, then `API_KEY`
