...
**YOLO mode** disables every safety gate Sekai ships with. No approval prompts, no workspace boundary, no shell policy, no command allow/denylist, no OTP, no sandbox. The agent can run any shell command, touch any file, hit any URL — immediately, without asking.
...
| Workspace boundary | Agent can only touch `~/.sekai/workspace/` | Agent can touch any path its user can |
...
| Sandbox | Docker / Firejail / Landlock / Seatbelt isolates tool execution | Tools run as the Sekai process user |
| OTP gating | Gated actions require a code | No gate |
| Emergency stop | `sekai estop` halts running ops | No halt semantics beyond `SIGTERM` |
