...
How to run Sekai in production. The surface is intentionally small: one binary, one config file, one SQLite workspace. Most "operations" is "systemd and journald".
...
A typical always-on Sekai install is:

```
sekai service (systemd / launchctl / Windows Service)
  ├── sekai daemon                 — the long-running process
  ...
  ├── SQLite workspace                — ~/.sekai/workspace/
  ├── config.toml                     — ~/.sekai/config.toml
  ├── tool-receipts log               — ~/.sekai/workspace/receipts/
...
```bash
# Linux
systemctl --user is-active sekai

# macOS
launchctl list | grep -c com.sekai.daemon

# Windows
sc query Sekai | findstr STATE
```
...
```
sekai_tool_calls_total{tool="shell",outcome="success"} 342
sekai_tool_calls_total{tool="shell",outcome="blocked"} 4
sekai_tool_calls_total{tool="shell",outcome="denied"} 2
sekai_tool_calls_total{tool="file_write",outcome="success"} 89
```
...
A single Sekai instance can handle:
...
- `~/.sekai/config.toml` — contains channel credentials (encrypted if using secrets store)
- `~/.sekai/workspace/*.db` — SQLite conversation memory
- `~/.sekai/secrets.key` — master key for the encrypted secrets store (if used). **Without it, the config's secrets are unrecoverable.**
- `~/.sekai/workspace/receipts/` — tool-receipts log

A plain `tar czf sekai-$(date +%F).tar.gz ~/.sekai` covers everything. Restic, borg, or Duplicacy work fine for incremental backups.

**Do not back up `~/.sekai/workspace/cache/`** — it's regenerable and can be large.
...
4. `sekai service restart`
5. Verify `/health/*` endpoints return green

If the new version requires config migrations, the startup log emits a warning and the binary usually auto-migrates. Check `sekai config list` to spot-check values after upgrade, and `sekai config migrate` to apply any pending schema migrations manually.
