...
Deploying Sekai so it can receive inbound traffic: gateway exposure, webhook channels, tunnels, and LAN-only vs. public-facing configurations. Raspberry Pis and other home-network hosts are first-class targets here.

## When inbound ports matter

| Mode | Needs inbound port | Notes |
|---|:---:|---|
| Telegram (long-poll) | No | Sekai polls `api.telegram.org` — works behind NAT |
...
```bash
curl -fsSL https://raw.githubusercontent.com/sekai-labs/sekai/master/install.sh | bash -s -- --prebuilt
```
...
- [ ] Install the service: `sekai service install && sekai service start`
...
- `/etc/init.d/sekai` — init script
- `/etc/sekai/` — config directory
- `/var/log/sekai/` — log files

Enable and start:

```bash
sudo rc-update add sekai default
sudo rc-service sekai start
sudo rc-service sekai status
```

Logs:

```bash
sudo tail -f /var/log/sekai/error.log
```

### OpenRC notes

- Service runs as `sekai:sekai` (least privilege)
- Config path is fixed: `/etc/sekai/config.toml`
...
1. `ps aux | grep sekai` and confirm only one daemon is running
2. Check you don't have `cargo run --bin sekai -- channel start telegram` from a dev session hanging around
