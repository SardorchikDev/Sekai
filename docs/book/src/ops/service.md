...
The stock unit (`~/.config/systemd/user/sekai.service`) uses:
...
```bash
sekai service stop     # free the gateway port if the service is running
sekai daemon
```

`sekai daemon` runs in the foreground, logs to stderr, and is the same process the service runs — just without the service harness. Useful when:
...
```bash
systemctl --user edit sekai.service
```
...
systemctl --user restart sekai
```

### macOS — launchd

Edit `~/Library/LaunchAgents/com.sekai.daemon.plist`:
...
Each Sekai instance owns one workspace. To run two:

1. Install the binary once
2. Create `~/.sekai-home/` and `~/.sekai-work/` (or wherever)
3. Run two services pointing at different workspaces:

```bash
SEKAI_WORKSPACE=~/.sekai-home sekai service install --name sekai-home
SEKAI_WORKSPACE=~/.sekai-work sekai service install --name sekai-work
```
...
```bash
# Linux
journalctl --user -u sekai --since "1 day ago" | grep -E 'Started|Stopped|failed'

# macOS
log show --predicate 'process == "sekai"' --last 1d | grep -E 'start|stop|error'
```

If you're seeing repeated restarts, enable debug logging (`RUST_LOG=sekai=debug` via the unit file's `Environment=`) and let one more crash happen to capture the full trace.
