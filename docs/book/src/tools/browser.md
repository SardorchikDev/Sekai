...
This guide covers setting up browser automation capabilities in Sekai, including both headless automation and GUI access via VNC.

## Overview

Sekai supports multiple browser access methods:
...
### 2. Verify Sekai Config

The browser tool is enabled by default with `allowed_domains = ["*"]`. Restrict domains or disable it via `sekai config set`:

```bash
sekai config set browser.allowed-domains '["example.com", "docs.example.com"]'
sekai config set browser.enabled false
```
...
```bash
echo "Open https://example.com and tell me what it says" | sekai agent
```
...
### Sekai Integration Tests

```bash
# Content extraction
echo "Open https://example.com and summarize it" | sekai agent

# Navigation
echo "Go to https://github.com/trending and list the top 3 repos" | sekai agent

# Form interaction
echo "Go to Wikipedia, search for 'Rust programming language', and summarize" | sekai agent
```
