...
Sekai picks the best available backend at startup:
...
- **Forbidden paths** — `~/.ssh`, `~/.aws`, `~/.config` (except Sekai's own), anything in `[autonomy] forbidden_paths`
...
- CPU: soft-limited to the Sekai service's share
...
Firejail's default profile is fairly permissive; we apply a custom profile bundled with Sekai.

### Docker

Works anywhere Docker does. Runs each tool invocation in an ephemeral container. The default image is `alpine:latest` (matches `DockerSandbox::default()` in `crates/sekai-runtime/src/security/docker.rs`); set `image` to any other public image you have access to, or build your own.
...
```bash
docker build -t sekai-sandbox:local dev/sandbox/
```

```toml
[security.sandbox]
backend = "docker"
image = "sekai-sandbox:local"
```
...
No sandboxing. Tools run with the full privileges of the Sekai service user. This is what YOLO mode enables. Loud, obvious, intentional.
...
- **"Sandbox backend unavailable"** on startup — check `sekai service status` and the journal; the auto-detect logs which backends it tried.
...
- Detection: `crates/sekai-runtime/src/security/detect.rs`
- Backends: `crates/sekai-runtime/src/security/sandbox/` (one file per backend)
- Config: `[security.sandbox]` block in `crates/sekai-config/src/schema.rs`
