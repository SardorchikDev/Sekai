...
Three ways to add a provider Sekai doesn't ship with:
...
Sekai ships dedicated provider kinds for three popular local-inference stacks.
...
```bash
sekai models refresh --provider <name>   # list models the endpoint advertises
sekai agent -m "hello"                    # smoke-test with a one-shot message
```
...
The trait lives in `crates/sekai-api/src/provider.rs`:
...
1. Add a file to `crates/sekai-providers/src/myprovider.rs`
...
- Endpoints behind a VPN or proxy? Confirm routing from the Sekai host
...
- [Catalog](./catalog.md) — every Sekai-provided provider
