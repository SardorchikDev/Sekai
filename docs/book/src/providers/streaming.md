...
Every provider in Sekai that speaks a streaming API streams token-by-token. The runtime forwards those streams to channel adapters that support partial updates (Discord, Slack, Telegram, the gateway's WebSocket), so the user sees text appear as the model generates it.
...
- `crates/sekai-api/src/provider.rs` — `Provider` trait, `StreamEvent` enum
- `crates/sekai-providers/src/compatible.rs` — OpenAI-compat SSE parser
- `crates/sekai-providers/src/anthropic.rs` — Anthropic streaming
- `crates/sekai-providers/src/ollama.rs` — Ollama streaming
- `crates/sekai-channels/src/orchestrator/mod.rs` — channel-side stream consumption
