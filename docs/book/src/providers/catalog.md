...
Every provider Sekai ships with. For each: what it talks to, config shape, and notes.
...
kind = "copilot"
model = "gpt-4o"
# authenticate once via `sekai onboard` — the wizard handles the OAuth flow
# and stores a token at ~/.config/sekai/copilot.json
```
