...
Every model provider is declared under `[providers.models.<name>]` in `~/.sekai/config.toml`. The `<name>` is your own alias — it's how you reference the provider elsewhere in config (`default_model = "claude"`, `fallback_providers = ["claude", "local"]`, etc.).
...
|---|---|---|
| `anthropic` | `crates/sekai-providers/src/anthropic.rs` | Accepts OAuth tokens (`sk-ant-oat*`) or API keys |
...
- **GitHub Copilot** — authenticate via the onboarding wizard (`sekai onboard`) which handles the OAuth flow. The token is stored in the secrets backend.
...
```bash
SEKAI_OLLAMA_BASE_URL=http://ollama:11434 sekai agent
```
