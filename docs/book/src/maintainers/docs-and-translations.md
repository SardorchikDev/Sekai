...
Sekai has two independent translation layers:
...
They are filled separately and stored separately. Both use a provider-agnostic fill pipeline: configure any OpenAI-compatible endpoint in `~/.sekai/config.toml` under `[providers.models.<name>]` and pass `--provider <name>` to the fill commands.
...
Ollama is the current canonical source for docs. Ensure you have [Ollama](https://ollama.com/) installed and have `qwen3.6:35-a3b` pulled. Then, in `~/.sekai/config.toml` (or your established config home):
...
App strings live in `crates/sekai-runtime/locales/`. English is the source of truth and is embedded at compile time. Non-English locales are loaded from `~/.sekai/workspace/locales/` at runtime.
...
```bash
mkdir -p ~/.sekai/workspace/locales/ja
cp crates/sekai-runtime/locales/ja/cli.ftl ~/.sekai/workspace/locales/ja/cli.ftl
```
