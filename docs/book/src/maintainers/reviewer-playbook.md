...
| `risk: medium` | `crates/sekai-providers/`, `crates/sekai-channels/`, `crates/sekai-memory/`, `crates/sekai-config/` | 1 subsystem-aware reviewer + behavior verification | Focused scenario proof, explicit side effects |
| `risk: high` | `crates/sekai-runtime/src/security/`, the rest of `crates/sekai-runtime/`, `crates/sekai-gateway/`, `crates/sekai-tools/`, `.github/workflows/` | Fast triage + deep review + rollback readiness | Security and failure-mode checks, rollback clarity |
