...
- **lint** — `cargo fmt --check`, `cargo clippy --workspace --exclude sekai-desktop --all-targets --features ci-all -- -D warnings`
...
- **test** — `cargo nextest run --locked --workspace --exclude sekai-desktop` on Linux
...
```bash
gh api repos/sekai-labs/sekai/actions/permissions
gh api repos/sekai-labs/sekai/actions/permissions/selected-actions
```
