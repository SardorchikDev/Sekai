...
| `sekai` | Operating the running Sekai instance (CLI + gateway API) |
...
Sekai uses squash-merge for all PRs. The `squash-merge` skill produces both the purple **Merged** badge *and* a conventional-commits formatted squash message with full commit history in the body.
