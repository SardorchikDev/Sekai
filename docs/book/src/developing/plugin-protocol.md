...
relates-to:
  - ADR-003
  - crates/sekai-plugins
---

# Plugin Protocol

This document defines the protocol between Sekai's plugin host and WASM
plugin modules.
...
Plugins are discovered from `~/.sekai/plugins/` (configurable via
...
Host functions are provided by the Sekai runtime and callable from within
...
sekai plugin install /path/to/my-plugin/

# Or manually
cp -r my-plugin/ ~/.sekai/plugins/my-plugin/
```
