...
`cargo web gen-api` renders the OpenAPI spec in-process from `sekai_gateway::openapi::build_spec()`, writes it to `target/openapi.json`, and feeds that file to `openapi-typescript`. The same `build_spec()` serves `/api/openapi.json` at runtime, so the spec on disk is never the source of truth — it is a transient handoff between Rust and the TS codegen.
...
1. Change a gateway handler or schema in `crates/sekai-gateway/`.
