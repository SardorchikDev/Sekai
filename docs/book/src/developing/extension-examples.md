...
Sekai's architecture is trait-driven and modular.
...
> **Source of truth**: the trait definitions live in `crates/sekai-api/src/`.
> If an example here conflicts with the trait file, the trait file wins.

---

## Tool (`crates/sekai-api/src/tool.rs`)
...
Register your tool in `crates/sekai-tools/src/lib.rs` via `default_tools()`.

```rust
// In your crate: use sekai::tools::traits::{Tool, ToolResult};
...
## Channel (`crates/sekai-api/src/channel.rs`)

Channels let Sekai communicate through any messaging platform.
...
Register your channel in `crates/sekai-channels/src/lib.rs` and add config to `ChannelsConfig` in `crates/sekai-config/src/schema.rs`.

```rust
// In your crate: use sekai::channels::traits::{Channel, ChannelMessage, SendMessage};
...
## Provider (`crates/sekai-api/src/provider.rs`)

Providers are LLM backend adapters. Each provider connects Sekai to a different model API.
...
Register your provider in `crates/sekai-providers/src/lib.rs`.

```rust
// In your crate: use sekai::providers::traits::Provider;
...
## Memory (`crates/sekai-api/src/memory_traits.rs`)
...
Register your backend in `crates/sekai-memory/src/lib.rs`.

```rust
// In your crate: use sekai::memory::traits::{Memory, MemoryEntry, MemoryCategory};
...
1. Create your implementation file in the relevant `crates/sekai-*/src/` directory.
2. Register it in the module's factory function (e.g., `default_tools()`, provider match arm).
3. Add any needed config keys to `crates/sekai-config/src/schema.rs`.
...
- **Dependency direction goes inward to contracts.** Concrete integrations depend on `sekai-api` traits, `sekai-config` schema, and `sekai-infra` utilities — not on each other. Provider code does not import channel internals; tool code does not mutate gateway policy directly.
- **Module responsibilities stay single-purpose.** Orchestration in `sekai-runtime/src/agent/`, transport in `sekai-channels/`, model I/O in `sekai-providers/`, policy in `sekai-runtime/src/security/`, execution in `sekai-tools/`.
