...
- `memory/` — wraps `sekai-memory` with runtime-level caching and consolidation schedules
...
- `traits.rs` — re-exports from `sekai-api` plus provider-internal helpers
...
All channels implement the `Channel` trait from `sekai-api`. Each is feature-gated — a minimal build includes only the channels you compile in.
...
Callable tools the agent invokes. Not to be confused with CLI `sekai` subcommands.
