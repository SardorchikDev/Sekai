...
- **Bot account.** Create via **Main Menu → Integrations → Bot Accounts**. Username like `sekai-bot`. Enable `post:all` and `channel:read` (or whichever scopes you want). Save the **access token**.
...
- **TLS**: terminate at your reverse proxy; Sekai makes plain HTTPS requests
- **Webhook vs. WebSocket**: the integration uses WebSocket for inbound real-time events and REST for outbound posts; only the Sekai → Mattermost direction matters for network configuration
