...
- **Bot account** in Talk settings — give it a display name (e.g. `sekai-bot`) and generate a bot webhook URL
...
bot_name = "sekai-bot"                      # the display name of the bot in Talk
...
Environment override: `SEKAI_NEXTCLOUD_TALK_WEBHOOK_SECRET` takes precedence over the config value. Useful for rotating secrets without editing the config.
...
3. Confirm Sekai receives and replies in the same room
