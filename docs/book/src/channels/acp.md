...
**ACP** is a JSON-RPC 2.0 protocol over stdio that lets editors and IDEs drive a running Sekai agent as a session host. Newline-delimited JSON — lightweight, streamable, easy to wire to a subprocess.

Think of it as "LSP for agents": the editor launches `sekai acp`, sends prompts over stdin, and receives session updates on stdout.
...
All messages are JSON-RPC 2.0 (newline-delimited). Sekai implements **protocol version 1**.
...
`_meta.sekai` carries Sekai-specific extension fields not in the base ACP spec. Clients that only implement the base spec can ignore this object.
...
`stopReason` is `"end_turn"` on normal completion. The ACP completion signal is `stopReason`; Sekai also includes the current final `content` string for existing clients.

### `session/update` notifications (agent → client)

Sekai sends four kinds of `session/update` notification during a prompt turn. The discriminant is the `sessionUpdate` field inside `update`:
...
The `name` field on `tool_call_update` is a Sekai extension (not required by the base ACP spec). Clients can use it for display; it's safe to ignore.

### `session/request_permission` (agent → client, outbound request)

When a tool requires user approval (via `always_ask` in the autonomy config, or the `ask_user`/`escalate_to_human` tools), Sekai issues a **JSON-RPC request** from agent to client. The client must reply with a result before the tool call proceeds.
...
### `session/cancel` _(Sekai extension)_

Abort an in-flight `session/prompt` turn. This method is a Sekai extension,
not part of the base ACP spec. If ACP later standardizes a conflicting
`session/cancel`, Sekai will move its extension to `_meta/session/cancel`.
...
### `session/stop` _(Sekai extension)_

Cleanly end a session. Not in the base ACP spec — Sekai-specific. If a future ACP spec revision adds `session/stop` with different semantics, this will be renamed `_meta/session/stop`.
...
### `session/update` (client → server) _(Sekai extension)_

Sekai also accepts inbound `session/update` (and the legacy `session/event` alias) notifications from the client for custom event injection. Not in the base ACP spec — Sekai-specific. If the ACP spec later defines an inbound `session/update` with different semantics, this will be renamed `_meta/session/update`.
...
When running `sekai acp` as a subprocess, the command starts the server unconditionally. When running as a daemon, the gateway exposes ACP over WebSocket at `/acp` with no additional config required.
...
- ACP server: `crates/sekai-channels/src/orchestrator/acp_server.rs`
- ACP back-channel: `crates/sekai-channels/src/acp_channel.rs`
- Per-session path enforcement: `crates/sekai-config/src/policy.rs` (`SecurityPolicy::from_config`), `crates/sekai-runtime/src/agent/agent.rs` (`from_config_with_session_cwd_and_mcp`)
- OS-level sandbox detection/backends: `crates/sekai-runtime/src/security/detect.rs`, `landlock.rs`, `bubblewrap.rs`, `seatbelt.rs`
