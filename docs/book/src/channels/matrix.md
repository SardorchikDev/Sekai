...
Run Sekai in Matrix rooms, including end-to-end encrypted (E2EE) rooms.
...
3. `room_id` is correct:
   ...
   - supported: room alias (`#alias:server`) — Sekai resolves it
...
All config management goes through `sekai config` or `sekai onboard`. Do not hand-edit `~/.sekai/config.toml`.

Easiest: run the wizard and let it prompt for every Matrix field:

```bash
sekai onboard --channels-only
```

Or set individual fields after onboarding:

```bash
sekai config set channels.matrix.homeserver https://matrix.example.com
sekai config set channels.matrix.room-id '!room:matrix.example.com'
sekai config set channels.matrix.access-token           # prompts, input masked
sekai config set channels.matrix.user-id @bot:matrix.example.com
sekai config set channels.matrix.device-id ABCDEF1234
sekai config set channels.matrix.allowed-users '["*"]'   # open for testing
sekai config set channels.matrix.ack-reactions true       # default: true (👀 → ✅)
sekai config set channels.matrix.reply-in-thread true     # default: true
```
...
- Sekai attempts to read identity from Matrix `/_matrix/client/v3/account/whoami`.
...
Brand-new bot accounts need a Matrix access token before Sekai can connect. Element doesn't expose the token directly, so the canonical path is a one-shot password-login API call that returns both the access token and a stable device ID together.
...
Run this once. Replace `your.homeserver`, the bot username, password, and pick any short `device_id` string (alphanumeric, no spaces — this is the *server-side* device label that Sekai will reuse on every restart):
...
### Step 2 — Apply both values to Sekai

```bash
sekai config set channels.matrix.access-token    # paste the access_token (input is masked)
sekai config set channels.matrix.device-id NEWDEVICE
sekai config set channels.matrix.user-id @bot:example.com
sekai service restart
```

The wizard (`sekai onboard --channels-only`) prompts for these same fields if you'd rather work through it interactively.

### Notes

- **Keep a copy of the token** when you first paste it. Secrets are encrypted at rest and `sekai config get` will print `[masked]` for the token field; you can't retrieve it later. Stash it in a scratch note if you'll need it for the curl validation snippets in §5C.
...
- **Rotating the access token later** without re-running the wizard:
  ```bash
  sekai config set channels.matrix.access-token    # prompts, masked
  sekai service restart
  ```
...
```bash
sekai onboard --channels-only
sekai service restart        # or `sekai daemon` to run foreground
```

Send a plain-text message in the configured Matrix room. Confirm:

- Sekai logs show the Matrix listener starting with no repeated sync/auth errors.
...
- For diagnosis, temporarily open it:
  ```bash
  sekai config set channels.matrix.allowed-users '["*"]'
  sekai service restart
  ```
...
> **About `$MATRIX_TOKEN` in the snippets below.** Secrets in Sekai are encrypted at rest and intentionally **not** retrievable via `sekai config get` — it prints `[masked]` for any secret field. You have two options:
>
...
> 2. **Keep a copy** of the token when you first paste it into `sekai onboard` or `sekai config set channels.matrix.access-token`. A one-time side-effect — write it to a scratch note if you want to run these curl checks later.
>
> The non-secret fields *are* retrievable:
>
> ```bash
> MATRIX_HOMESERVER=$(sekai config get channels.matrix.homeserver)
> MATRIX_USER=$(sekai config get channels.matrix.user-id)
> ```
...
Sekai suppresses `matrix_sdk`, `matrix_sdk_base`, and `matrix_sdk_crypto` to `warn` by default — they're noisy at `info`. Restore SDK output for debugging:

```bash
RUST_LOG=info,matrix_sdk=info,matrix_sdk_base=info,matrix_sdk_crypto=info sekai daemon
```

### F. Message formatting (Markdown)

- Sekai sends Matrix replies as markdown-capable `m.room.message` text content.
- Matrix clients that support `formatted_body` render emphasis, lists, and code blocks.
- If formatting appears as plain text: check client capability first, then confirm Sekai is running a build with markdown-enabled Matrix output.
...
Sekai needs a stable `device_id` for E2EE session restore. Without it, a new device is registered every restart, breaking key sharing and device verification.
...
3. Delete the local crypto store:
   ...
   ```bash
   rm -rf ~/.sekai/state/matrix/
   ```

4. Apply the new credentials:

   ```bash
   sekai config set channels.matrix.access-token <new_token>
   sekai config set channels.matrix.device-id <new_device_id>
   ```
...
A recovery key lets Sekai automatically restore room keys and cross-signing secrets from server-side backup. Device resets, crypto-store deletions, and fresh installs all recover automatically — no emoji verification, no manual key sharing.
...
#### Step 2 — Add the recovery key to Sekai

Either path works. The onboarding wizard is easier for fresh installs; `sekai config set` is preferred for existing installs.
...
```bash
sekai config set channels.matrix.recovery-key    # input masked
sekai service restart
```
...
From now on, even if the local crypto store is deleted, Sekai recovers automatically on next startup.
...
```bash
RUST_LOG=sekai::channels::matrix=debug sekai daemon
```
...
```bash
RUST_LOG=sekai::channels::matrix=debug,matrix_sdk_crypto=debug sekai daemon
```
...
- **Persistent sessions:** on first successful login, Sekai writes `~/.sekai/state/matrix/session.json` (user_id + device_id + access_token + optional refresh_token). Subsequent restarts call `restore_session()` from that blob — no re-login. The matrix-rust-sdk SQLite crypto store lives alongside it at `~/.sekai/state/matrix/store/`. **Once `session.json` exists, rotating `access-token` in config has no effect until the file is deleted** — the saved token wins. Delete `session.json` to force a re-login from config values.
- **Cross-signing:** when `recovery-key` matches what is sealed in your account's server-side secret storage, Sekai runs `recovery().recover(key)` on every startup, the SDK imports your existing master / self-signing / user-signing keys, and the freshly registered device is automatically signed. **No bootstrap, no UIA, no key rotation.** If your account doesn't yet have cross-signing set up, generate the recovery key in Element (Settings → Security & Privacy → Secure Backup) before configuring `recovery-key`.
- **Cron delivery:** `delivery.to` should be a plain room id (`!abc:server`) or alias (`#room:server`). Older configs that wrote `<sender>||<room>` are tolerated — Sekai extracts the last `!`/`#`-prefixed segment and warns about the malformed value.

## 8. Auto-recovery from corrupted local state

The matrix-rust-sdk default SQLite store is single-device and assumes the local view stays in sync with the homeserver. Two failure modes break that assumption irrecoverably; Sekai detects each at startup and (when `password` + `user-id` are both configured) auto-wipes `~/.sekai/state/matrix/` and re-authenticates so a fresh device is created server-side.
...
If `password` + `user-id` aren't configured, auto-recovery can't run — the channel bails with an actionable error pointing at the two choices: configure them, or `rm -rf ~/.sekai/state/matrix/` manually.
