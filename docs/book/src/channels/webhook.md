...
3. **Public bind** — set `SEKAI_ALLOW_PUBLIC_BIND=1` and `[gateway] host = "0.0.0.0"`. Not recommended without a firewall in front.
...
- Channel: `crates/sekai-channels/src/webhook.rs` (plus gateway ingress in `crates/sekai-gateway/`)
- Templates: `crates/sekai-runtime/src/templating.rs`
