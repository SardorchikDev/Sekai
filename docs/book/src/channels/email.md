...
port = 993
username = "you@example.com"
oauth_token = "..."              # managed via `sekai channel auth email`
```
...
client_secret_json = "~/.sekai/gmail-client-secret.json"
pubsub_topic = "projects/my-project/topics/gmail-inbox"
pubsub_subscription = "projects/my-project/subscriptions/sekai-sub"
...
3. Create a pull subscription on that topic for Sekai
4. Create OAuth client credentials (desktop app type), download JSON
5. On first run, `sekai channel auth gmail-push` opens a browser for the OAuth consent
