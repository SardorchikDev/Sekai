...
be set with `sekai config get/set/list/init/migrate` is also reachable via
...
| `POST` | `/api/config/init?section=...` | Instantiate `None` nested sections with defaults. Mirrors `sekai config init`. |
| `POST` | `/api/config/migrate` | Apply on-disk schema migration in place. Mirrors `sekai config migrate`. |
...
The CLI counterpart is `sekai config patch <file-or-stdin>`, which applies
...
live calls. The CLI shortcut for the URL is `sekai config docs`.
