...
Sekai routes MQTT/webhook/cron/peripheral events through a unified SOP dispatcher (`dispatch_sop_event`).
...
Configure broker access with `sekai config set channels.mqtt.<field> <value>` — the keys land under `[channels.mqtt]` in the stored config. See the [Config reference](../reference/config.md) for all fields. The `use_tls` flag must match the scheme of `broker_url` (`mqtts://` ⇒ `true`, `mqtt://` ⇒ `false`).
...
| **Cron not firing** | daemon not running or invalid expression | run `sekai daemon`; check logs for cron parse warnings |
