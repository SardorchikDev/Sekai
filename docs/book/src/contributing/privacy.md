...
Sekai artifacts are public — git history, releases, fixtures, snapshots, the docs book, every rendered locale. Anything you commit ships with the project forever. Treat privacy as a merge gate, not best-effort.
...
| Use case | Examples |
|---|---|
| Actor labels | `sekai_user`, `sekai_operator`, `sekai_maintainer`, `test_user`, `user_a`, `project_bot` |
| Service / runtime labels | `sekai_bot`, `sekai_service`, `sekai_runtime`, `sekai_node` |
| Environment labels | `sekai_project`, `sekai_workspace`, `sekai_channel` |
| Hostnames | `example.com`, `host.invalid`, `192.0.2.x` (RFC 5737 documentation range) |
| Email addresses | `user@example.com`, `bot@sekai.invalid` |
...
If a test or doc genuinely needs a role-shaped identity, use Sekai-scoped roles only: `SekaiAgent`, `SekaiOperator`, `SekaiMaintainer`. Don't borrow real names, even pseudonyms — pseudonyms drift back into being real over time.
