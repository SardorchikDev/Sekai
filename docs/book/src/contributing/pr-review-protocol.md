...
This is the procedure followed when reviewing a pull request in `sekai-labs/sekai`. It's loaded by the `github-pr-review-session` skill and read by human reviewers — it's authoritative for both.
...
2. **Top-level conversation**
   ...
   ```bash
   gh pr view <number> --comments --repo sekai-labs/sekai
   ```

3. **Inline threads (every reply chain)**

   ```bash
   gh api repos/sekai-labs/sekai/pulls/<number>/comments --paginate
   ```
...
4. **Formal reviews**
   ...
   ```bash
   gh api repos/sekai-labs/sekai/pulls/<number>/reviews --paginate
   ```
...
6. **Diff**
   ...
   ```bash
   gh pr diff <number> --repo sekai-labs/sekai
   ```
