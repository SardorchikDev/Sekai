...
- Trait-first — define the trait in `sekai-api`, then implement in the right edge crate
...
- Integration tests in `tests/` and crate-local unit tests — run via `cargo nextest run --locked --workspace --exclude sekai-desktop`
...
| Area | Where to start |
|---|---|
| New channel | `crates/sekai-channels/` — copy an existing channel of similar shape |
| New provider | `crates/sekai-providers/` — `compatible.rs` covers most OpenAI-like ones |
...
| Hardware | `crates/sekai-hardware/` — new board support, new sensor drivers |
