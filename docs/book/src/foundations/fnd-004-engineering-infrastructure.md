...
> Discussion thread and full revision history: [#5579](https://github.com/sekai-labs/sekai/issues/5579)
...
```
PR changes: crates/sekai-tool-call-parser/src/lib.rs

Affected crates:
  sekai-tool-call-parser     ← directly changed
  sekai-misc                 ← depends on it
  sekailabs (root)           ← depends on it

Not affected:
  sekai-channels             ← no dependency path
  sekai-memory               ← no dependency path
  sekai-providers            ← no dependency path
```
...
1. **Classify the advisory**: Is the affected crate a direct dependency or transitive? Does Sekai call the vulnerable code path? Is there a fixed version available?
...
- Crates with `version.workspace = true` are bumped together; independently-versioned crates (`sekai-api`, hardware library crates) are handled separately per the versioning policy
...
For Sekai's current scale and team size, **SLSA Level 2** is the appropriate target:
...
The categories that matter for Sekai's changelog:
...
**Why this phase:** By v0.8.0 the workspace will have grown further. Running the full pipeline on every PR will be increasingly expensive. Contributors to `sekai-tool-call-parser` should not wait 30 minutes for a gateway rebuild.
...
- A PR touching only `sekai-tool-call-parser` runs tests for that crate and its dependents, not the full workspace
...
**Why this phase:** Phase 3 of the architecture RFC extracts `sekai-gw` as a separate binary. The first multi-artifact release happens here. The release pipeline must be ready before it is needed.

#### Deliverables

**D1: Introduce `release-plz` and remove `version-sync.yml`**

Configure `release-plz` for the workspace. Workspace application crates use `version.workspace = true`. `sekai-api` and hardware library crates are configured with independent release settings. The `version-sync.yml` workflow is retired.
