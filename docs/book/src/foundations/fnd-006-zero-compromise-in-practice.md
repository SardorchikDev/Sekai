...
> Discussion thread and full revision history: [#5653](https://github.com/sekai-labs/sekai/issues/5653)
...
>
> This is the sixth document in Sekai's maturity framework. The five before it
...
This RFC is the sixth in a set of documents that together form Sekai's maturity
...
This distinction matters especially in this project's context. Sekai is operated in
...
|--------|-------|-------------------|
| `sekai-config/src/schema.rs` | 16,800 lines | Now the largest file in the codebase; the original `loop_.rs` was called out at 9,500 lines in the architecture RFC — this surpasses it |
| `sekai-channels/src/orchestrator/mod.rs` | 11,813 lines | Second-largest file; a single module carrying concentrated responsibility |
| `sekai-runtime/src/onboard/wizard.rs` | 7,988 lines | A single workflow in a single file |
| `sekai-runtime/src/agent/loop_.rs` | 6,101 lines | Reduced from ~9,500 in the monolith — real, measurable progress; still large |
| `sekai-channels/src/orchestrator/telegram.rs` | 5,122 lines | One channel implementation; one file |
...
| Public functions in `sekai-api` | 371 | The entire foundational API surface — every other crate depends on this |
| Doc comment lines in `sekai-api` | ~27 | Roughly 14:1 ratio of undocumented public API — see §4.2 |
...
- Whether the public functions in `sekai-api` can be correctly implemented by someone
...
The trait layer in `sekai-api` is the right architecture. `Provider`, `Channel`,
...
start Sekai because of a misconfiguration should leave the process with a clear
...
The `sekai-api` situation is specific enough to name directly. This is the one crate
...
At minimum, every public item in `sekai-api` should carry:
...
isolation. `sekai-tool-call-parser` should be testable with a `&str` input and no
runtime. `sekai-config` should be testable by constructing config structs directly.
Trait implementations in `sekai-api` should be testable against fake implementations
...
**Blast radius.** Debt in `sekai-api` — the foundation everything else depends on —
...
size, type, or content of data you did not produce. The Sekai security model defines
...
development, but in the hands of users running Sekai on hardware you will never see,
