# FND-001: Intentional Architecture — Sekai Microkernel Transition
### Starting v0.7.0 · Type: Architecture · Rev. 3

> **Canonical reference** · Ratified by the team · Rev. 3
> Discussion thread and full revision history: [#5574](https://github.com/sekai-labs/sekai/issues/5574)
...
2. [The Vision — What Sekai Is](#2-the-vision--what-sekai-is)
...
Sekai was bootstrapped by AI tools working from OpenClaw's TypeScript codebase. AI code generation works at the **Implementation** layer. It writes functions, structs, and modules that do things. It does not set Vision. It does not make Architecture decisions. It does not define Design contracts.
...
> **Sekai is a personal AI assistant runtime that any person can run on any hardware — from a $10 embedded board to a cloud server — with zero configuration overhead, zero external service requirements, and zero compromise on capability or security.**
...
**Zero external requirements.** A user who downloads Sekai and has an LLM provider configured should have a working, useful AI assistant without installing anything else. Channels, dashboards, and integrations are things you add when you want them — not things you need before it works.

**Zero compromise.** Lean does not mean weak. Sekai must have a serious security model, real observability, and genuine extensibility. The tension between "small binary" and "full capability" is resolved through composition: a small core, extended by components you choose.

**For every skill level.** A student on a $10 Raspberry Pi and a team running a production deployment should both feel like Sekai was designed for them. This means the default experience must be simple, and the advanced experience must be powerful — not two different products.

**User-owned.** Your data, your hardware, your configuration. Sekai does not require an account, does not phone home, and does not lock you into a platform.
...
The entire Sekai codebase currently lives in a single Rust crate. This means:
...
We are not rewriting Sekai. We are giving its existing good ideas a structure they can grow in.
...
| OS Microkernel Concept | Sekai Equivalent |
|---|---|
...
| Init / runtime system | **Agent runtime layer** — Orchestration loop, security policy enforcement, plugin host, core tools, IPC API. The `sekai-runtime` crate, gated by the `agent-runtime` feature. This is what makes Sekai an *agent*, not just a library. |
...
```
    sekai-api          ← defines all traits (Provider, Channel, Tool, ...)
         ▲                  no implementations, no heavy dependencies
         │ depends on
  foundation crates       ← sekai-config, sekai-providers, sekai-memory,
         ▲                  sekai-infra, sekai-tool-call-parser
         │ depends on        all depend on sekai-api; no cross-dependencies
    sekai-runtime      ← implements the agent loop (agent-runtime feature)
         ▲                  depends on sekai-api + foundation crates
         │ depends on        knows nothing about specific channels or tools
  plugin crates           ← sekai-channel-discord, sekai-tools-web, ...
         ▲                  depend on sekai-api (not the runtime)
         │ depends on
  sekai binary         ← thin wiring layer
                             reads config, registers plugins, starts runtime
```

If `sekai-runtime` ever imports `TelegramChannel`, the architecture has been violated. The compiler will enforce this once crate boundaries are drawn.
...
│                                                                     │
│   ┌──────────────────────────────────────────────────────────────┐  │
│   │              sekai-runtime  (agent-runtime feature)       │  │
│   │                                                              │  │
...
│   │  │                                                      │   │  │
│   │  │  sekai-api · sekai-config · sekai-infra     │   │  │
│   │  │  sekai-providers · sekai-memory                │   │  │
│   │  │  sekai-tool-call-parser                           │   │  │
│   │  │                                                      │   │  │
│   │  │  Vision target: <5 MB RAM at runtime                 │   │  │
│   │  └──────────────────────────────────────────────────────┘   │  │
│   └──────────────────────────────────────────────────────────────┘  │
│                              ▲                                      │
│                   sekai-api (traits only)                        │
│                              ▲                                      │
│   ┌──────────────┐  ┌────────┴────────┐  ┌─────────────────────┐   │
│   │  sekai-gw │  │  Channel plugins│  │   Tool plugins      │   │
│   │  (opt-in     │  │                 │  │                     │   │
...
│          ▼                                                          │
│   ┌─────────────────┐                                               │
│   │ sekai-desktop│   ← Tauri app (already exists in apps/tauri)  │
│   │ System tray app │     bundles sekai-gw as a sidecar          │
│   │ Native GUI      │                                               │
│   └─────────────────┘                                               │
└─────────────────────────────────────────────────────────────────────┘
```
...
| User wants | What they download | What `sekai onboard` does |
|---|---|---|
| CLI only | `sekai` runtime binary | Configure provider, done |
| CLI + Discord | `sekai` runtime binary | Download + install `channel-discord.wasm` |
| Local web UI | `sekai` + `sekai-gw` | Configure both, open browser |
| Desktop app | `sekai-desktop` installer | Bundles runtime + gateway + UI |
| Everything | `sekai-desktop` or `sekai --profile full` | Downloads all plugins |

The `sekai plugin install` command (backed by `PluginHost`, which already exists) becomes the package manager. The `sekai onboard` wizard integrates it so non-technical users never see `cargo`.

#### 4.4.1 Versioning Policy

As Sekai transitions from a single crate to a multi-crate workspace, two concerns must be kept separate from the start:

- **The product version** — what `sekai --version` reports, what GitHub Releases, changelogs, and package managers (Homebrew, apt, cargo-binstall) track. This is the version operators and users reason about.
...
# crates/sekai-kernel/Cargo.toml
[package]
...
- It reflects Sekai's identity as a **product**, not a library ecosystem
...
|---|---|
| `sekai-api` | Starts at `0.1.0`; its `1.0.0` release is a formal milestone deliverable of v1.0.0, signalling a stable Rust trait surface for plugin SDK authors |
| `aardvark-sys`, `sekai-robot-kit` | Hardware library crates with their own user audiences and maintenance cadences; not application components |
...
|---|---|---|
| **Stable** | Covered by the product's breaking-change policy. No breaking changes without a MAJOR version bump and a published migration guide. | Kernel (target: v0.8.0), `sekai-api` WIT interface (target: v0.9.0), kernel IPC API (target: v1.0.0) |
| **Beta** | Functional and tested. Breaking changes are permitted in MINOR releases but are announced in the changelog with upgrade notes. | `sekai-gw` (v0.9.0 → v1.0.0), mature channel and tool plugins |
...
- **What capabilities are available** — determined at runtime by which plugins are installed via `sekai plugin install`
...
| IPC server | Web assets (moved to `sekai-gw`) |
| Platform sandbox where supported | `peripheral-rpi` (separate hardware build) |

There is no longer a "build with everything" binary. That mental model is replaced by `sekai plugin install --profile full`, which downloads the full plugin catalog after installing the lean kernel binary.
...
|---|---|---|
| `sekai` kernel binary | `x86_64-unknown-linux-musl`, `aarch64-unknown-linux-gnu`, `armv7-unknown-linux-gnueabihf`, `x86_64-apple-darwin`, `aarch64-apple-darwin`, `x86_64-pc-windows-msvc` | Static musl build for Linux x86_64; GNU for ARM targets |
| `sekai` kernel binary (hardware) | `aarch64-unknown-linux-gnu`, `armv7-unknown-linux-gnueabihf` | Same targets, compiled with `peripheral-rpi` and `hardware` flags for Raspberry Pi deployments |
| `sekai-gw` gateway binary | Same platform matrix as kernel | Published alongside the kernel; users install separately |
| WASM plugin files | `wasm32-wasip1` | Published to the plugin registry (not GitHub Releases); installable via `sekai plugin install` |
| `sekai-desktop` installer | `x86_64` and `aarch64` for macOS, Windows, Linux (AppImage/deb) | Bundles kernel + gateway + full plugin set; built by the Tauri workflow |
...
Target (correct):
  ...
  sekai-gw (separate binary, optional)
...
Standards are agreements that have been made by many smart people over many years. Adopting them means we get those years of thinking for free, and it means our software integrates naturally with the rest of the ecosystem. Here are the ones that apply directly to Sekai.
...
**Why it matters for Sekai:** We have already implemented `OtelObserver` against our `Observer` trait. We have Prometheus metrics and DORA metrics. The issue is that these are not yet standardized across the codebase — some modules log with `tracing::info!`, others emit `ObserverEvent`s, and the two are not connected.
...
- Structured log output should be JSON when `SEKAI_LOG_FORMAT=json` is set (already using the `tracing` crate, just needs a JSON subscriber)
...
**Why it matters for Sekai:** Our `WasmTool` and `WasmChannel` bridges currently have no formal contract for what a plugin WASM binary must export. This means a plugin author has to guess. WIT files define that contract precisely and enable automatic code generation for plugin authors in any language.
...
- A plugin author writes Rust (or Go, or C, or Python) against the WIT interface and `cargo build --target wasm32-wasi` — the result drops into `~/.sekai/plugins/`
...
**Why it matters for Sekai:** The kernel's local IPC API (the socket that the gateway and other components connect to) needs a stable, documented contract. Without a formal spec, the gateway and kernel will drift apart silently over time.
...
**Why it matters for Sekai:** The gateway handles webhooks from external services, processes untrusted user input, and manages secrets. The pairing system, WebAuthn support, and rate limiting all exist — but there is no framework for verifying that they are complete or correct.
...
**Why it matters for Sekai:** When someone asks "is this good enough to merge?" the answer is currently subjective. ISO 25010 gives us a vocabulary for that conversation. The vision commitments map directly: "zero overhead" → performance efficiency; "any hardware" → portability; "zero compromise" → security + reliability.
...
**D1: Extract `sekai-api` crate**

Create a new crate `crates/sekai-api` containing only trait definitions and their supporting types. No implementations. No heavy dependencies. This crate should compile in under two seconds.
...
Every other crate in the workspace that needs these types adds `sekai-api` as a dependency. The compiler now enforces that no implementation crate can import another implementation crate without going through the API layer.

**D2: Extract `sekai-tool-call-parser` crate**
...
Create `crates/sekai-tool-call-parser` with a public API of approximately:
...
Formalize what is already implemented: document that `ObserverEvent` and `ObserverMetric` are the internal event bus, and that `OtelObserver` is the canonical production backend. Add a JSON structured logging subscriber for `SEKAI_LOG_FORMAT=json`. Adopt W3C Trace Context for future cross-component tracing.
...
- `sekai:tool/tool.wit` — the Tool plugin interface
- `sekai:channel/channel.wit` — the Channel plugin interface
...
- `sekai-api` compiles in < 2 seconds with zero implementation dependencies
- `sekai-tool-call-parser` has ≥ 95% test coverage (the logic is fully testable in isolation)
...
**Why this phase:** Once the seams exist (v0.7.0), we can draw the runtime boundary explicitly. This phase extracts `sekai-runtime` as a standalone crate, completes the WASM plugin execution bridge, and wires the plugin registry client — the mechanism by which everything outside the runtime connects to it.

**Vision alignment:** This is where the composition model becomes real for users. A user who wants only a CLI agent downloads one binary, runs `sekai onboard`, and is done — no Rust toolchain, no compilation. The `sekai onboard` wizard gains the ability to download plugin components on demand.

#### Deliverables

**D1: Formalize `sekai-runtime` crate**

Extract the agent orchestration loop, CLI channel, security policy, plugin host, and IPC API into `crates/sekai-runtime`, gated by the `agent-runtime` feature. This crate depends on `sekai-api` and the foundation crates. It has no knowledge of Telegram, Discord, Anthropic, or any specific tool implementation.
...
Add a `sekai plugin` subcommand backed by a simple registry client:

```
sekai plugin list              # list installed plugins
sekai plugin search <query>    # search the component registry
sekai plugin install <name>    # download, verify, and install a plugin
sekai plugin remove <name>     # remove an installed plugin
sekai plugin update            # update all installed plugins
```

The registry is a JSON index file served from a known URL (e.g., `https://plugins.sekailabs.ai/index.json`). Each entry includes name, version, download URL, SHA-256 checksum, and the publisher's Ed25519 public key. The `PluginHost` signature verification already handles the security model.

**D4: Integrate `sekai onboard` with the plugin system**

The onboarding wizard should ask the user which channels and integrations they want, then call `PluginRegistry::install` for each. No compilation required. The user downloads a binary, runs `sekai onboard`, and has a working configured agent in under two minutes.
...
- `sekai-runtime` compiles independently with no channel or tool implementation code
- `sekai plugin install channel-discord` works end-to-end
- `sekai onboard` installs plugins without requiring a Rust toolchain
...
**Vision alignment:** This phase delivers the "zero external requirements" promise fully. A user on a Raspberry Pi gets a kernel binary with no web server, no React app, and no HTTP listener. A user who wants the web dashboard installs `sekai-gw` separately.
...
Add the IPC server to `sekai-kernel` behind a feature flag (`--features ipc`). On platforms that support it, the kernel listens on a Unix socket at `~/.sekai/kernel.sock`. On Windows, use a named pipe. The `sekai gateway` command (the current entrypoint for the web server) becomes `sekai-gw` connecting to this socket.

**D3: Extract `sekai-gw` as a separate binary**

Move `src/gateway/` to a new `crates/sekai-gw/` crate with its own binary. It depends on `sekai-api` and connects to the kernel via the IPC API. The embedded React application via `rust-embed` moves entirely into this crate — the kernel binary no longer contains any web assets.
...
Update `apps/tauri/` to bundle `sekai-gw` as a Tauri sidecar binary. The Tauri app becomes the "full experience" distribution: it starts the kernel and gateway automatically and opens the web UI. Users who download the Tauri app get everything working without touching a terminal.

#### Success Metrics for v0.9.0

- Kernel binary (release) does not contain any web assets or HTTP server code
- `sekai-gw` starts, connects to the kernel via IPC, and serves the web dashboard
- Removing `sekai-gw` does not break the kernel or any channel plugins
...
Approximately 60 of the 70+ tools move to plugin crates, grouped by domain: `sekai-tools-web` (browser, search, screenshot, PDF), `sekai-tools-integrations` (Jira, Notion, Google Workspace, MS365, LinkedIn), `sekai-tools-hardware` (board info, GPIO), `sekai-tools-cloud` (cloud ops, security ops). The kernel retains only the 10–12 core tools identified in v0.8.0.
...
1. Add `sekai-plugin-sdk` as a dependency
2. Implement the WIT-generated trait
3. `cargo build --target wasm32-wasi`
4. `sekai plugin install ./my-plugin/`
...
- `sekai onboard` completes a full setup in under 2 minutes on a Raspberry Pi Zero 2W with no Rust toolchain installed
- The full plugin catalog is installable with `sekai plugin install --profile full`
...
|---|---|---|
| Tool call parser (from `loop_.rs`) | ~1,400 | `sekai-tool-call-parser` crate |
| 60+ non-core tool implementations | ~30,000 | Plugin crates |
| 24+ non-core channel implementations | ~7,200 | Plugin crates |
| Gateway HTTP server | ~2,260 | `sekai-gw` crate |
| Embedded React app (binary weight) | — | `sekai-gw` crate |
...
| `src/gateway/mod.rs` | ~2,260 | Moves to `sekai-gw` | 100% |
...
With the microkernel architecture, the answer is: "it goes to the kernel's `Channel` receiver, via the `channel-discord` plugin." A new contributor can understand the Discord channel completely by reading one plugin crate. They can understand the full agent loop by reading `sekai-kernel` without any channel or tool code in scope.
...
Every bug report will have a clear home. "The agent is calling tools incorrectly" → `sekai-tool-call-parser` or `sekai-runtime`. "The Discord integration is broken" → `channel-discord` plugin. "The web dashboard is not loading" → `sekai-gw`. Right now, any of those bugs could be anywhere in 50,000+ lines.
...
A published WIT interface and plugin SDK means anyone can extend Sekai without forking it. A company that needs a specific integration can write a plugin against the public interface. This is how ecosystems are built.
...
**Dependency Inversion Principle** — High-level modules should not depend on low-level modules. Both should depend on abstractions. This is why `sekai-runtime` depends on `sekai-api` (abstractions) and not on `channel-discord` (a specific implementation).
...
*This proposal was developed from a detailed analysis of the Sekai codebase at v0.6.8. The code metrics cited are based on direct measurement of the source files. The architectural recommendations reflect established patterns in systems software design applied to the specific constraints and goals of the Sekai project.*
