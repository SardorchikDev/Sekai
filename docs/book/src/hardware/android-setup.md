...
Sekai provides prebuilt binaries for Android devices.
...
The easiest way to run Sekai on Android is via [Termux](https://termux.dev/).
...
# For 64-bit (aarch64):
curl -LO https://github.com/sekai-labs/sekai/releases/latest/download/sekai-aarch64-linux-android.tar.gz
tar xzf sekai-aarch64-linux-android.tar.gz

# For 32-bit (armv7):
curl -LO https://github.com/sekai-labs/sekai/releases/latest/download/sekai-armv7-linux-androideabi.tar.gz
tar xzf sekai-armv7-linux-androideabi.tar.gz
```
...
For advanced users who want to run Sekai outside Termux:

```bash
# From your computer with ADB
adb push sekai /data/local/tmp/
adb shell chmod +x /data/local/tmp/sekai
adb shell /data/local/tmp/sekai --version
```
