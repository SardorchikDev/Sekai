# Sekai on Nucleo-F401RE — Step-by-Step Guide

Run Sekai on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

Sekai can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
**Config:** Run `sekai onboard` (hardware step adds the board interactively), or use `sekai config set peripherals.boards.0.board nucleo-f401re`, `transport serial`, and `path <your-serial-port>`. See the [Config reference](../reference/config.md) for all fields.
...
Sekai includes everything for Nucleo-F401RE:

| Component | Location | Purpose |
|-----------|----------|---------|
| Firmware | `firmware/nucleo/` | Embassy Rust — USART2 (115200), gpio_read, gpio_write |
| Serial peripheral | `crates/sekai-hardware/src/peripherals/serial.rs` | JSON-over-serial protocol (same as Arduino/ESP32) |
| Flash command | `sekai peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
```bash
sekai agent --message "Turn on the LED on pin 13"
```
...
| 3 | `sekai peripheral flash-nucleo` |
| 4 | `sekai onboard` → hardware step (or `sekai config set peripherals.boards.0.path <serial-port>`) |
| 5 | `sekai daemon` or `sekai agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `sekai peripheral list` to verify.
