...
This guide covers installing and running Sekai on Raspberry Pi (Pi 3, Pi 4, Pi 5, Pi Zero 2 W).
...
**Runtime memory is minimal.** Even on a Pi Zero 2 W, the core agent runs in well under 5 MB RSS once it's started. The hardware ladder above is about whether you can compile on the device, not whether Sekai can run on it.
...
```bash
curl -LsSf https://raw.githubusercontent.com/sekai-labs/sekai/master/install.sh | sh
```

The script auto-detects your architecture (`aarch64` or `armv7`) and installs the matching release binary into `$CARGO_HOME/bin/sekai` (defaulting to `~/.cargo/bin/sekai`). Make sure that directory is on your `PATH`.

### Manual download

Pick the matching tarball from the [latest release](https://github.com/sekai-labs/sekai/releases/latest):

```bash
# 64-bit (Pi 4/5 with 64-bit Raspberry Pi OS)
curl -LO https://github.com/sekai-labs/sekai/releases/latest/download/sekai-aarch64-unknown-linux-gnu.tar.gz
tar xzf sekai-aarch64-unknown-linux-gnu.tar.gz
sudo install -m 0755 sekai /usr/local/bin/

# 32-bit (Pi Zero 2 W, older Pi 3 with 32-bit OS)
curl -LO https://github.com/sekai-labs/sekai/releases/latest/download/sekai-armv7-unknown-linux-gnueabihf.tar.gz
tar xzf sekai-armv7-unknown-linux-gnueabihf.tar.gz
sudo install -m 0755 sekai /usr/local/bin/
```
...
# Copy to your Pi
scp target/aarch64-unknown-linux-gnu/release/sekai pi@raspberrypi:~/
```
...
sudo install -m 0755 target/release/sekai /usr/local/bin/
# or target/release-fast/sekai, or target/ci/sekai
```
...
**Pis are memory-constrained, and that's the operating reality this section is written against.** The 2 GB Pi 4 is the low-bar test unit for this guide — if a setup doesn't leave headroom on a 2 GB box, it's not a setup we recommend. Sekai itself runs in well under 5 MB RSS at runtime, but everything you stack alongside it (channel transports, browser-control, MCP servers, an adjacent agent or two, plus the OS) competes for the same fixed pool. Memory you don't spend on container infrastructure is memory Sekai and its tools get to use.
...
| Sekai runtime (gateway only) | ~5 MB |
| One agent container (e.g. ghcr.io/sekai-labs/sekai) | ~30-80 MB |
| **Available with Docker** | ~1.3-1.5 GB |
| **Available with Podman (no daemon)** | ~1.5-1.7 GB |

The Podman delta is on the order of ~150-200 MB freed up — small in absolute terms, large as a percentage of what's left over after the OS gets its share. On a 2 GB unit that's the difference between comfortably running Sekai + a heavy channel transport (Matrix with media, browser-automation skills) and OOM-killing under load.
...
### Running Sekai under Podman

The published OCI image works under Podman without modification:

```bash
podman pull ghcr.io/sekai-labs/sekai:latest
...
> **Bind gotcha:** Sekai defaults to `127.0.0.1` for the gateway. Inside a container that means the gateway is unreachable from the host. Always pass `--host 0.0.0.0` (or set `SEKAI_BIND=0.0.0.0`) when running in a container.
...
```ini
# ~/.config/containers/systemd/sekai.container
[Unit]
Description=Sekai gateway
...
Environment=SEKAI_BIND=0.0.0.0
Exec=daemon --host 0.0.0.0 --port 42617
Volume=sekai-data:/root/.sekai
...
systemctl --user start sekai.service
```
...
This walks you through provider auth, gateway config, and creates `~/.sekai/config.toml`.

### 2. Verify it works

```bash
sekai doctor
sekai agent -m "what's 2+2?"
```
...
systemctl --user enable --now sekai
...
```bash
sekai daemon --host 0.0.0.0 --port 42617
```

### 5. Enable channels

Sekai can connect to chat platforms (Matrix, Mattermost, Discord, Telegram, etc.). See [Channels → Overview](../channels/overview.md). Most channel transports work fine on a Pi; the heaviest is the WebRTC stack used by some voice channels, which can spike CPU during call setup.
...
Sekai binds `127.0.0.1` by default — inside a container that means localhost-of-the-container. Pass `--host 0.0.0.0` (or `SEKAI_BIND=0.0.0.0`) when running in Podman/Docker.
...
- **tmpfs for build artifacts** (if you have RAM + swap headroom): `export CARGO_TARGET_DIR=/tmp/sekai-target`.
