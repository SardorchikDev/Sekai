# Sekai on Arduino Uno Q — Step-by-Step Guide

Run Sekai on the Arduino Uno Q's Linux side. Telegram works over WiFi; GPIO control uses the Bridge (requires a minimal App Lab app).

---

## What's Included (No Code Changes Needed)

Sekai includes everything needed for Arduino Uno Q. **Clone the repo and follow this guide — no patches or custom code required.**

| Component | Location | Purpose |
|-----------|----------|---------|
| Bridge app | `firmware/uno-q-bridge/` | MCU sketch + Python socket server (port 9999) for GPIO |
| Bridge tools | `crates/sekai-hardware/src/peripherals/uno_q_bridge.rs` | `gpio_read` / `gpio_write` tools that talk to the Bridge over TCP |
| Setup command | `crates/sekai-hardware/src/peripherals/uno_q_setup.rs` | `sekai peripheral setup-uno-q` deploys the Bridge via scp + arduino-app-cli |
...
## Phase 2: Install Sekai on Uno Q
...
# Clone sekai (or scp your project)
...
# Install
sudo cp target/release/sekai /usr/local/bin/
```
...
# Copy to Uno Q
scp target/aarch64-unknown-linux-gnu/release/sekai arduino@<UNO_Q_IP>:~/
ssh arduino@<UNO_Q_IP> "sudo mv ~/sekai /usr/local/bin/"
```
...
# Quick config
sekai onboard --api-key YOUR_OPENROUTER_KEY --provider openrouter

# Or create config manually
mkdir -p ~/.sekai/workspace
nano ~/.sekai/config.toml
```
...
## Phase 4: Run Sekai Daemon

```bash
ssh arduino@<UNO_Q_IP>

# Run daemon (Telegram polling works over WiFi)
sekai daemon --host 127.0.0.1 --port 42617
```

**At this point:** Telegram chat works. Send messages to your bot — Sekai responds. No GPIO yet.

---

## Phase 5: GPIO via Bridge (Sekai Handles It)

Sekai includes the Bridge app and setup command.

### 5.1 Deploy Bridge App

**From your Mac** (with sekai repo):
```bash
sekai peripheral setup-uno-q --host 192.168.0.48
```

**From the Uno Q** (SSH'd in):
```bash
sekai peripheral setup-uno-q
```
...
Now when you message your Telegram bot *"Turn on the LED"* or *"Set pin 13 high"*, Sekai uses `gpio_write` via the Bridge.
...
| 5 | `git clone https://github.com/sekai-labs/sekai.git && cd sekai` |
| 6 | `cargo build --release --features hardware` |
| 7 | `sekai onboard --api-key KEY --provider openrouter` |
| 8 | `sekai config set channels.telegram.bot-token <TOKEN>` |
| 9 | `sekai daemon --host 127.0.0.1 --port 42617` |
...
- **"command not found: sekai"** — Use full path: `/usr/local/bin/sekai` or ensure `~/.cargo/bin` is in PATH.
...
- **GPIO commands ignored** — Ensure Bridge app is running (`sekai peripheral setup-uno-q` deploys and starts it). Config must have `board = "arduino-uno-q"` and `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Use `default_provider = "glm"` or `"zhipu"` with `GLM_API_KEY` in env or config. Sekai uses the correct v4 endpoint.
