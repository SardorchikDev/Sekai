# Hardware Peripherals Design — Sekai

Sekai enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** Sekai acts as a hardware-aware AI agent that:
...
**Mental model:** Sekai = brain that understands hardware. Peripherals = arms and legs it controls.
...
Sekai runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  Sekai on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. Sekai fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
Sekai runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  Sekai on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. Sekai identifies connected hardware (VID/PID, architecture)
...
2. Sekai writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. Sekai auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| Sekai runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs Sekai; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

Sekai on Pi; GPIO via rppal or sysfs. No separate firmware.
...
See the [CLI reference](../reference/cli.md) for `sekai hardware` / `sekai peripheral` subcommands and the [Config reference](../reference/config.md) for the `[peripherals]` and `[[peripherals.boards]]` fields.
...
1. **Startup:** Sekai loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between Sekai and peripherals:
...
- **sekai-firmware** or **sekai-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; Sekai connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`sekai peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `sekai hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `sekai hardware introspect <path>`: memory map, peripheral list
...
**Usage:** `sekai config set peripherals.datasheet-dir docs/datasheets`. Place `.md` or `.txt` files named by board (e.g. `nucleo-f401re.md`, `rpi-gpio.md`). Files in `_generic/` or named `generic.md` apply to all boards. Chunks are retrieved by keyword match and injected into the user message context.

### Phase 5: Edge-Native — RPi ✅ (Done)

- [x] Sekai on Raspberry Pi (native GPIO via rppal)
...
- [ ] Sekai *on* ESP32 (WiFi + LLM, edge-native) — future
...
- Running full Sekai *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). Sekai runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. Sekai gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: Sekai from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey Sekai, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
