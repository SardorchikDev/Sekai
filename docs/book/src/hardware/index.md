...
Sekai's hardware subsystem lets the agent control microcontrollers, SBCs, and peripherals directly. Enable with `--features hardware`.
...
See [Adding boards & tools](./adding-boards-and-tools.md) for the step-by-step. TL;DR: implement the `Peripheral` trait from `crates/sekai-hardware/src/`, add a board-specific feature flag, write a probe routine that identifies the board from USB descriptors or serial handshake.
