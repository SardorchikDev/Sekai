...
- **Installer overhaul.** `sekai install` learns preset and gateway flags, a
...
- **Drift detection** compares in-memory state to on-disk `config.toml` via
  ...
  `X-Sekai-Override-Drift: true` overrides (#6179).
- **OpenAPI 3.1 spec** at `/api/openapi.json`, Scalar explorer at `/api/docs`,
  committed snapshot at `crates/sekai-gateway/openapi.json` regenerated by
...
- **CLI parity:** `sekai config patch` (JSON Patch over the same core),
...
- **`sekai memory reindex` CLI** for rebuilding the embedding index in place
  (#6046).
...
- **Three-surface personality editor** for the seven runtime markdown files
  ...
  allowlist enforced in `crates/sekai-gateway/src/api_personality.rs` —
