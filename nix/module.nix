# services.sekai — multi-instance NixOS module for the Sekai agent.
#
# Design memo: see ./README.md for usage; the upstream PR body links the full
# design rationale.
#
# Layout:
#   - `services.sekai.instances.<name>` is an attrset of instances.
#     Membership in the attrset is the activation signal — there is no
#     top-level `enable`. Mirrors `services.restic.backups.<name>`.
#   - Each instance gets:
#       * a dedicated systemd unit          `sekai-<name>.service`
#       * a dedicated state directory       `/var/lib/sekai-<name>`
#       * a dedicated system user / group   `sekai-<name>`
...
#     This substitution is a property of *this module*, not of Sekai —
#     Sekai itself reads `config.toml` verbatim plus a handful of named
#     env-var overrides documented in `crates/sekai-config/src/schema.rs`
#     (e.g. `OPENROUTER_API_KEY`, `SEKAI_PROVIDER`).
#
# Single-instance usage (laptop / single-host case):
#
#   services.sekai.instances.me = {
#     environmentFile = "/run/agenix/sekai-bot-token";
#     settings = {
...
#
#   services.sekai.instances = lib.genAttrs slots (n: {
...
let
  ...
  instanceModule =
    ...
    {
      options = {
        package = mkPackageOption pkgs "sekai" { };

        user = mkOption {
          type = types.str;
          default = "sekai-${name}";
          defaultText = literalExpression ''"sekai-''${name}"'';
          description = ''
        ...
        dataDir = mkOption {
          type = types.path;
          default = "/var/lib/sekai-${name}";
          defaultText = literalExpression ''"/var/lib/sekai-''${name}"'';
          description = ''
            State directory. Holds `config.toml`, the workspace at
            `''${dataDir}/workspace`, and Sekai's SQLite databases.

            Created by `systemd-tmpfiles` at activation time with mode `0750`
            owned by {option}`user`:{option}`group`, so any absolute path is
            valid — `/var/lib/sekai-me`, `/srv/sekai-me`, or a nested
            location like `/var/lib/sekai/me` all work and are created
        ...
        settings = mkOption {
          type = types.submodule {
            ...
            # Sekai config key flow through with TOML's value-model
          ...
          '';
          description = ''
            Sekai configuration as a Nix attrset. Rendered to TOML in the
        ...
        environmentFile = mkOption {
          ...
          default = null;
          example = "/run/agenix/sekai-bot-token";
          description = ''
        ...
        extraConfig = mkOption {
          ...
          description = ''
            ...
            block. Documented escape hatch (per RFC-42) for Sekai config
        ...
        bindReadOnlyPaths = mkOption {
          ...
          example = literalExpression ''
            {
              "/var/lib/sekai-me/workspace/skills/git" = "/etc/sekai-skills/git";
            }
          '';
          description = ''
  ...
  #
  #   systemd.services."sekai-myinstance".serviceConfig.MemoryMax =
  #     lib.mkForce "1G";
  #
  ...
  renderConfigFile =
    name: instanceCfg:
    let
      base = tomlFormat.generate "sekai-${name}-config.toml" instanceCfg.settings;
    in
    if instanceCfg.extraConfig == "" then
      base
    else
      pkgs.runCommand "sekai-${name}-config.toml" { } ''
        cat ${base} > $out
        cat <<'SEKAI_EXTRA_CONFIG_EOF' >> $out

        ${instanceCfg.extraConfig}
        SEKAI_EXTRA_CONFIG_EOF
      '';
  ...
  mkInstanceService =
    ...
    let
      ...
      configResolveScript = pkgs.writeShellApplication {
        name = "sekai-${name}-resolve-config";
        ...
        text = ''
          ...
          if [ ! -s "$tmp" ]; then
            echo "sekai-${name}: rendered config.toml is empty after envsubst" >&2
            rm -f "$tmp"
            exit 1
          fi
          chmod 0600 "$tmp"
    ...
    in
    nameValuePair "sekai-${name}" {
      description = "Sekai agent (instance ${name})";
      ...
      environment = {
        SEKAI_CONFIG_DIR = instanceCfg.dataDir;
        SEKAI_WORKSPACE = "${instanceCfg.dataDir}/workspace";
      };
      ...
      serviceConfig = {
        ...
        # ${dataDir}/config.toml so Sekai reads it at a stable path
        ...
        # `systemd.services."sekai-<name>".serviceConfig.X = mkForce ...`
        # rather than via a module escape hatch.
        NoNewPrivileges = true;
        PrivateTmp = true;
        PrivateDevices = true;
        # Closed device policy + empty allow-list — matches `atticd`.
        # Sekai doesn't need /dev/* nodes for normal operation.
        DeviceAllow = "";
        DevicePolicy = "closed";
        ...
        # Rust binary with no JIT. Sekai 0.7.x has no JIT path.
        ...
        # leaves behind on stop. Sekai doesn't use SysV IPC, so this
...
in
{
  options.services.sekai = {
    instances = mkOption {
      type = types.attrsOf (types.submodule instanceModule);
      default = { };
      description = ''
        Sekai instances to run on this host. Each entry produces a
        `sekai-<name>.service` systemd unit with its own state
  ...
  config = mkIf (cfg.instances != { }) {
    ...
    users.users = mapAttrs' (
      ...
      nameValuePair instanceCfg.user {
        ...
        description = "Sekai instance ${name}";
      }
      ...
      [
        {
          assertion = badNames == [ ];
          message = ''
            services.sekai.instances: instance name(s) ${toString badNames}
        ...
        }
        {
          ...
          message = ''
            services.sekai.instances: two or more instances declare the
  ...
  meta = {
    ...
    # Sekai repository. `[]` rather than a guess so `meta.maintainers`
