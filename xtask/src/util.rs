...
pub fn fluent_locales_dir(root: &Path) -> PathBuf {
    root.join("crates/sekai-runtime/locales")
}
...
/// Read a `[providers.models.<name>]` entry from ~/.sekai/config.toml.
pub fn read_provider_config(provider_name: &str) -> anyhow::Result<ProviderConfig> {
    ...
    let candidates = [
        format!("{home}/.sekai/config.toml"),
        format!("{home}/.config/sekai/config.toml"),
    ];
    let raw = candidates
        .iter()
        .find_map(|p| std::fs::read_to_string(p).ok())
        .ok_or_else(|| anyhow::anyhow!("config.toml not found (tried ~/.sekai/config.toml)"))?;
