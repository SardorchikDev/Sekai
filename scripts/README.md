...
This directory contains everything needed to cross-compile Sekai and deploy it to a Raspberry Pi over SSH.
...
| `rpi-config.toml` | Production config template deployed to `~/.sekai/config.toml` |
| `sekai.service` | systemd unit file installed on the Pi |
...
| `RPI_DIR` | `~/sekai` | Remote directory for the binary and `.env` |
...
2. **Stop service** — runs `sudo systemctl stop sekai` on the Pi (continues if not yet installed).
3. **Create remote directory** — ensures `$RPI_DIR` exists on the Pi.
4. **Copy binary** — SCPs the compiled binary to `$RPI_DIR/sekai`.
...
6. **Deploy config** — copies `rpi-config.toml` to `~/.sekai/config.toml`, preserving any `api_key` already present in the file.
7. **Install systemd service** — copies `sekai.service` to `/etc/systemd/system/`, then enables and restarts it.
...
## Interacting with Sekai on the Pi
...
```bash
ssh pi@raspberrypi.local 'journalctl -u sekai -f'
```
...
Sekai is deployed with the `peripheral-rpi` feature, which enables two LLM-callable tools:
...
|------------|--------|-------------|
| `~/sekai/sekai` | compiled binary | Main agent binary |
| `~/sekai/.env` | created on first deploy | API key and environment variables |
| `~/.sekai/config.toml` | `rpi-config.toml` | Agent configuration |
| `/etc/systemd/system/sekai.service` | `sekai.service` | systemd service unit |
...
To customise, edit `~/.sekai/config.toml` directly on the Pi and restart the service.
...
```bash
ssh pi@raspberrypi.local 'sudo systemctl status sekai'
ssh pi@raspberrypi.local 'journalctl -u sekai -n 50 --no-pager'
```
...
```bash
ssh pi@raspberrypi.local 'file ~/sekai/sekai'
