#!/usr/bin/env bash
# deploy-rpi.sh — cross-compile Sekai for Raspberry Pi and deploy via SSH.
#
...
#   RPI_DIR         — remote deployment dir           (default: /home/$RPI_USER/sekai)
...
RPI_DIR="${RPI_DIR:-/home/${RPI_USER}/sekai}"
TARGET="aarch64-unknown-linux-gnu"
FEATURES="hardware,peripheral-rpi"
BINARY="target/${TARGET}/release/sekai"
...
echo "==> Building Sekai for Raspberry Pi (${TARGET})"
...
echo ""
echo "==> Stopping sekai service (if running)"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo systemctl stop sekai 2>/dev/null || true"
...
# ── 4. Deploy binary ──────────────────────────────────────────────────────────
echo ""
echo "==> Deploying binary to ${RPI_USER}@${RPI_HOST}:${RPI_DIR}/sekai"
${SCP_CMD} ${SCP_OPTS} "${BINARY}" "${RPI_USER}@${RPI_HOST}:${RPI_DIR}/sekai"
...
# ── 5. Deploy config ─────────────────────────────────────────────────────────
CONFIG_DEST="/home/${RPI_USER}/.sekai/config.toml"
echo ""
echo "==> Deploying config to ${CONFIG_DEST}"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" "mkdir -p /home/${RPI_USER}/.sekai"
...
SERVICE_DEST="/etc/systemd/system/sekai.service"
echo ""
echo "==> Installing systemd service (requires sudo on the Pi)"
${SCP_CMD} ${SCP_OPTS} "scripts/sekai.service" "${RPI_USER}@${RPI_HOST}:/tmp/sekai.service"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo mv /tmp/sekai.service ${SERVICE_DEST} && \
   sudo systemctl daemon-reload && \
   sudo systemctl enable sekai && \
   sudo systemctl restart sekai && \
   sudo systemctl status sekai --no-pager || true"
...
# ── 8. Reset ACT LED trigger so Sekai can control it ──────────────────────
echo ""
...
echo ""
echo "    Sekai is running at http://${RPI_HOST}:8080"
echo "    POST /api/chat  — chat with the agent"
echo "    GET  /health    — health check"
echo ""
echo "    To check logs: ssh ${RPI_USER}@${RPI_HOST} 'journalctl -u sekai -f'"
