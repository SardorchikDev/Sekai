<p align="center">
  <img src="https://raw.githubusercontent.com/sekai-labs/sekai/master/docs/assets/sekai-banner.png" alt="Sekai" width="600" />
</p>

<h1 align="center">🦀 Sekai — Personal AI Assistant</h1>
...
<p align="center">
  <a href="https://github.com/sekai-labs/sekai/actions/workflows/ci.yml"><img src="https://img.shields.io/github/actions/workflow/status/sekai-labs/sekai/ci.yml?branch=master&label=build" alt="Build Status" /></a>
  <a href="https://github.com/sekai-labs/sekai/releases/latest"><img src="https://img.shields.io/github/v/release/sekai-labs/sekai?label=release" alt="Latest release" /></a>
  ...
  <a href="https://github.com/sekai-labs/sekai/graphs/contributors"><img src="https://img.shields.io/github/contributors/sekai-labs/sekai?color=green" alt="Contributors" /></a>
...
Sekai is an agent runtime — a single Rust binary you configure and run. It talks to LLM providers (Anthropic, OpenAI, Ollama, and ~20 others), reaches the world through 30+ channels (Discord, Telegram, Matrix, email, voice, webhooks, your own CLI), and acts through tools (shell, browser, HTTP, hardware, custom MCP servers). Everything runs on your machine, with your keys, in your workspace.
...
The installer asks whether you want a prebuilt binary (fast, ~seconds) or a source build (slower, customisable). Both end the same way — `sekai onboard` kicks off automatically.
...
```bash
sekai onboard                  # wizard: picks a provider, wires channels
sekai agent                    # interactive chat in the terminal
sekai service install          # register as systemd/launchctl/Windows Service
sekai service start            # run it always-on in the background
```
...
One TOML file at `~/.sekai/config.toml`. Pointers:
...
- If you see `provider streaming failed, falling back to non-streaming chat`, Sekai retries the same request in non-streaming mode. Check `sekai auth status` before changing provider config.
...
Start with [how to contribute](docs/book/src/contributing/how-to.md). Larger changes go through the [RFC process](docs/book/src/contributing/rfcs.md). Real-time chat lives on [Discord](https://discord.com/invite/wDshRVqRjx) (the best way to reach the team); durable work tracking is on [GitHub issues](https://github.com/sekai-labs/sekai/issues).

Good places to start:

- New channel → `crates/sekai-channels/`
- New provider → `crates/sekai-providers/`
- New tool → `crates/sekai-tools/`
- Hardware support → `crates/sekai-hardware/`
- Docs → `docs/book/src/`

AI-assisted PRs are welcome; see [Contribution culture (RFC #5615)](https://github.com/sekai-labs/sekai/issues/5615) for the co-authorship norms.
...
Do not file public issues for security vulnerabilities. Email `security@sekai.dev`. See [SECURITY.md](SECURITY.md) for the full policy.

## Official repository & impersonation notice

This is the only official Sekai repository:

> <https://github.com/sekai-labs/sekai>

Any other repository, organization, domain, or package claiming to be "Sekai" or implying affiliation with Sekai Labs is **unauthorized and not affiliated with this project**.

## License

Dual-licensed: [MIT](LICENSE-MIT) OR [Apache 2.0](LICENSE-APACHE). You may choose either. Contributors automatically grant rights under both — see [CLA](docs/book/src/contributing/cla.md). The **Sekai** name and logo are trademarks of Sekai Labs.
...
<p align="center">
  <a href="https://www.star-history.com/#sekai-labs/sekai&type=date&legend=top-left">
    <picture>
     <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=sekai-labs/sekai&type=date&theme=dark&legend=top-left" />
     <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=sekai-labs/sekai&type=date&legend=top-left" />
     <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=sekai-labs/sekai&type=date&legend=top-left" />
    </picture>
  </a>
</p>

<p align="center">
  <a href="https://github.com/sekai-labs/sekai/graphs/contributors">
    <img src="https://contrib.rocks/image?repo=sekai-labs/sekai" alt="Sekai contributors" />
  </a>
</p>
# Sekai
