...
use sekai_config::schema::resolve_runtime_dirs_for_onboarding;

const CONFIG_NOT_FOUND_ERROR: &str = "ERROR: config.toml not found.  Are you sure the bridge and Sekai are running on the same host?  Tool use will not work remotely!";
const PAIRING_TOKEN_NOT_FOUND_ERROR: &str = "ERROR: Gateway pairing is active but no ACP bridge token is cached. Run `sekai gateway get-paircode --new`, then run `sekai-acp-bridge --pair-code <code>`, or set SEKAI_ACP_BRIDGE_TOKEN.";
const ACP_BRIDGE_TOKEN_ENV: &str = "SEKAI_ACP_BRIDGE_TOKEN";
const ACP_BRIDGE_PAIRING_CODE_ENV: &str = "SEKAI_ACP_PAIRING_CODE";
...
mod tests {
    ...
    fn acp_websocket_url_includes_path_prefix() {
        assert_eq!(
            acp_websocket_url("ws", "127.0.0.1", 42617, Some("/sekai")),
            "ws://127.0.0.1:42617/sekai/acp"
        );
    }
    ...
    fn token_from_env_uses_plaintext_bridge_token() {
        let _guard = EnvGuard::set("SEKAI_ACP_BRIDGE_TOKEN", "zc_plaintext");
    ...
    fn cached_token_path_lives_next_to_config_without_using_config_toml() {
        let dir = std::path::Path::new("/tmp/sekai-config");
    ...
    fn http_gateway_url_honors_path_prefix_and_ipv6() {
        assert_eq!(
            http_gateway_url("https", "::1", 42617, Some("/sekai"), "/pair"),
            "https://[::1]:42617/sekai/pair"
        );
    }
    ...
    fn acp_bridge_target_honors_tls_and_path_prefix() {
        let config = BridgeGatewayConfig {
            require_pairing: false,
            path_prefix: Some("/sekai".to_string()),
        ...
        assert_eq!(target.url, "wss://127.0.0.1:42617/sekai/acp");
    }
    ...
    fn missing_config_error_matches_acp_client_guidance() {
        assert_eq!(
            ...
            "ERROR: config.toml not found.  Are you sure the bridge and Sekai are running on the same host?  Tool use will not work remotely!"
        );
    }
}
