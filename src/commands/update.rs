//! `sekai update` — self-update pipeline with rollback.
...
const GITHUB_RELEASES_LATEST_URL: &str =
    "https://api.github.com/repos/sekai-labs/sekai/releases/latest";
const GITHUB_RELEASES_TAG_URL: &str =
    "https://api.github.com/repos/sekai-labs/sekai/releases/tags";
...
pub async fn check(target_version: Option<&str>) -> Result<UpdateInfo> {
    ...
    let client = reqwest::Client::builder()
        .user_agent(format!("sekai/{current}"))
...
pub async fn run(target_version: Option<&str>) -> Result<()> {
    ...
    let download_path = temp_dir.path().join("sekai_new");
...
fn is_installable_release_asset(name: &str, target: &str) -> bool {
    name == format!("sekai-{target}.tar.gz") || name == format!("sekai-{target}.tgz")
}
...
async fn download_binary(url: &str, dest: &Path) -> Result<()> {
    let client = reqwest::Client::builder()
        .user_agent(format!("sekai/{}", env!("CARGO_PKG_VERSION")))
    ...
    // Release assets are .tar.gz archives containing a single `sekai` binary.
...
/// Extract the `sekai` binary from a `.tar.gz` archive.
fn extract_tar_gz(archive_bytes: &[u8], dest: &Path) -> Result<()> {
    ...
    for entry in archive.entries().context("failed to read tar entries")? {
        ...
        // The archive contains a single binary named "sekai" (or "sekai.exe" on Windows).
        let file_name = path.file_name().and_then(|n| n.to_str()).unwrap_or("");

        if file_name == "sekai" || file_name == "sekai.exe" {
            let mut buf = Vec::new();
            entry
                .read_to_end(&mut buf)
    ...
    bail!("archive does not contain a 'sekai' binary")
}
...
async fn validate_binary(path: &Path) -> Result<()> {
    ...
    if !stdout.contains("sekai") {
        bail!("downloaded binary does not appear to be sekai");
    }
...
mod tests {
    ...
    fn find_asset_url_picks_correct_gnu_over_android() {
        let release = make_release(&[
            "sekai-aarch64-linux-android.tar.gz",
            "sekai-aarch64-unknown-linux-gnu.tar.gz",
            "sekai-x86_64-unknown-linux-gnu.tar.gz",
            "sekai-x86_64-apple-darwin.tar.gz",
            "sekai-aarch64-apple-darwin.tar.gz",
            "sekai-x86_64-pc-windows-msvc.zip",
            "sekai-aarch64-pc-windows-msvc.zip",
        ]);
    ...
    fn find_asset_url_ignores_non_installable_assets() {
        ...
        let release = make_release(&[
            &format!("sekai-{target}.tar.gz.sha256"),
            &format!("sekai-{target}.zip.sha256"),
            &format!("sekai-{target}.zip"),
            &format!("sekai-{target}.tar.gz"),
        ]);
    ...
    fn find_asset_url_skips_matching_asset_with_unusable_url() {
        ...
        let release = serde_json::json!({
            "assets": [
                {
                    "name": format!("sekai-{target}.tar.gz"),
                    "browser_download_url": ""
                },
                {
                    "name": format!("sekai-{target}.tgz"),
                    "browser_download_url": null
                },
                {
                    "name": format!("sekai-{target}.tar.gz"),
                    "browser_download_url": format!("https://example.com/sekai-{target}.tar.gz")
                }
            ]
        });

        let url = find_asset_url(&release).expect("should skip unusable URLs");
        assert_eq!(url, format!("https://example.com/sekai-{target}.tar.gz"));
    }

    #[test]
    fn find_asset_url_ignores_non_sekai_assets() {
        let target = current_target_triple().expect("supported test platform");
        let release = make_release(&[
        ...
        let url = find_asset_url(&release).expect("should select sekai asset");
        assert!(
            url.contains(&format!("sekai-{target}.tar.gz")),
            "should select sekai archive, got: {url}"
        );
    }
    ...
    fn extract_tar_gz_finds_binary() {
        ...
        // Build a tar.gz in memory containing a fake "sekai" binary.
        ...
        let mut tar_buf = Vec::new();
        {
            ...
            header.set_cksum();
            builder
                .append_data(&mut header, "sekai", &fake_binary[..])
                .unwrap();
        ...
        let dest = tmp.path().join("sekai_extracted");
    ...
    fn extract_tar_gz_errors_on_missing_binary() {
        ...
        // Build a tar.gz with a file that is NOT named "sekai".
        let mut tar_buf = Vec::new();
        {
            let mut builder = tar::Builder::new(&mut tar_buf);
            let mut header = tar::Header::new_gnu();
