...
Examples:
  sekai gateway start              # use config defaults
  sekai gateway start -p 8080      # listen on port 8080
  sekai gateway start --host 0.0.0.0   # requires [gateway].allow_public_bind=true or a tunnel
  sekai gateway start -p 0         # random available port")]
    Start {
...
Examples:
  sekai gateway restart            # restart with config defaults
  sekai gateway restart -p 8080    # restart on port 8080")]
    Restart {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
...
Examples:
  sekai gateway get-paircode       # show current pairing code
  sekai gateway get-paircode --new # generate a new pairing code
  sekai gateway get-paircode --new --port 3001 # target alternate-port gateway")]
    GetPaircode {
...
Examples:
  ...
  sekai channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  sekai channel bind-telegram 123456789")]
    BindTelegram {
...
Examples:
  sekai channel send 'Someone is near your device.' --channel-id telegram --recipient 123456789
  sekai channel send 'Build succeeded!' --channel-id discord --recipient 987654321")]
    Send {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `Sekai` workspace
    Openclaw {
...
Examples:
  sekai cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  sekai cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  sekai cron add-every 60000 'Ping heartbeat'     # every minute
  sekai cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  sekai cron once 30m 'Run backup in 30 minutes'
  sekai cron once 2h 'Follow up on deployment'
  sekai cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Treat the argument as an agent prompt instead of a shell command
...
Examples:
  sekai cron update TASK_ID --expression '0 8 * * *'
  sekai cron update TASK_ID --tz Europe/London --name 'Morning check'
  sekai cron update TASK_ID --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
pub enum MemoryCommands {
    ...
    ///
    /// Run after `sekai migrate openclaw` or other bulk writes that
...
Examples:
  ...
  sekai hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  sekai peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash Sekai firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash Sekai firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  sekai peripheral flash
  sekai peripheral flash --port /dev/cu.usbmodem12345
  sekai peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash Sekai firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
