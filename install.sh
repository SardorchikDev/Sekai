...
# ── Sekai installer ───────────────────────────────────────────
# Builds and installs Sekai from source.
...
REPO_URL="https://github.com/sekai-labs/sekai.git"
...
install_prebuilt() {
  ...
  version=$(curl -fsSL "https://api.github.com/repos/sekai-labs/sekai/releases/latest" \
  ...
  asset_name="sekai-${triple}.tar.gz"
  asset_url="https://github.com/sekai-labs/sekai/releases/download/${version}/${asset_name}"
  sha256_url="https://github.com/sekai-labs/sekai/releases/download/${version}/SHA256SUMS"

  echo
  printf "%s\n" "$(bold "Installing Sekai ${version} (pre-built)")"
  ...
  case "$(uname -s)" in
    Darwin)
      web_data_dir="${HOME}/Library/Application Support/sekai/web/dist"
      ;;
    MINGW*|CYGWIN*|MSYS*)
      web_data_dir="${LOCALAPPDATA}/sekai/web/dist"
      ;;
    *)
      web_data_dir="${XDG_DATA_HOME:-${PREFIX}/.local/share}/sekai/web/dist"
      ;;
  esac

  if [ "$DRY_RUN" = true ]; then
    info "[dry-run] Would download $asset_url"
    info "[dry-run] Would install to $CARGO_HOME/bin/sekai"
  ...
  install -m 755 "$tmp_dir/sekai" "$CARGO_HOME/bin/sekai"
...
Options:
  ...
  --uninstall          Remove Sekai binary and optionally config/data
...
Examples:
  ...
  $0 --uninstall                               # remove Sekai

Environment:
  SEKAI_INSTALL_DIR   Source checkout override (default: PREFIX/.sekai/src)
  SEKAI_CARGO_FEATURES  Extra cargo features (legacy; prefer --features)
EOF
}
...
do_uninstall() {
  echo
  printf "%s\n" "$(bold "Uninstalling Sekai")"
  echo

  local bin="$CARGO_HOME/bin/sekai"
  ...
  # Check if another sekai still lurks in PATH
  local other_bin
  other_bin=$(PATH="$ORIGINAL_PATH" command -v sekai 2>/dev/null || true)
  if [ -n "$other_bin" ]; then
    local other_version
    other_version=$("$other_bin" --version 2>/dev/null | awk '{print $NF}' || echo "unknown")
    echo
    warn "Another sekai found at $other_bin (v$other_version)"
    warn "Remove it manually if you want a full uninstall"
  fi

  echo
  info "Sekai uninstalled"
  exit 0
}
...
if [ -n "${SEKAI_CARGO_FEATURES:-}" ]; then
  USER_FEATURES="${USER_FEATURES:+$USER_FEATURES,}$SEKAI_CARGO_FEATURES"
fi
  ...
  case "$1" in
    ...
    -V|--version)
      if [ -f "Cargo.toml" ]; then
        ...
        echo "install.sh for Sekai v$VERSION"
      else
...
INSTALL_DIR="${SEKAI_INSTALL_DIR:-$PREFIX/.sekai/src}"
ORIGINAL_PATH="$PATH"
...
if [ "$INSTALL_MODE" = "" ]; then
  ...
  if [ -n "$triple" ]; then
    if [ -t 0 ]; then
      echo
      printf "  %s\n" "$(bold "How would you like to install Sekai?")"
...
echo
printf "%s\n" "$(bold "Sekai — source install")"
if [ "$PREFIX" != "$HOME" ]; then
  printf "  prefix: %s\n" "$(bold "$PREFIX")"
fi
echo

if [ -f "Cargo.toml" ] && grep -q "sekai" "Cargo.toml" 2>/dev/null; then
  INSTALL_DIR="$(pwd)"
...
if [ "$DRY_RUN" != true ]; then
  ...
  if ! version_gte "$RUST_VERSION" "$MSRV"; then
    die "Rust $RUST_VERSION is too old. Sekai requires $MSRV+ (edition $EDITION). Run: rustup update stable"
  fi
...
PATH_BIN=$(PATH="$ORIGINAL_PATH" command -v sekai 2>/dev/null || true)
if [ -n "$PATH_BIN" ]; then
  PATH_VERSION=$("$PATH_BIN" --version 2>/dev/null | awk '{print $NF}' || echo "unknown")
  TARGET_BIN="$CARGO_HOME/bin/sekai"
  if [ "$PATH_BIN" != "$TARGET_BIN" ]; then
    warn "sekai found at $PATH_BIN (v$PATH_VERSION)"
...
if [ "$DRY_RUN" = true ]; then
  ...
  info "Binary:   $CARGO_HOME/bin/sekai"
  info "Config:   $PREFIX/.sekai/"
...
if [ -f "$BIN" ]; then
  ...
  ACTIVE_BIN=$(PATH="$ORIGINAL_PATH" command -v sekai 2>/dev/null || true)
  if [ -n "$ACTIVE_BIN" ] && [ "$ACTIVE_BIN" != "$BIN" ]; then
    ACTIVE_VERSION=$("$ACTIVE_BIN" --version 2>/dev/null | awk '{print $NF}' || echo "unknown")
    echo
    warn "$(bold "WARNING:") sekai in your PATH is $ACTIVE_BIN (v$ACTIVE_VERSION)"
...
if [ "$SKIP_ONBOARD" = false ] && [ "$DRY_RUN" != true ] && [ -f "$BIN" ]; then
  ...
  # Sekai — re-installs should not re-prompt.
  if ! onboarding_needed; then
    info "Existing Sekai config detected at $PREFIX/.sekai/config.toml — skipping onboard prompt."
    info "Run 'sekai onboard' to reconfigure."
  elif [ -t 0 ]; then
    ...
    echo
    printf "%s\n" "$(bold "Sekai installed. How would you like to complete onboarding?")"
    printf "  [1] CLI/TUI  (sekai onboard)\n"
    printf "  [2] Open gateway in browser (sekai daemon + dashboard)\n"
    ...
    case "${onboard_choice:-1}" in
      1|"")
        echo
        "$BIN" onboard || warn "Onboard wizard exited with an error — run 'sekai onboard' manually"
        ;;
      2)
        echo
        ...
        info "Stop the daemon with Ctrl+C when done; then run 'sekai service install' for always-on."
        "$BIN" daemon || warn "Daemon exited with an error — run 'sekai daemon' manually"
        ;;
      3)
        info "Skipped onboarding. Run 'sekai onboard' (CLI) or 'sekai daemon' (browser) when ready."
        ;;
      *)
        warn "Unknown choice '$onboard_choice' — skipping. Run 'sekai onboard' to configure."
        ;;
    esac
  else
    info "Non-interactive — skipping onboard prompt. Run 'sekai onboard' to configure."
  fi
fi

echo
info "Done. Run $(bold "sekai agent") to start chatting."
echo
