...
export async function apiFetch<T = unknown>(
  ...
  if (response.status === 401) {
    clearToken();
    window.dispatchEvent(new Event('sekai-unauthorized'));
...
// spec in-process from `sekai_gateway::openapi::build_spec()` and
...
export interface SectionInfo {
  /** Stable section key — matches `Section::as_path_prefix` in sekai-runtime. */
  key: string;
