...
1. **Find an issue.** Look for [`good first issue`](https://github.com/sekai-labs/sekai/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) labels — these are scoped for newcomers and include enough context to get moving.
...
| Variable | Effect |
|---|---|
| `SEKAI_STRICT_LINT=1` | Strict lint pass on the full repo |
| `SEKAI_DOCS_LINT=1` | Markdown gate on changed lines |
| `SEKAI_DOCS_LINKS=1` | Link check on added links only |

Skip the hook for rapid iteration with `git push --no-verify`. CI runs the same checks regardless.

## Local secret management

Sekai supports layered secret management for local development.

**Storage options:**

1. **Environment variables** (recommended for development) — copy `.env.example` to `.env` and fill in values. `.env` is git-ignored.
2. **Config file** (`~/.sekai/config.toml`) — when `secrets.encrypt = true` (default), values are encrypted with the key at `~/.sekai/.secret_key`. Use `sekai onboard` for guided setup.
...
3. Generic env vars (`SEKAI_API_KEY`, `API_KEY`).

Provider/model overrides via `SEKAI_PROVIDER`, `PROVIDER`, `SEKAI_MODEL`. See `.env.example` for the supported list.

**Never commit:** `.env`, API keys / tokens / passwords / OAuth tokens / webhook signing secrets, `~/.sekai/.secret_key`, or any personal identifier in tests or fixtures. The full content discipline is in **[Privacy & PII](docs/book/src/contributing/privacy.md)**.
...
- **Bugs** — use the bug template; include OS, `sekai --version`, and `sekai doctor` output.
