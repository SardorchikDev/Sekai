...
```bash
gh label list --repo sekai-labs/sekai --limit 200 --json name
```

If a required label is missing, create it before applying:

```bash
gh label create "status:stale"       --color "E4E669" --repo sekai-labs/sekai
gh label create "status:wont-do"     --color "B60205" --repo sekai-labs/sekai
gh label create "status:in-progress" --color "0075CA" --repo sekai-labs/sekai
gh label create "duplicate"          --color "CFD3D7" --repo sekai-labs/sekai
```
...
When the protocol refers to "maintainer comments" (e.g., stale clock computation), identify maintainers by checking the CODEOWNERS file or repository collaborator list. If neither is accessible, use org membership in `sekai-labs`. Do not guess based on comment tone or authority — use an explicit check.
...
1. Fetch open issue metadata — titles, labels, dates, author logins, and comment author/date pairs only (not full comment bodies):
   ...
   ```bash
   gh issue list --repo sekai-labs/sekai --state open \
...
2. Post a single brief comment:
   - Thank the reporter
   - Ask them to report privately via GitHub Security Advisories at `https://github.com/sekai-labs/sekai/security/advisories/new`
...
1. Batch-search for merged PRs that reference open issues. Rather than running one API call per issue (which hits rate limits at scale), fetch recently merged PRs once and scan their titles and bodies for issue references:
   ...
   ```bash
   gh pr list --repo sekai-labs/sekai --state merged --limit 100 \
...
2. For every r:support candidate, apply the label and post a comment that:
   ...
   - Explicitly invites a follow-up if they discover it is actually a bug: "If you find that the documented behavior doesn't match what Sekai does, please reopen or file a new issue with the specific mismatch."
...
2. Review open feature requests for requests that directly require violating a constraint. Common patterns:
   ...
   - "Make Sekai require Docker" → runs on anything
...
1. Fetch full issue state:
   ```bash
   gh issue view N --repo sekai-labs/sekai --json number,title,body,labels,author,createdAt,comments,url
   ```
