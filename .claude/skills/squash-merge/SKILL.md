...
Squash-merge a PR into `upstream/master` (sekai-labs/sekai) with fully preserved commit history in the squash message body. Use this skill when the user explicitly mentions squash-merging, merging a specific PR number, or landing a PR by number — e.g. "squash-merge #123", "merge PR 456", "land #789", "/squash-merge 123". Do **not** trigger on vague phrases like "ship it" or "merge it" without a PR number or clear upstream-merge context.
...
```bash
NUMBER=$(gh pr view <PR_NUMBER_OR_URL> --repo sekai-labs/sekai --json number --jq '.number')
```

Then fetch PR metadata:

```bash
gh pr view "$NUMBER" --repo sekai-labs/sekai \
...
```bash
REVIEW_DECISION=$(gh pr view "$NUMBER" --repo sekai-labs/sekai \
...
```bash
COMMITS=$(gh pr view "$NUMBER" --repo sekai-labs/sekai \
  --json commits \
...
```bash
BASE_REF=$(gh pr view "$NUMBER" --repo sekai-labs/sekai --json baseRefName --jq '.baseRefName')
HEAD_REF=$(gh pr view "$NUMBER" --repo sekai-labs/sekai --json headRefName --jq '.headRefName')
...
```bash
SHA=$(gh pr view "$NUMBER" --repo sekai-labs/sekai --json commits --jq '.commits[-1].oid')
...
```bash
PR_TITLE=$(gh pr view "$NUMBER" --repo sekai-labs/sekai --json title --jq '.title')
...
**About to run:**
```
gh pr merge $NUMBER --repo sekai-labs/sekai --squash \
  --subject "$SUBJECT" \
  --body "$COMMITS"
```
